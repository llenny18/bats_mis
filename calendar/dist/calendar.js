/*! Calendar.js v2.12.0 | (c) Bunoon 2024 | MIT License */
function calendarJs(elementOrId, options, searchOptions) {
  var _that = this,
    ...
    _value = { notFound: -1 },
    _enum_Day = { monday: 0, saturday: 5, sunday: 6 },
    _enum_KeyCodes = {
      enter: 13,
      escape: 27,
      left: 37,
    ...
    _enum_RepeatCustomType = { daily: 0, weekly: 1, monthly: 2, yearly: 3 },
    _options = {},
    _options_Search = {},
    _options_EventType = {
  ...
  function build(newStartDateTime, fullRebuild, forceRefreshViews) {
    _calendar_CurrentDate = isDefinedDate(newStartDateTime)
      ? newStartDateTime
      : new Date();
    fullRebuild = isDefined(fullRebuild) ? fullRebuild : false;
    forceRefreshViews = isDefined(forceRefreshViews)
      ? forceRefreshViews
      : false;
  ...
  function buildLayout(fullRebuild, forceRefreshViews) {
    if (_options.isWidget) {
      ...
      _calendar_CurrentDate_IsToday = isDateTodaysMonthAndYear(
        _calendar_CurrentDate
      );
      var firstDay = new Date(
          _calendar_CurrentDate.getFullYear(),
          _calendar_CurrentDate.getMonth(),
          1
        ),
  ...
  function buildLayoutModalsAndMainView(
    startDay,
    fullRebuild,
    forceRefreshViews,
    firstRender
    ...
    if (_element_Calendar !== null) {
      ...
      if (firstRender && isDefinedString(_options.viewToOpenOnFirstLoad)) {
        if (
          _options.viewToOpenOnFirstLoad.toLowerCase() ===
            _element_View_Name.fullDay &&
          _options.views.fullDay.enabled
        ) {
          showFullDayView();
        } else if (
          _options.viewToOpenOnFirstLoad.toLowerCase() ===
            _element_View_Name.fullWeek &&
          _options.views.fullWeek.enabled
        ) {
          showFullWeekView();
        } else if (
          _options.viewToOpenOnFirstLoad.toLowerCase() ===
            _element_View_Name.fullYear &&
          _options.views.fullYear.enabled
        ) {
          showFullYearView();
        } else if (
          _options.viewToOpenOnFirstLoad.toLowerCase() ===
            _element_View_Name.timeline &&
          _options.views.timeline.enabled
        ) {
          showTimelineView();
        } else if (
          _options.viewToOpenOnFirstLoad.toLowerCase() ===
            _element_View_Name.allEvents &&
          _options.views.allEvents.enabled
        ) {
          showAllEventsView();
        }
      }
    ...
    if (_element_Calendar === null) {
      ...
      if (element !== null) {
        if (
          ...
          _element_Calendar.className = !isWidget
            ? "calendar"
            : "calendar-widget";
  ...
  function buildSideMenuDisabledBackground() {
    if (_element_SideMenu_DisabledBackground === null) {
      _element_SideMenu_DisabledBackground = createElement(
        "div",
  ...
  function buildFullSideMenu() {
    if (!_initialized_FirstTime) {
      _element_SideMenu = createElement(
        "div",
        "side-menu custom-scroll-bars dark-thumb-scroll"
      );
    ...
    createTextHeaderElement(
      _element_SideMenu_Header,
      _options.sideMenuHeaderText
    );
    buildToolbarButton(
      _element_SideMenu_Header,
      "ib-close",
    ...
    if (
      _options.configurationDialogEnabled ||
      _options.exportEventsEnabled ||
      (_options.importEventsEnabled && _options.manualEditingEnabled)
    ) {
      _element_SideMenu_Header.appendChild(
        createElement("div", "right-divider-line")
      );
    }
    ...
    if (
      (_options.configurationDialogEnabled && _options.exportEventsEnabled) ||
      (_options.importEventsEnabled && _options.manualEditingEnabled)
    ) {
      _element_SideMenu_Header.appendChild(
        createElement("div", "right-divider-line")
    ...
    if (_options.exportEventsEnabled) {
      _element_SideMenu_TitleBar_ExportEventsButton = buildToolbarButton(
        _element_SideMenu_Header,
    ...
    if (_options.importEventsEnabled && _options.manualEditingEnabled) {
      buildToolbarButton(
        ...
        _options.importEventsTooltipText,
        importEventsFromFileSelected
      );
    }
    if (!_initialized_FirstTime) {
  ...
  function buildSideMenuContent(openDays) {
    var isDaysOpen =
        isSideMenuContentOpen(_element_SideMenu_Content_Section_Days_Content) ||
        openDays === true,
      isEventTypesOpen = isSideMenuContentOpen(
        _element_SideMenu_Content_Section_EventTypes_Content,
        true
      ),
      isGroupsOpen = isSideMenuContentOpen(
        _element_SideMenu_Content_Section_Groups_Content,
        true
      ),
      isWorkingDaysOpen = isSideMenuContentOpen(
        _element_SideMenu_Content_Section_WorkingDays_Content,
        true
      ),
      isWeekendDaysOpen = isSideMenuContentOpen(
        _element_SideMenu_Content_Section_WeekendDays_Content,
        true
      );
  ...
  function updateSideMenuExportButtonVisibleState() {
    var viewOpen = getActiveView();
    if (viewOpen === null) {
      updateToolbarButtonVisibleState(
        _element_SideMenu_TitleBar_ExportEventsButton,
        _element_View_FullMonth_EventsShown.length > 0
      );
    } else {
      if (viewOpen === _element_View_FullDay) {
        updateToolbarButtonVisibleState(
          _element_SideMenu_TitleBar_ExportEventsButton,
          _element_View_FullDay_EventsShown.length > 0
        );
      } else if (viewOpen === _element_View_AllEvents) {
        updateToolbarButtonVisibleState(
          _element_SideMenu_TitleBar_ExportEventsButton,
          _element_View_AllEvents_EventsShown.length > 0
        );
      } else if (viewOpen === _element_View_FullWeek) {
        updateToolbarButtonVisibleState(
          _element_SideMenu_TitleBar_ExportEventsButton,
          _element_View_FullWeek_EventsShown.length > 0
        );
      } else if (viewOpen === _element_View_Timeline) {
        updateToolbarButtonVisibleState(
          _element_SideMenu_TitleBar_ExportEventsButton,
          _element_View_Timeline_EventsShown.length > 0
        );
      } else {
        updateToolbarButtonVisibleState(
          _element_SideMenu_TitleBar_ExportEventsButton,
  ...
  function isSideMenuAvailable() {
    return (
      _options.sideMenu.showDays ||
      _options.sideMenu.showEventTypes ||
      _options.sideMenu.showGroups ||
      _options.sideMenu.showWorkingDays ||
      _options.sideMenu.showWeekendDays
    );
  }
  ...
  function isSideMenuOpen() {
    return (
      _element_SideMenu !== null &&
      _element_SideMenu.className.indexOf("side-menu-open") > _value.notFound
    );
  }
  function saveSideMenuSelections() {
    if (_element_SideMenu_Changed) {
      ...
      if (_element_SideMenu_Content_Section_Groups !== null) {
        var visibleGroups = getSideMenuCheckedCheckBoxNames(
          _element_SideMenu_Content_Section_Groups
        );
        if (
          !areArraysTheSame(_options_Configuration.visibleGroups, visibleGroups)
        ) {
          _options_Configuration.visibleGroups = [].slice.call(visibleGroups);
          itemWasChanged = true;
          fireCustomTrigger(
            _options.events.onVisibleGroupsChanged,
            _options_Configuration.visibleGroups
          );
        }
      }
      if (_element_SideMenu_Content_Section_EventTypes !== null) {
        var visibleEventTypes = getSideMenuCheckedCheckBoxNames(
          _element_SideMenu_Content_Section_EventTypes,
          true
        );
        if (
          !areArraysTheSame(
          ...
          _options_Configuration.visibleEventTypes = [].slice.call(
            visibleEventTypes
          );
          itemWasChanged = true;
          fireCustomTrigger(
            _options.events.onVisibleEventTypesChanged,
            _options_Configuration.visibleEventTypes
          );
        }
      }
      if (_element_SideMenu_Content_Section_Days !== null) {
        var visibleDays = getSideMenuCheckedCheckBoxNames(
          _element_SideMenu_Content_Section_Days,
          true
        );
        if (
          visibleDays.length >= 1 &&
          !areArraysTheSame(_options.visibleDays, visibleDays)
        ) {
      ...
      if (_element_SideMenu_Content_Section_WorkingDays !== null) {
        var workingDays = getSideMenuCheckedCheckBoxNames(
          _element_SideMenu_Content_Section_WorkingDays,
          true
        );
      ...
      if (_element_SideMenu_Content_Section_WeekendDays !== null) {
        var weekendDays = getSideMenuCheckedCheckBoxNames(
          _element_SideMenu_Content_Section_WeekendDays,
          true
        );
  ...
  function getSideMenuCheckedCheckBoxNames(container, isNumericName) {
    ...
    if (checkboxesLength > 0) {
      for (
        var checkboxIndex = 0;
        checkboxIndex < checkboxesLength;
        checkboxIndex++
      ) {
  ...
  function buildSideMenuWeekendDays(opened) {
    ...
    _element_SideMenu_Content_Section_WeekendDays = createElement(
      "div",
      "content-section content-section-opened"
    );
    _element_SideMenu_Content_Section_WeekendDays_Content = createElement(
      "div",
      "checkbox-container"
    );
    _element_SideMenu_Content.appendChild(
      _element_SideMenu_Content_Section_WeekendDays
    );
    var dayIndex = 1,
      dayEndIndex = 8,
      ...
      checkBoxes = [];
    buildSideMenuHeaderAndContentOpener(
      _element_SideMenu_Content_Section_WeekendDays,
      _element_SideMenu_Content_Section_WeekendDays_Content,
      _options.weekendDaysText,
      opened,
      checkBoxes
    );
    _element_SideMenu_Content_Section_WeekendDays.appendChild(
      _element_SideMenu_Content_Section_WeekendDays_Content
    );
    if (
      _options.startOfWeekDay === _enum_Day.saturday ||
      _options.startOfWeekDay === _enum_Day.sunday
    ) {
    ...
    for (; dayIndex < dayEndIndex; dayIndex++) {
      var actualDayIndex1 = dayIndex > 6 ? 0 : dayIndex,
        visible1 =
          _options.weekendDays.indexOf(actualDayIndex1) > _value.notFound;
      checkBoxes.push(
        buildCheckBox(
          _element_SideMenu_Content_Section_WeekendDays_Content,
          _options.dayNames[dayIndex - 1],
          sideMenuSelectionsChanged,
          actualDayIndex1.toString(),
          visible1,
          null,
          cancelBubbleOnly
        )[0]
    ...
    if (processOtherDays) {
      ...
      for (; dayIndex < dayEndIndex; dayIndex++) {
        var actualDayIndex2 = dayIndex > 6 ? 0 : dayIndex,
          visible2 =
            _options.weekendDays.indexOf(actualDayIndex2) > _value.notFound;
        checkBoxes.push(
          buildCheckBox(
            _element_SideMenu_Content_Section_WeekendDays_Content,
            _options.dayNames[dayIndex - 1],
            sideMenuSelectionsChanged,
            actualDayIndex2.toString(),
            visible2,
            null,
            cancelBubbleOnly
          )[0]
  ...
  function buildSideMenuWorkingDays(opened) {
    opened = isDefined(opened) ? opened : true;
    _element_SideMenu_Content_Section_WorkingDays = createElement(
      "div",
      "content-section content-section-opened"
    );
    _element_SideMenu_Content_Section_WorkingDays_Content = createElement(
      "div",
      "checkbox-container"
    );
    _element_SideMenu_Content.appendChild(
    ...
    buildSideMenuHeaderAndContentOpener(
      _element_SideMenu_Content_Section_WorkingDays,
      _element_SideMenu_Content_Section_WorkingDays_Content,
      _options.workingDaysText,
      opened,
      checkBoxes
    );
    _element_SideMenu_Content_Section_WorkingDays.appendChild(
      _element_SideMenu_Content_Section_WorkingDays_Content
    );
    if (
      _options.startOfWeekDay === _enum_Day.saturday ||
  ...
  function buildSideMenuGroups(opened) {
    ...
    if (groupsLength > 0) {
      var checkBoxes = [];
      _element_SideMenu_Content_Section_Groups = createElement(
        "div",
        "content-section content-section-opened"
      );
      _element_SideMenu_Content_Section_Groups_Content = createElement(
        "div",
        "checkbox-container"
      );
      _element_SideMenu_Content.appendChild(
      ...
      _element_SideMenu_Content_Section_Groups.appendChild(
        _element_SideMenu_Content_Section_Groups_Content
      );
      for (var groupIndex = 0; groupIndex < groupsLength; groupIndex++) {
        ...
        if (isDefined(_options_Configuration.visibleGroups)) {
          visible =
            _options_Configuration.visibleGroups.indexOf(configGroupName) >
            _value.notFound;
        }
        checkBoxes.push(
          buildCheckBox(
    ...
    for (var eventType in _options_EventType) {
      if (_options_EventType.hasOwnProperty(eventType)) {
        if (!sectionAndHeaderAdded) {
          _element_SideMenu_Content_Section_EventTypes = createElement(
            "div",
            "content-section content-section-opened"
          );
          _element_SideMenu_Content.appendChild(
          ...
          _element_SideMenu_Content_Section_EventTypes_Content = createElement(
            "div",
            "checkbox-container"
          );
          buildSideMenuHeaderAndContentOpener(
            _element_SideMenu_Content_Section_EventTypes,
            _element_SideMenu_Content_Section_EventTypes_Content,
            _options.eventTypesText,
            opened,
            checkBoxes
          );
          _element_SideMenu_Content_Section_EventTypes.appendChild(
            _element_SideMenu_Content_Section_EventTypes_Content
          );
        ...
        if (isDefined(_options_Configuration.visibleEventTypes)) {
          visible =
            _options_Configuration.visibleEventTypes.indexOf(
              parseInt(eventType)
            ) > _value.notFound;
        }
        checkBoxes.push(
          buildCheckBox(
            _element_SideMenu_Content_Section_EventTypes_Content,
            _options_EventType[eventType].text,
            sideMenuSelectionsChanged,
            eventType,
            visible,
            null,
  ...
  function buildSideMenuDays(opened) {
    opened = isDefined(opened) ? opened : true;
    _element_SideMenu_Content_Section_Days = createElement(
      "div",
      "content-section content-section-opened"
    );
    _element_SideMenu_Content_Section_Days_Content = createElement(
      "div",
      "checkbox-container"
    );
    _element_SideMenu_Content.appendChild(
    ...
    buildSideMenuHeaderAndContentOpener(
      _element_SideMenu_Content_Section_Days,
      _element_SideMenu_Content_Section_Days_Content,
      _options.sideMenuDaysText,
      opened,
      checkBoxes
    );
    _element_SideMenu_Content_Section_Days.appendChild(
      _element_SideMenu_Content_Section_Days_Content
    );
    if (
      _options.startOfWeekDay === _enum_Day.saturday ||
  ...
  function buildSideMenuHeaderAndContentOpener(
    mainContainer,
    mainContent,
    text,
    opened,
    ...
    var selectAll = buildToolbarButton(
      header,
      "ib-square",
      _options.selectAllText,
      function (e) {
        changeCheckboxesCheckedState(e, checkBoxes, true);
      }
    );
    var selectNone = buildToolbarButton(
      header,
      "ib-square-hollow",
      _options.selectNoneText,
      function (e) {
    ...
    header.onclick = function () {
      ...
      mainContainer.className = isClosed
        ? "content-section content-section-opened"
        : "content-section";
  ...
  function buildWidgetModeEvent(events, eventDetails) {
    ...
    if (
      !fireCustomTrigger(
        _options.events.onWidgetEventRender,
        event,
        eventDetails
      )
    ) {
    ...
    if (
      ...
      events.className.indexOf(" scroll-margin") === _value.notFound
    ) {
  ...
  function buildDatePickerMode(element) {
    ...
    _element_Mode_DatePicker_Input = createElement(
      "input",
      "calendar-date-picker-input"
    );
    _element_Mode_DatePicker_Input.readOnly = true;
    _element_Mode_DatePicker_Input.placeholder =
      _options.selectDatePlaceholderText;
  ...
  function toggleDatePickerModeVisible(e) {
    cancelBubble(e);
    hideAllElementsAcrossInstances(
      "calendar calendar-shown",
    ...
    if (!_element_Mode_DatePicker_Visible) {
      ...
      fireCustomTrigger(
        _options.events.onDatePickerOpened,
        _parameter_ElementID
      );
      ...
      hideAllDropDowns();
      fireCustomTrigger(
        _options.events.onDatePickerClosed,
        _parameter_ElementID
      );
    }
  ...
  function updateDatePickerPosition() {
    ...
    if (top + _element_Calendar.offsetHeight > _parameter_Window.innerHeight) {
      var calendarBorderWidth = getStyleValueByName(
          _element_Calendar,
          "border-width"
        ),
        inputBorderWidth = getStyleValueByName(
          _element_Mode_DatePicker_Input,
          "border-width"
        );
      actualTop -=
        _element_Calendar.offsetHeight +
        _element_Mode_DatePicker_Input.clientHeight +
  ...
  function updateDatePickerInputValueDisplay(date) {
    _element_Mode_DatePicker_Input.value = getCustomFormattedDateText(
      _options.views.datePicker.selectedDateFormat,
      date
    );
    _element_Mode_DatePicker_HiddenInput.value =
      date.getFullYear() +
      "-" +
  ...
  function getDataPickerInputValueDate() {
    var values = _element_Mode_DatePicker_HiddenInput.value.split("/"),
      valuesDate = null;
    if (values.length === 3) {
      var newDateFromValues = new Date(
        parseInt(values[2]),
        parseInt(values[1]) - 1,
        parseInt(values[0])
      );
  ...
  function isDateValidForDatePicker(newDate) {
    ...
    if (_options.views.datePicker.minimumDate !== null) {
      newDateAllowed = isDateSmallerOrEqualToDate(
        _options.views.datePicker.minimumDate,
        newDate
      );
    }
    if (newDateAllowed && _options.views.datePicker.maximumDate !== null) {
      newDateAllowed = isDateSmallerOrEqualToDate(
        newDate,
        _options.views.datePicker.maximumDate
      );
    }
  ...
  function handleDocumentEvents(addEvents) {
    ...
    var documentBodyFunc = addEvents
        ? _parameter_Document.body.addEventListener
        : _parameter_Document.body.removeEventListener,
      documentFunc = addEvents
        ? _parameter_Document.addEventListener
        : _parameter_Document.removeEventListener,
      windowFunc = addEvents
        ? _parameter_Window.addEventListener
        : _parameter_Window.removeEventListener;
  ...
  function hideAllDropDowns(hideSearchHistoryDropDown) {
    ...
    hideSearchHistoryDropDown = isDefined(hideSearchHistoryDropDown)
    ...
    if (!_element_Mode_DatePicker_Enabled) {
      if (!isSideMenuOpen()) {
        if (_element_Calendar_FullScreenModeOn) {
          ...
          if (
            isControlKey(e) &&
            e.keyCode === _enum_KeyCodes.left &&
            isMainDisplayVisible
          ) {
            e.preventDefault();
            moveBackYear();
          } else if (
            isControlKey(e) &&
            e.keyCode === _enum_KeyCodes.right &&
            isMainDisplayVisible
          ) {
            e.preventDefault();
            moveForwardYear();
            ...
            if (
              !closeActiveDialog() &&
              isMainDisplayVisible &&
              _options.useEscapeKeyToExitFullScreenMode
            ) {
        ...
        if (
          ...
          isControlKey(e) &&
          isShiftKey(e) &&
          e.keyCode === _enum_KeyCodes.f
        ) {
          onFKey(e);
        } else if (
          isControlKey(e) &&
          isShiftKey(e) &&
          e.keyCode === _enum_KeyCodes.m
        ) {
          e.preventDefault();
  ...
  function closeActiveDialog() {
    ...
    if (!done && _element_Dialog_AllOpened.length > 0) {
      var lastFunc =
        _element_Dialog_AllOpened[_element_Dialog_AllOpened.length - 1];
  ...
  function buildFullDayView() {
    if (!_element_Mode_DatePicker_Enabled && _options.views.fullDay.enabled) {
      ...
      if (
        _options.views.fullDay.showExtraTitleBarButtons &&
        _options.fullScreenModeEnabled
      ) {
        _element_View_FullDay_FullScreenButton = buildToolbarButton(
          titleBar,
        ...
        titleBar.appendChild(
          createElement("div", "right-divider-line-full-screen-mode")
        );
      }
      buildToolbarButton(
        titleBar,
          ...
          function () {
            if (_options.useTemplateWhenAddingNewEvent) {
              var newBlankTemplateEvent = buildBlankTemplateEvent(
                _element_View_FullDay_DateSelected,
                _element_View_FullDay_DateSelected
              );
      ...
      if (_options.views.timeline.enabled) {
        ...
        buildToolbarButton(
          titleBar,
          "ib-bar-graph",
          _options.viewTimelineTooltipText,
          function () {
      ...
      if (_options.views.fullDay.showExtraTitleBarButtons) {
        _element_View_FullDay_TodayButton = buildToolbarButton(
          titleBar,
          "ib-pin",
          _options.todayTooltipText,
          onCurrentFullDay
        ...
        if (_options_Search.enabled) {
          _element_View_FullDay_SearchButton = buildToolbarButton(
            titleBar,
            "ib-search",
      ...
      _element_View_FullDay_Contents = createElement(
        "div",
      ...
      _element_View_FullDay_Contents.oncontextmenu = function (e) {
        var hoursMinutes = getHourMinutesFromMousePositionClick(
          e,
          _element_View_FullDay_Contents_Hours
        );
        _element_ContextMenu_FullDay_ClickPositionHourMinutes =
          padNumber(hoursMinutes[0]) + ":" + padNumber(hoursMinutes[1]);
        showFullDayContextMenu(
          e,
          _element_View_FullDay_DateSelected,
          _element_View_FullDay_EventsShown
        );
      };
      _element_View_FullDay_Contents_AllDayEvents = createElement(
        "div",
        "content-events-all-day"
      );
      _element_View_FullDay_Contents.appendChild(
        _element_View_FullDay_Contents_AllDayEvents
      );
      ...
      _element_View_FullDay_Contents_Hours = createElement(
        "div",
        "contents-events"
      );
      _element_View_FullDay_Contents_Hours.ondblclick =
        onFullDayViewDoubleClick;
      _element_View_FullDay_Contents.appendChild(
        _element_View_FullDay_Contents_Hours
      );
      _element_View_FullDay_Contents_WorkingHours = createElement(
        "div",
        "working-hours"
      );
      _element_View_FullDay_Contents.appendChild(
        _element_View_FullDay_Contents_WorkingHours
      );
      if (
        _options.manualEditingEnabled &&
        _options.dragAndDropForEventsEnabled
      ) {
      ...
      buildHoursForTimeBasedView(
        _element_View_FullDay_Contents_Hours,
        _options.views.fullDay.minutesBetweenSections
      );
  ...
  function buildFullDayViewWorkingHours() {
    if (
      _options.workingHoursStart !== null &&
      _options.workingHoursEnd !== null
    ) {
      var workingHoursStart = null,
        workingHoursEnd = null;
      if (isDefinedObject(_options.workingHoursStart)) {
        var startWeekdayNumber = getWeekdayNumber(
          _element_View_FullDay_DateSelected
        );
        if (
          _options.workingHoursStart.hasOwnProperty(
            startWeekdayNumber.toString()
          )
        ) {
          workingHoursStart =
            _options.workingHoursStart[startWeekdayNumber.toString()];
        }
      } else {
        workingHoursStart = _options.workingHoursStart;
      }
      if (isDefinedObject(_options.workingHoursEnd)) {
        var endWeekdayNumber = getWeekdayNumber(
          _element_View_FullDay_DateSelected
        );
        if (
          _options.workingHoursEnd.hasOwnProperty(endWeekdayNumber.toString())
        ) {
          workingHoursEnd =
            _options.workingHoursEnd[endWeekdayNumber.toString()];
        }
      } else {
      ...
      if (
        ...
        var pixelsPerMinute = getPixelsPerMinuteForHeight(
            _element_View_FullDay_Contents_Hours
          ),
          workingHoursStartParts = workingHoursStart.split(":"),
          workingHoursEndParts = workingHoursEnd.split(":"),
          top =
            (parseInt(workingHoursStartParts[0]) * 60 +
              parseInt(workingHoursStartParts[1])) *
            pixelsPerMinute,
          height =
            (parseInt(workingHoursEndParts[0]) * 60 +
              parseInt(workingHoursEndParts[1])) *
              pixelsPerMinute -
            top;
        ...
        _element_View_FullDay_Contents_WorkingHours.style.height =
          height + "px";
      }
    }
  }
  function buildFullDayViewRepeatedDayEvents(
    eventDetails,
    orderedEvents,
    date,
    dateFunc,
    dateFuncForwardValue
  ) {
    ...
    while (newFromDate < date) {
      ...
      var repeatEnded = !(
        !isDefined(eventDetails.repeatEnds) ||
        isDateSmallerOrEqualToDate(newFromDate, eventDetails.repeatEnds)
      );
      if (
        excludeDays.indexOf(newFromDate.getDay()) === _value.notFound &&
        !repeatEnded
      ) {
  ...
  function buildFullDayViewEvent(eventDetails, displayDate) {
    ...
    if (
      isEventVisible(eventDetails) &&
      seriesIgnoreDates.indexOf(formattedDate) === _value.notFound
    ) {
      var event = createElement("div", "event");
      ...
      setEventClassesAndColors(
        event,
        eventDetails,
        getToTimeWithPassedDate(eventDetails, displayDate)
      );
      ...
      if (
        !fireCustomTrigger(
          _options.events.onFullDayEventRender,
          event,
          eventDetails
        )
      ) {
        ...
        if (
          ...
          _options.views.fullDay.showAllDayEventDetails
        ) {
          var startTime = createElement("div", "date");
          event.appendChild(startTime);
          var duration = createElement("div", "duration");
      ...
      if (!eventDetails.isAllDay) {
        scrollTop = setEventPositionAndGetScrollTop(
          _element_View_FullDay_Contents,
          _element_View_FullDay_Contents_Hours,
          displayDate,
          event,
          eventDetails
  ...
  function buildFullDayViewTimeArrow() {
    ...
    _element_View_FullDay_Contents_Hours.appendChild(
      _element_View_FullDay_TimeArrow
    );
    _element_View_FullDay_TimeArrow.appendChild(
      createElement("div", "arrow-left")
    );
  ...
  function buildFullDayViewTitle() {
    if (
      !fireCustomTrigger(
        _options.events.onFullDayTitleRender,
        _element_View_FullDay_DateSelected
      )
    ) {
      buildDateTimeDisplay(
        _element_View_FullDay_TitleBar,
        _element_View_FullDay_DateSelected,
        false,
        true,
        true
  ...
  function showFullDayView(date, fromOpen) {
    ...
    var currentDate = new Date(),
      ...
      isCurrentDateVisible =
        _options.visibleDays.indexOf(weekDayNumber) > _value.notFound;
    ...
    updateToolbarButtonVisibleState(
      _element_View_FullDay_TodayButton,
      isCurrentDateVisible
    );
    ...
    var orderedEventsLength = orderedEvents.length,
      ...
      timeArrowPosition = updateViewTimeArrowPosition(
        _element_View_FullDay_DateSelected,
        _element_View_FullDay,
        _element_View_FullDay_TimeArrow,
        _element_View_FullDay_Contents_Hours,
        _options.views.fullDay.showTimelineArrow
      );
    for (
      var orderedEventIndex = 0;
      orderedEventIndex < orderedEventsLength;
      ...
      var newTopPosition = buildFullDayViewEvent(
        orderedEvents[orderedEventIndex],
        date
      );
    ...
    if (fromOpen) {
      if (
        ...
        var allDayEventsHeight =
          _element_View_FullDay_Contents_AllDayEvents.offsetHeight;
        allDayEventsHeight =
          allDayEventsHeight <= 1 ? _options.spacing * 4 : allDayEventsHeight;
        _element_View_FullDay_Contents.scrollTop =
          timeArrowPosition - allDayEventsHeight;
      } else {
        _element_View_FullDay_Contents.scrollTop =
          orderedEventsFirstTopPosition -
          _element_View_FullDay_Contents.offsetHeight / 2;
      }
    }
    ...
    updateToolbarButtonVisibleState(
      _element_View_FullDay_SearchButton,
      _element_View_FullDay_EventsShown.length > 0
    );
  ...
  function getFullDayViewOrderedEvents(date, orderedEvents) {
    getAllEventsFunc(function (eventDetails) {
      var totalDays =
          getTotalDaysBetweenDates(eventDetails.from, eventDetails.to) + 1,
      ...
      if (repeatEvery > _enum_RepeatType.never) {
        if (repeatEvery === _enum_RepeatType.everyDay) {
          buildFullDayViewRepeatedDayEvents(
            eventDetails,
            orderedEvents,
            date,
            moveDateForwardDay,
          ...
          var repeatEveryCustomType = getNumber(
              eventDetails.repeatEveryCustomType
            ),
            repeatEveryCustomValue = getNumber(
              eventDetails.repeatEveryCustomValue
            );
          if (repeatEveryCustomValue > 0) {
            if (repeatEveryCustomType === _enum_RepeatCustomType.daily) {
              ...
              repeatEveryCustomType === _enum_RepeatCustomType.weekly
            ) {
              buildFullDayViewRepeatedDayEvents(
                eventDetails,
                orderedEvents,
              ...
              repeatEveryCustomType === _enum_RepeatCustomType.monthly
            ) {
              buildFullDayViewRepeatedDayEvents(
                eventDetails,
                orderedEvents,
              ...
              repeatEveryCustomType === _enum_RepeatCustomType.yearly
            ) {
              buildFullDayViewRepeatedDayEvents(
                eventDetails,
                orderedEvents,
  ...
  function onFullDayViewDoubleClick(e) {
    if (_options.manualEditingEnabled) {
      ...
      if (_options.useTemplateWhenAddingNewEvent) {
        var newBlankTemplateEventTime =
            padNumber(hoursMinutes[0]) + ":" + padNumber(hoursMinutes[1]),
          newBlankTemplateEvent = buildBlankTemplateEvent(
            _element_View_FullDay_DateSelected,
            _element_View_FullDay_DateSelected,
            newBlankTemplateEventTime,
            newBlankTemplateEventTime
          );
        showEventEditingDialog(newBlankTemplateEvent);
        showEventEditingDialogTitleSelected();
  ...
  function buildFullWeekView() {
    if (!_element_Mode_DatePicker_Enabled && _options.views.fullWeek.enabled) {
      ...
      titleBar.appendChild(createElement("div", "right-divider-line"));
      if (
        _options.views.fullWeek.showExtraTitleBarButtons &&
        _options.fullScreenModeEnabled
      ) {
        _element_View_FullWeek_FullScreenButton = buildToolbarButton(
          titleBar,
          "ib-arrow-expand-left-right",
          _options.enableFullScreenTooltipText,
          fullScreenModeHeaderDoubleClick
      ...
      if (
        ...
        buildToolbarButton(
          ...
          function () {
            showEventEditingDialog(
              null,
              new Date(_element_View_FullWeek_DateSelected_StartOfWeek)
            );
          }
        );
      }
      ...
      buildToolbarButton(
        titleBar,
        "ib-arrow-left-full",
        _options.previousWeekTooltipText,
        onPreviousFullWeek
      );
      if (_options.views.fullWeek.showExtraTitleBarButtons) {
        ...
        if (_options_Search.enabled) {
          _element_View_FullWeek_SearchButton = buildToolbarButton(
            titleBar,
            "ib-search",
            _options.searchTooltipText,
            showSearchDialog
      ...
      _element_View_FullWeek_Contents = createElement(
        "div",
        "contents custom-scroll-bars"
      );
  ...
  function buildFullWeekViewLayout() {
    ...
    if (_options.views.fullWeek.showDayNamesHeaders) {
      _element_View_FullWeek_Contents_DayNamesHeader = createElement(
        "div",
        "row-cells header-days"
      );
      dayNamesHeaderContainer.appendChild(
        _element_View_FullWeek_Contents_DayNamesHeader
      );
    }
    _element_View_FullWeek_Contents_AllDayEvents = createElement(
      "div",
      "content-events-all-day"
    );
    _element_View_FullWeek_Contents.appendChild(
      _element_View_FullWeek_Contents_AllDayEvents
    );
    _element_View_FullWeek_Contents_Days_AllDay = createElement(
      "div",
      "row-cells days"
    );
    _element_View_FullWeek_Contents_AllDayEvents.appendChild(
      _element_View_FullWeek_Contents_Days_AllDay
    );
    var allDayText = createElement("div", "all-day-text");
    setNodeText(allDayText, _options.allDayText);
    ...
    _element_View_FullWeek_Contents.appendChild(
      _element_View_FullWeek_Contents_Hours
    );
    buildHoursForTimeBasedView(
      _element_View_FullWeek_Contents_Hours,
      _options.views.fullWeek.minutesBetweenSections
    );
    _element_View_FullWeek_Contents_Days = createElement(
      "div",
      "row-cells days"
    );
    _element_View_FullWeek_Contents_Hours.appendChild(
      _element_View_FullWeek_Contents_Days
    );
  }
  function buildFullWeekViewDayColumns(weekStartDate, weekEndDate) {
    ...
    if (
      ...
      buildFullWeekViewDayColumn(
        _options.startOfWeekDay,
        headerNamesLength,
        droppableDates
      );
  ...
  function buildFullWeekDayViewColumnWithEvents(
    droppableDates,
    headerNameIndex
  ) {
    ...
    column.oncontextmenu = function (e) {
      ...
      showFullDayContextMenu(
        e,
        columnDate,
        _element_View_FullWeek_EventsShown_PerDay[headerNameIndex]
      );
    };
  }
  function buildFullWeekViewDayColumnTimeArrow(column, columnDate) {
    ...
    _element_View_FullWeek_TimeArrow.appendChild(
      createElement("div", "arrow-left")
    );
    _element_View_FullWeek_TimeArrow.appendChild(createElement("div", "line"));
    _element_View_FullWeek_TimeArrow_Position = updateViewTimeArrowPosition(
      columnDate,
      _element_View_FullWeek,
      _element_View_FullWeek_TimeArrow,
      column,
      _options.views.fullWeek.showTimelineArrow
    );
  }
  function buildFullWeekViewDayColumnWorkingHours(column, headerNameIndex) {
    if (
      _options.workingHoursStart !== null &&
      _options.workingHoursEnd !== null &&
      isIndexWorkingDay(headerNameIndex)
    ) {
      var workingHoursStart = null,
        workingHoursEnd = null;
      if (
        isDefinedObject(_options.workingHoursStart) &&
        _options.workingHoursStart.hasOwnProperty(headerNameIndex.toString())
      ) {
        workingHoursStart =
          _options.workingHoursStart[headerNameIndex.toString()];
      } else {
        workingHoursStart = _options.workingHoursStart;
      }
      if (
        isDefinedObject(_options.workingHoursEnd) &&
        _options.workingHoursEnd.hasOwnProperty(headerNameIndex.toString())
      ) {
  ...
  function buildFullWeekViewTitle(weekStartDate, weekEndDate) {
    ...
    if (
      !fireCustomTrigger(
        _options.events.onFullWeekTitleRender,
        weekStartDate,
        weekEndDate
      )
    ) {
      if (_options.views.fullWeek.showWeekNumbersInTitles) {
        createSpanElement(
          _element_View_FullWeek_TitleBar,
          _options.weekText +
            _string.space +
            getWeekNumber(weekStartDate) +
            ": "
        );
      }
      if (_options.reverseOrderDaysOfWeek) {
  ...
  function buildFullWeekViewTitleDate(from, to) {
    if (from.getFullYear() === to.getFullYear()) {
      if (from.getMonth() === to.getMonth()) {
        ...
        createSpanElement(
          _element_View_FullWeek_TitleBar,
          _options.monthNames[from.getMonth()]
        );
      } else {
        buildDateTimeDisplay(
          _element_View_FullWeek_TitleBar,
  ...
  function buildFullWeekViewRepeatedDayEvents(
    orderedEvent,
    weekStartDate,
    weekEndDate,
    dateFunc,
    ...
    while (newFromDate < weekEndDate) {
      dateFunc(newFromDate, dateFuncForwardValue);
      var repeatEnded = !(
        !isDefined(orderedEvent.repeatEnds) ||
        isDateSmallerOrEqualToDate(newFromDate, orderedEvent.repeatEnds)
      );
      if (
        excludeDays.indexOf(newFromDate.getDay()) === _value.notFound &&
        !repeatEnded
        ...
        if (newFromDate >= weekStartDate && newFromDate <= weekEndDate) {
          var column = getElementByID(
              _element_ID_WeekDayElement + getWeekdayNumber(newFromDate)
            ),
            columnAllDay = getElementByID(
              _element_ID_WeekAllDayElement + getWeekdayNumber(newFromDate)
            );
          if (column !== null) {
            buildFullWeekViewEvent(
              column,
              columnAllDay,
              orderedEvent,
              newFromDate
  ...
  function buildFullWeekViewEvent(
    column,
    columnAllDay,
    eventDetails,
    displayDate
    ...
    if (
      ...
      if (eventDetails.isAllDay) {
        ...
        if (
          _options.manualEditingEnabled &&
          _options.dragAndDropForEventsEnabled &&
          !isEventLocked(eventDetails)
        ) {
          if (
            doDatesMatch(eventDetails.from, eventDetails.to) &&
            !eventDetails.isAllDay
          ) {
            event.className += " resizable";
          }
      ...
      setEventClassesAndColors(
        event,
        eventDetails,
        getToTimeWithPassedDate(eventDetails, actualDisplayDate)
      );
      setEventCustomTriggers(event, eventDetails, editEventDate);
      ...
      if (
        !fireCustomTrigger(
          _options.events.onFullWeekEventRender,
          event,
          eventDetails
        )
      ) {
        ...
        if (
          !eventDetails.isAllDay ||
          _options.views.fullWeek.showAllDayEventDetails
        ) {
          var startTime = createElement("div", "date");
          event.appendChild(startTime);
          var duration = createElement("div", "duration");
      ...
      if (!eventDetails.isAllDay) {
        setEventPositionAndGetScrollTop(
          column,
          column,
          actualDisplayDate,
          event,
      ...
      if (
        !_element_View_FullWeek_EventsShown_PerDay.hasOwnProperty(weekdayNumber)
      ) {
      ...
      if (_element_View_FullWeek_Contents_SmallestEventTop === 0) {
        ...
        _element_View_FullWeek_Contents_SmallestEventTop = _parameter_Math.min(
          _element_View_FullWeek_Contents_SmallestEventTop,
          event.offsetTop
        );
      }
      _element_View_FullWeek_EventsShown_PerDay[weekdayNumber].push(
        eventDetails
      );
      added = true;
    }
  ...
  function buildFullWeekViewDayNameHeaderDates(weekStartDate, weekEndDate) {
    if (
      _element_View_FullWeek_Contents_DayNamesHeader !== null &&
      _options.views.fullWeek.showDatesInDayHeaders
    ) {
      ...
      while (fromDate < weekEndDate) {
        ...
        if (_options.visibleDays.indexOf(weekDayNumber) > _value.notFound) {
          children[childrenIndex].innerHTML +=
            _string.space +
            fromDate.getDate() +
            "/" +
            (fromDate.getMonth() + 1);
  ...
  function showFullWeekView(weekDate, fromOpen) {
    ...
    for (
      ...
      var orderedEvent = orderedEvents[orderedEventIndex],
        totalDays =
          getTotalDaysBetweenDates(orderedEvent.from, orderedEvent.to) + 1,
        nextDate = new Date(orderedEvent.from),
        addedNow = false;
      for (var dayIndex = 0; dayIndex < totalDays; dayIndex++) {
        if (nextDate >= weekStartDate && nextDate <= weekEndDate) {
          var column = getElementByID(
              _element_ID_WeekDayElement + getWeekdayNumber(nextDate)
            ),
            columnAllDay = getElementByID(
              _element_ID_WeekAllDayElement + getWeekdayNumber(nextDate)
            );
          if (column !== null) {
            var added = buildFullWeekViewEvent(
              column,
              columnAllDay,
              orderedEvent,
              nextDate
      ...
      if (repeatEvery > _enum_RepeatType.never) {
        if (repeatEvery === _enum_RepeatType.everyDay) {
          repeatAdded = buildFullWeekViewRepeatedDayEvents(
            orderedEvent,
            weekStartDate,
            weekEndDate,
            moveDateForwardDay,
          ...
          var repeatEveryCustomType = getNumber(
              orderedEvent.repeatEveryCustomType
            ),
            repeatEveryCustomValue = getNumber(
              orderedEvent.repeatEveryCustomValue
            );
          if (repeatEveryCustomValue > 0) {
            if (repeatEveryCustomType === _enum_RepeatCustomType.daily) {
              repeatAdded = buildFullWeekViewRepeatedDayEvents(
    ...
    for (
      ...
      headerNamesIndex < headerNamesLength;
      headerNamesIndex++
    ) {
      var columnForOverlapChecks = getElementByID(
        _element_ID_WeekDayElement + headerNamesIndex
      );
    ...
    if (fromOpen) {
      if (_element_View_FullWeek_TimeArrow_Position !== null) {
        allDayEventsHeight =
          allDayEventsHeight <= 0 ? _options.spacing * 4 : allDayEventsHeight;
        _element_View_FullWeek_Contents.scrollTop =
          _element_View_FullWeek_TimeArrow_Position - allDayEventsHeight;
      } else {
        _element_View_FullWeek_Contents.scrollTop =
          _element_View_FullWeek_Contents_SmallestEventTop;
      }
    ...
    updateToolbarButtonVisibleState(
      _element_View_FullWeek_SearchButton,
      _element_View_FullWeek_EventsShown.length > 0
    );
  }
  ...
  function updateUpdateFullViewAllDayEventsHeight() {
    ...
    if (_element_View_FullWeek_AllDayEventsAdded) {
      ...
      var columnsLength =
        _element_View_FullWeek_Contents_Days_AllDay.children.length;
      for (var columnIndex = 0; columnIndex < columnsLength; columnIndex++) {
        var events =
            _element_View_FullWeek_Contents_Days_AllDay.children[columnIndex]
              .children,
      ...
      _element_View_FullWeek_Contents_AllDayEvents.style.height =
        height + _options.spacing + "px";
    }
    return height;
  }
  ...
  function buildFullMonthViewTitleBar() {
    ...
    if (_options.fullScreenModeEnabled) {
      _element_View_FullMonth_TitleBar.ondblclick =
        fullScreenModeHeaderDoubleClick;
    }
    if (_element_Mode_DatePicker_Enabled) {
    ...
    if (!_element_Mode_DatePicker_Enabled && isSideMenuAvailable()) {
      buildToolbarButton(
        _element_View_FullMonth_TitleBar,
        "ib-hamburger",
        _options.showMenuTooltipText,
        showSideMenu
      );
      _element_View_FullMonth_TitleBar.appendChild(
        createElement("div", "left-divider-line")
      );
    }
    buildToolbarButton(
      _element_View_FullMonth_TitleBar,
      "ib-arrow-left-full",
      _options.previousMonthTooltipText,
      onPreviousMonth
    );
    if (_options.views.fullMonth.addYearButtons) {
      buildToolbarButton(
        _element_View_FullMonth_TitleBar,
        "ib-rewind",
        _options.previousYearTooltipText,
        moveBackYear
      );
    }
    if (
      _element_Mode_DatePicker_Enabled ||
      _options.views.fullMonth.showExtraTitleBarButtons
    ) {
      buildToolbarButton(
        _element_View_FullMonth_TitleBar,
        "ib-pin",
        _options.currentMonthTooltipText,
        onCurrentMonth
      );
    }
    if (_options.views.fullMonth.showExtraTitleBarButtons) {
      ...
      _element_View_FullMonth_TitleBar.appendChild(
        createElement("div", "left-divider-line-events")
      );
      buildToolbarButton(
        _element_View_FullMonth_TitleBar,
        "ib-refresh",
      ...
      if (_options_Search.enabled) {
        _element_View_FullMonth_TitleBar_SearchButton = buildToolbarButton(
          _element_View_FullMonth_TitleBar,
          "ib-search",
          _options.searchTooltipText,
          showSearchDialog
    ...
    if (
      ...
      _element_View_FullMonth_TitleBar_FullScreenButton = buildToolbarButton(
        _element_View_FullMonth_TitleBar,
        "ib-arrow-expand-left-right",
        _options.enableFullScreenTooltipText,
        fullScreenModeHeaderDoubleClick
    ...
    if (!_element_Mode_DatePicker_Enabled) {
      if (
        ...
        _element_View_FullMonth_TitleBar.appendChild(
          createElement("div", "right-divider-line-views")
        );
      }
      if (_options.views.timeline.enabled) {
        buildToolbarButton(
      ...
      if (_options.views.allEvents.enabled) {
        buildToolbarButton(
          _element_View_FullMonth_TitleBar,
          "ib-eye",
          _options.viewAllEventsTooltipText,
          function () {
      ...
      if (_options.views.fullYear.enabled) {
        buildToolbarButton(
          _element_View_FullMonth_TitleBar,
          "ib-expand",
          _options.viewFullYearTooltipText,
          function () {
      ...
      if (_options.views.fullWeek.enabled) {
        buildToolbarButton(
          _element_View_FullMonth_TitleBar,
          "ib-hamburger-side",
          _options.viewFullWeekTooltipText,
          function () {
      ...
      if (_options.views.fullDay.enabled) {
        buildToolbarButton(
          _element_View_FullMonth_TitleBar,
          "ib-hourglass",
          _options.viewFullDayTooltipText,
          function () {
  ...
  function buildFullMonthViewPinUpImage() {
    if (
      ...
      if (_options.views.fullMonth.pinUpViewImageUrls.length > 0) {
        _element_View_FullMonth_PinUp.style.backgroundImage =
          "url('" +
          _options.views.fullMonth.pinUpViewImageUrls[
        ...
        if (
          _element_View_FullMonth_PinUp_ImageIndex ===
          _options.views.fullMonth.pinUpViewImageUrls.length
        ) {
          _element_View_FullMonth_PinUp_ImageIndex = 0;
        }
      } else if (_options.views.fullMonth.pinUpViewImageCssClasses.length > 0) {
        _element_View_FullMonth_PinUp.className =
          "pin-up" +
          _string.space +
          _options.views.fullMonth.pinUpViewImageCssClasses[
            _element_View_FullMonth_PinUp_ImageIndex
          ];
        _element_View_FullMonth_PinUp_ImageIndex++;
        if (
          _element_View_FullMonth_PinUp_ImageIndex ===
          _options.views.fullMonth.pinUpViewImageCssClasses.length
        ) {
          _element_View_FullMonth_PinUp_ImageIndex = 0;
        }
      }
  ...
  function buildFullMonthViewPinUpImageText() {
    if (
      ...
      _element_View_FullMonth_PinUp !== null &&
      !fireCustomTrigger(
        _options.events.onFullMonthPinUpRender,
        _element_View_FullMonth_PinUp,
        _calendar_CurrentDate
      )
    ) {
      var currentDate = new Date();
      if (
        _element_View_FullMonth_PinUp_CurrentDate === null ||
        !doDatesMatch(_element_View_FullMonth_PinUp_CurrentDate, currentDate)
      ) {
        ...
        setNodeText(
          pinUpTextMonth,
          _options.monthNames[currentDate.getMonth()]
        );
  ...
  function buildFullMonthViewDayNamesHeader() {
    ...
    if (_options.views.fullMonth.showDayNamesHeaders) {
      ...
      if (!wasAddedAlready) {
        _element_View_FullMonth_DayNamesHeader = createElement(
          "div",
          "row-cells header-days"
        );
        _element_View_FullMonth.appendChild(
          _element_View_FullMonth_DayNamesHeader
        );
      }
      if (_element_Mode_DatePicker_Enabled) {
      ...
      if (wasAddedAlready) {
        _element_View_FullMonth.removeChild(
          _element_View_FullMonth_DayNamesHeader
        );
        _element_View_FullMonth_DayNamesHeader = null;
      }
      ...
      for (var columnDataIndex = 0; columnDataIndex < 7; columnDataIndex++) {
        ...
        if (_options.visibleDays.indexOf(dayNumber) > _value.notFound) {
          var columnDataNumber = rowIndex * 7 + (columnDataIndex + 1),
            columnData = createElement(
              "div",
              getCellName(_options.views.fullMonth.allowEventScrolling)
            );
          ...
          if (_options.views.fullMonth.minimumDayHeight > 0) {
            columnData.style.height =
              _options.views.fullMonth.minimumDayHeight + "px";
          }
        }
      }
      if (_options.reverseOrderDaysOfWeek) {
  ...
  function buildFullMonthViewYearDropDownButton(titleContainer) {
    _element_View_FullMonth_TitleBar_YearSelector_DropDown = createElement(
      "span",
      "year-dropdown-button"
    );
    _element_View_FullMonth_TitleBar_YearSelector_DropDown.ondblclick =
      cancelBubble;
    _element_View_FullMonth_TitleBar_YearSelector_DropDown.onclick =
      showFullMonthViewYearDropDown;
    ...
    _element_View_FullMonth_TitleBar_YearSelector_DropDown_Arrow =
      createElement("span", "ib-arrow-down-full-medium");
    _element_View_FullMonth_TitleBar_YearSelector_DropDown.appendChild(
      _element_View_FullMonth_TitleBar_YearSelector_DropDown_Arrow
    );
  }
  function buildFullMonthViewYearDropDown(container) {
    var yearDate = new Date(_options.minimumYear, 1, 1),
      monthContainer = null;
    _element_View_FullMonth_TitleBar_YearSelector = createElement(
      "div",
      _options.views.fullMonth.showMonthButtonsInYearDropDownMenu
        ? "years-drop-down"
        : "years-drop-down-no-months"
    );
    ...
    _element_View_FullMonth_TitleBar_YearSelector_Contents = createElement(
      "div",
      "contents custom-scroll-bars"
    );
    _element_View_FullMonth_TitleBar_YearSelector.appendChild(
      _element_View_FullMonth_TitleBar_YearSelector_Contents
    );
    while (true) {
  ...
  function buildFullMonthViewYearDropDownMonthNameButton(
    container,
    monthNumber
  ) {
  ...
  function setFullMonthViewYearDropDownButtonText() {
    _element_View_FullMonth_TitleBar_YearSelector_DropDown_Text.innerText =
      getCustomFormattedDateText(
        _options.views.fullMonth.titleBarDateFormat,
        _calendar_CurrentDate
      );
  }
  function updateFullMonthViewYearMonthSelected() {
    ...
    if (_calendar_CurrentDate.getFullYear() === today.getFullYear()) {
      ...
      if (
        ...
        _element_View_FullMonth_TitleBar_YearSelector_Contents_Months[
          currentMonthNumber
        ].className = "month-name-current-month";
      }
    }
    if (
      _element_View_FullMonth_TitleBar_YearSelector_Contents_Months.hasOwnProperty(
    ...
    if (!_element_Mode_DatePicker_Enabled) {
      ...
      getAllEventsFunc(function (eventDetails) {
        ...
        if (yearsHandledForEvents.indexOf(fromYear) === _value.notFound) {
          ...
          if (
            yearEvents !== null &&
            yearEvents.className.indexOf(" year-selected") === _value.notFound
          ) {
  ...
  function showFullMonthViewYearDropDown(e) {
    ...
    if (
      ...
      hideAllDropDowns();
      _element_View_FullMonth_TitleBar_YearSelector.style.display = "block";
      _element_View_FullMonth_TitleBar_YearSelector_DropDown_Arrow.className =
        "ib-arrow-up-full-medium";
      updateFullMonthViewYearMonthSelected();
      var year = updateFullMonthViewYearDropDownColors();
      if (year !== null) {
        _element_View_FullMonth_TitleBar_YearSelector_Contents.scrollTop =
          year.offsetTop -
          _element_View_FullMonth_TitleBar_YearSelector_Contents.offsetTop -
          _options.spacing;
      } else {
  ...
  function buildFullMonthViewPreviousMonthDays(startDay) {
    if (startDay > 1) {
      ...
      var totalDaysInMonth = getTotalDaysInMonth(
  ...
  function buildFullMonthViewNextMonthDays(lastDayFilled) {
    if (lastDayFilled < 42) {
      ...
      var nextDay = getTotalDaysInMonth(
        nextMonth.getFullYear(),
        nextMonth.getMonth()
      );
      nextDay = _parameter_Math.round(nextDay / 2);
      _element_View_FullMonth_LargestDateAvailable = new Date(
        nextMonth.getFullYear(),
        nextMonth.getMonth(),
        nextDay
      );
  ...
  function buildDay(
    ...
    if (dayElement !== null) {
      ...
      dayElement.className = dayElement.className
        .replace(" cell-today", _string.empty)
        .replace(" cell-selected", _string.empty)
        .replace(" cell-no-click", _string.empty);
      ...
      if (
        ...
        !dayIsToday &&
        _calendar_CurrentDate_ForDatePicker !== null &&
        doDatesMatch(dayDate, _calendar_CurrentDate_ForDatePicker)
      ) {
      ...
      dayText.className +=
        dayIsToday && !_element_Mode_DatePicker_Enabled
          ? " today"
          : _string.empty;
      ...
      if (
        ...
        dayElement.className.indexOf("weekend-day") === _value.notFound
      ) {
        dayElement.className += " weekend-day";
      }
      if (
        isWorkingDay(dayDate) &&
        dayElement.className.indexOf("working-day") === _value.notFound
      ) {
      ...
      if (_options.views.fullDay.enabled) {
        var expandDayButton = createElement(
          "div",
      ...
      if (
        includeMonthName &&
        _options.views.fullMonth.showPreviousNextMonthNames
      ) {
        createSpanElement(
          dayElement,
          _options.monthNames[month],
          "month-name" + dayMutedClass,
          function () {
            if (actualDay === 1) {
              onNextMonth();
            } else {
  ...
  function buildFullMonthViewDayEvents() {
    ...
    for (
      ...
      if (repeatEvery > _enum_RepeatType.never) {
        if (repeatEvery === _enum_RepeatType.everyDay) {
          buildFullMonthViewRepeatedDayEvents(
            orderedEvent,
            moveDateForwardDay,
            1
          );
    ...
    updateFullMonthViewTitleBarButtonsVisibleStates(
      _element_View_FullMonth_EventsShown.length
    );
    startAutoRefreshTimer();
  }
  function buildFullMonthViewRepeatedDayEvents(
    orderedEvent,
    dateFunc,
    dateFuncForwardValue
  ) {
    ...
    while (newFromDate < largestDate) {
      ...
      if (
        ...
        updateDateTotalEventsTracked(
          toStorageFormattedDate(newFromDate),
          orderedEvent.id
        );
  ...
  function buildFullMonthViewDayEventAcrossDays(orderedEvent) {
    ...
    if (
      orderedEvent.from.getDate() !== orderedEvent.to.getDate() ||
      orderedEvent.from.getMonth() !== orderedEvent.to.getMonth() ||
      orderedEvent.from.getFullYear() !== orderedEvent.to.getFullYear()
    ) {
      var totalDays = getTotalDaysBetweenDates(
        orderedEvent.from,
        orderedEvent.to
      );
      if (totalDays > 0) {
        ...
        for (var dayIndex = 0; dayIndex < totalDays; dayIndex++) {
          ...
          updateDateTotalEventsTracked(
            toStorageFormattedDate(nextDayDate),
            orderedEvent.id
          );
  ...
  function buildFullMonthViewDayEvent(dayDate, eventDetails) {
    ...
    if (
      isEventVisible(eventDetails) &&
      seriesIgnoreDates.indexOf(formattedDayDate) === _value.notFound
    ) {
      ...
      if (elementDay !== null) {
        ...
        if (!_element_Mode_DatePicker_Enabled) {
          ...
          if (
            events.length <
              _options.views.fullMonth.maximumEventsPerDayDisplay ||
            _options.views.fullMonth.maximumEventsPerDayDisplay <= 0 ||
            _options.views.fullMonth.useOnlyDotEvents
          ) {
            var event = createElement("div", "event"),
              eventRendered = false;
            event.setAttribute("event-type", getNumber(eventDetails.type));
            ...
            if (!_options.views.fullMonth.useOnlyDotEvents) {
              if (
                !fireCustomTrigger(
                  _options.events.onFullMonthEventRender,
                  event,
                  eventDetails
                )
              ) {
                var eventTitle = eventDetails.title,
                  repeatEvery = getNumber(eventDetails.repeatEvery);
                if (
                  _options.views.fullMonth.showTimesInEvents &&
                  !eventDetails.isAllDay &&
                  eventDetails.from.getDate() === eventDetails.to.getDate()
                ) {
                  eventTitle =
                    getTimeToTimeDisplay(eventDetails.from, eventDetails.to) +
                    ": " +
                ...
                if (repeatEvery > _enum_RepeatType.never) {
                  var icon = createElement(
                    "div",
                    "ib-refresh-small ib-no-hover ib-no-active"
                  );
                  icon.style.borderColor = event.style.color;
            ...
            setEventClassesAndColors(
              event,
              eventDetails,
              getToTimeWithPassedDate(eventDetails, dayDate),
              _options.views.fullMonth.applyCssToEventsNotInCurrentMonth
            );
            setEventCustomTriggers(event, eventDetails, editEventDate);
            ...
            event.onmousemove = function (e) {
              if (
                _element_Tooltip_EventDetails !== null &&
                _element_Tooltip_EventDetails.id === eventDetails.id
              ) {
                cancelBubble(e);
              } else {
  ...
  function buildFullMonthViewDayEventPlusText(elementDay, dayDate) {
    ...
    if (plusXEventsText === null) {
      ...
      if (
        (_options.views.fullMonth.applyCssToEventsNotInCurrentMonth &&
          dayDate.getMonth() !== _calendar_CurrentDate.getMonth()) ||
        dayDate.getFullYear() !== _calendar_CurrentDate.getFullYear()
      ) {
      ...
      if (_options.views.fullDay.enabled) {
        ...
        plusXEventsText.className += _string.space + "prevent-pointer-events";
      }
    } else {
      ...
      setNodeText(
        plusXEventsText,
        "+" + numberOfEvents + _string.space + _options.moreText
      );
    }
  }
  function updateFullMonthViewTitleBarButtonsVisibleStates(
    orderedEventsLength
  ) {
    if (_element_View_FullMonth_TitleBar_SearchButton !== null) {
      updateToolbarButtonVisibleState(
        _element_View_FullMonth_TitleBar_SearchButton,
        orderedEventsLength > 0
      );
    }
  }
  function getFullMonthViewDayElement(date) {
    ...
    if (
      ...
      elementDayNumber = getStartOfWeekDayNumber(
        firstDayNumber -
          getTotalDaysBetweenDates(date, _calendar_CurrentDate) +
          1
      );
    } else if (
      date.getMonth() === _calendar_CurrentDate.getMonth() &&
      date.getFullYear() === _calendar_CurrentDate.getFullYear()
    ) {
  ...
  function clearFullMonthViewEventsFromDays() {
    for (var rowIndex = 0; rowIndex < 6; rowIndex++) {
      for (var columnDataIndex = 0; columnDataIndex < 7; columnDataIndex++) {
        var columnDataNumber = rowIndex * 7 + (columnDataIndex + 1),
          columnDataElement = getElementByID(
            _element_ID_DayElement + columnDataNumber
          );
  ...
  function updateDateTotalEventsTracked(formattedNewFromDate, eventId) {
    ...
    if (
      _events_DatesAvailable[formattedNewFromDate].indexOf(eventId) ===
      _value.notFound
    ) {
  ...
  function buildFullYearView() {
    if (!_element_Mode_DatePicker_Enabled && _options.views.fullYear.enabled) {
      ...
      if (_options.views.fullYear.showExtraTitleBarButtons) {
        buildToolbarButton(
          titleBar,
          "ib-pin",
          _options.currentYearTooltipText,
          onCurrentFullYear
        );
        if (_options.jumpToDateEnabled) {
          buildToolbarButton(
      ...
      titleBar.appendChild(createElement("div", "right-divider-line"));
      if (
        _options.views.fullYear.showExtraTitleBarButtons &&
        _options.fullScreenModeEnabled
      ) {
        _element_View_FullYear_FullScreenButton = buildToolbarButton(
          titleBar,
          "ib-arrow-expand-left-right",
          _options.enableFullScreenTooltipText,
          fullScreenModeHeaderDoubleClick
      ...
      if (
        ...
        buildToolbarButton(
          ...
          function () {
            showEventEditingDialog(
              null,
              new Date(_element_View_FullYear_CurrentYear, 0, 1)
            );
          }
        );
      }
      _element_View_FullYear_Contents = createElement(
        "div",
        "contents custom-scroll-bars"
      );
  ...
  function buildFullYearMonths(fromOpen) {
    var previousScrollTop = !fromOpen
      ? _element_View_FullYear_Contents.scrollTop
      : 0;
    ...
    if (fromOpen) {
      if (_element_View_FullYear_Contents_CurrentMonthTop > 0) {
        _element_View_FullYear_Contents.scrollTop =
          _element_View_FullYear_Contents_CurrentMonthTop -
          _options.spacing * 4;
      } else {
  ...
  function buildFullYearViewMonth(monthIndex) {
    var expandMonthDate = new Date(
        _element_View_FullYear_CurrentYear,
        monthIndex,
        1
      ),
    ...
    var currentMonthFillDetails = buildFullYearViewMonthDays(
        startDay,
        monthDayId,
        monthIndex
      ),
  ...
  function buildFullYearViewMonthPreviousMonthDays(
    startDay,
    monthDayId,
    monthIndex
  ) {
    ...
    previousMonth.setMonth(previousMonth.getMonth() - 1);
    var totalDaysInMonthInPreviousMonth = getTotalDaysInMonth(
        previousMonth.getFullYear(),
        previousMonth.getMonth()
      ),
    ...
    while (previousDayIndex > 0) {
      buildFullYearViewMonthPreviousMonthDay(
        monthDayId,
        previousDayIndex,
        previousMonth,
        previousDay
  ...
  function buildFullYearViewMonthPreviousMonthDay(
    monthDayId,
    previousDayIndex,
    previousMonth,
    previousDay
  ) {
    var previousMonthDayFullDayElement = getElementByID(
      monthDayId + previousDayIndex
    );
    if (previousMonthDayFullDayElement !== null) {
      ...
      buildFullYearViewMonthDayClasses(
        previousMonthDayFullDayElement,
        previousMonthDate
      );
  ...
  function buildFullYearViewMonthDays(startDay, monthDayId, monthIndex) {
    ...
    for (var day = 0; day < totalDaysInMonth; day++) {
      elementDayNumber = startDay + day;
      var isDayInCurrentMonth = buildFullYearViewMonthDay(
        monthDayId,
        monthIndex,
        day,
        elementDayNumber
  ...
  function buildFullYearViewMonthDay(
    monthDayId,
    monthIndex,
    day,
    elementDayNumber
  ) {
    var currentMonthDayFullDayElement = getElementByID(
        monthDayId + elementDayNumber
      ),
      isCurrentMonth = false;
    if (currentMonthDayFullDayElement !== null) {
      var currentMonthDayDate = new Date(
        _element_View_FullYear_CurrentYear,
        monthIndex,
        day + 1
      );
      ...
      isCurrentMonth = buildFullYearViewMonthDayClasses(
        currentMonthDayFullDayElement,
        currentMonthDayDate,
        true
      );
  ...
  function buildFullYearViewMonthNextMonthDays(
    lastDayFilled,
    monthDayId,
    monthIndex
  ) {
    if (lastDayFilled < 42) {
      ...
      for (var day = lastDayFilled + 1; day < 43; day++) {
        buildFullYearViewMonthNextMonthDay(
          monthDayId,
          nextMonth,
          day,
          actualDay
  ...
  function buildFullYearViewMonthNextMonthDay(
    monthDayId,
    nextMonth,
    day,
    actualDay
  ...
  function showFullYearView(year, fromOpen) {
    fromOpen = isDefined(fromOpen) ? fromOpen : false;
    _element_View_FullYear_CurrentYear = isDefined(year)
      ? year
      : _calendar_CurrentDate.getFullYear();
    _element_View_FullYear_Contents_CurrentMonthTop = 0;
    _element_View_FullYear_TitleBar.innerText =
      _element_View_FullYear_CurrentYear;
  ...
  function buildAllEventsView() {
    if (!_element_Mode_DatePicker_Enabled && _options.views.allEvents.enabled) {
      ...
      if (_options.views.allEvents.showExtraTitleBarButtons) {
        titleBar.appendChild(createElement("div", "right-divider-line"));
        if (_options.fullScreenModeEnabled) {
          _element_View_AllEvents_FullScreenButton = buildToolbarButton(
            titleBar,
            "ib-arrow-expand-left-right",
            _options.enableFullScreenTooltipText,
            fullScreenModeHeaderDoubleClick
      ...
      if (_options.views.allEvents.showExtraTitleBarButtons) {
        ...
        if (_options_Search.enabled) {
          _element_View_AllEvents_SearchButton = buildToolbarButton(
            titleBar,
            "ib-search",
            _options.searchTooltipText,
            showSearchDialog
      ...
      _element_View_AllEvents_Contents = createElement(
        "div",
        "contents custom-scroll-bars"
      );
  ...
  function buildAllEventsViewEvent(eventDetails) {
    if (isEventVisible(eventDetails)) {
      ...
      if (
        !fireCustomTrigger(
          _options.events.onAllEventsEventRender,
          event,
          eventDetails
        )
      ) {
  ...
  function buildAllEventsViewMonth(date) {
    ...
    if (monthContents === null) {
      ...
      setNodeText(
        header,
        _options.monthNames[date.getMonth()] +
          _string.space +
          date.getFullYear()
      );
      ...
      var minimizeRestoreFunction = function () {
        minimizeRestoreAllEventsViewMonth(
          minimizeButton,
          monthContents,
          monthContentsID
        );
      };
      var minimizeButton = buildToolbarButton(
        header,
        "ib-minus",
        _options.minimizedTooltipText,
        minimizeRestoreFunction
      );
      _element_View_AllEvents_MinimizeRestoreFunctions.push(
        minimizeRestoreFunction
      );
      ...
      if (
        _options_Configuration.visibleAllEventsMonths.hasOwnProperty(
          monthContentsID
        ) &&
        !_options_Configuration.visibleAllEventsMonths[monthContentsID]
      ) {
  ...
  function buildAllEventsViewMonthHeader(date) {
    var yearHeader = null;
    if (
      _element_View_AllEvents_YearHeadersAdded.indexOf(date.getFullYear()) ===
      _value.notFound
    ) {
      ...
      if (_options.views.fullYear.enabled) {
        ...
        text.className += _string.space + "prevent-pointer-events";
      }
  ...
  function showAllEventsView(fromOpen) {
    ...
    updateToolbarButtonVisibleState(
      _element_View_AllEvents_SearchButton,
      _element_View_AllEvents_EventsShown.length > 0
    );
  ...
  function minimizeRestoreAllEventsViewMonth(
    minimizeButton,
    monthContents,
    monthContentsID
  ) {
  ...
  function callMinimizeRestoreFunctionsForAllEventView() {
    if (isViewVisible(_element_View_AllEvents)) {
      var functionsLength =
        _element_View_AllEvents_MinimizeRestoreFunctions.length;
      for (
        var functionIndex = 0;
        functionIndex < functionsLength;
        functionIndex++
      ) {
  ...
  function buildTimelineView() {
    if (!_element_Mode_DatePicker_Enabled && _options.views.timeline.enabled) {
      ...
      if (_element_View_Timeline_Selected_Axis === null) {
        _element_View_Timeline_Selected_Axis =
          _options.views.timeline.defaultAxis;
      }
      if (!wasAddedAlready) {
      ...
      if (_options.views.timeline.showExtraTitleBarButtons) {
        if (_options.fullScreenModeEnabled) {
          _element_View_Timeline_FullScreenButton = buildToolbarButton(
            titleBar,
            "ib-arrow-expand-left-right",
            _options.enableFullScreenTooltipText,
            fullScreenModeHeaderDoubleClick
      ...
      if (
        _options.views.timeline.showExtraTitleBarButtons &&
        _options.manualEditingEnabled
      ) {
        buildToolbarButton(
          titleBar,
          ...
          function () {
            showEventEditingDialog(
              null,
              new Date(_element_View_Timeline_DateSelected)
            );
          }
        );
      }
      ...
      if (_options.views.timeline.showExtraTitleBarButtons) {
        _element_View_Timeline_TodayButton = buildToolbarButton(
          titleBar,
          "ib-pin",
          _options.currentYearTooltipText,
          onCurrentTimelineDay
        ...
        if (_options_Search.enabled) {
          _element_View_Timeline_SearchButton = buildToolbarButton(
            titleBar,
            "ib-search",
            _options.searchTooltipText,
            showSearchDialog
      ...
      _element_View_Timeline_Contents = createElement(
        "div",
        "contents custom-scroll-bars"
      );
  ...
  function buildTimelineViewContentLayout() {
    _element_View_Timeline_Contents_Header = createElement(
      "div",
      "timeline-header"
    );
    _element_View_Timeline_Contents.appendChild(
      _element_View_Timeline_Contents_Header
    );
    ...
    buildToolbarButton(
      groupHeader,
      "ib-arrow-left-full",
      _options.previousPropertyTooltipText,
      onPreviousAxisTimelineView
    );
    buildToolbarButton(
      groupHeader,
      "ib-arrow-right-full",
      _options.nextPropertyTooltipText,
      onNextAxisTimelineView
    );
    var loopDateMinutesIncrease =
        _options.views.timeline.minutesBetweenSections,
    ...
    if (orderedEventsLength > 0) {
      for (
        ...
        var eventDetails = orderedEvents[orderedEventIndex],
          axisName = getString(
            eventDetails[_element_View_Timeline_Selected_Axis],
            _options.noneText
          ),
        ...
        if (
          !_element_View_Timeline_Contents_Groups_Rows_Cache.hasOwnProperty(
            storageAxisName
          )
        ) {
          var timelineRow = createElement("div", "timeline-row");
          _element_View_Timeline_Contents.appendChild(timelineRow);
          var axisNameRowName = createElement(
            "div",
            "timeline-row-item" +
              (rowCount % 2 !== 0 ? " timeline-row-item-odd" : _string.empty)
          );
          ...
          _element_View_Timeline_Contents_Groups_Rows_Cache[storageAxisName] =
            timelineRowItems;
          rowCount++;
        } else {
          timelineRowItems =
  ...
  function buildTimelineViewColumnHeights() {
    ...
    for (var columnIndex = 0; columnIndex < columnsLength; columnIndex++) {
      ...
      if (columnHeight === 0) {
        columnHeight =
          _element_View_Timeline_Contents.scrollHeight - column.offsetTop;
      }
  ...
  function buildTimelineViewEvent(timelineRowItems, eventDetails) {
    var event = createElement("div", "event"),
      formattedDate = toStorageFormattedDate(
        _element_View_Timeline_DateSelected
      ),
    ...
    if (
      !fireCustomTrigger(
        _options.events.onTimelineEventRender,
        event,
        eventDetails
      )
    ) {
    ...
    setEventClassesAndColors(
      event,
      eventDetails,
      getToTimeWithPassedDate(eventDetails, _element_View_Timeline_DateSelected)
    );
    var scrollLeft = setEventPositionAndGetScrollLeft(
      timelineRowItems,
      _element_View_Timeline_DateSelected,
      event,
      eventDetails
    ...
    if (_element_View_Timeline_Contents_SmallestEventLeft === null) {
      ...
      _element_View_Timeline_Contents_SmallestEventLeft = _parameter_Math.min(
        _element_View_Timeline_Contents_SmallestEventLeft,
        scrollLeft
      );
    }
    if (_element_View_Timeline_Contents_SmallestEventTop === null) {
      _element_View_Timeline_Contents_SmallestEventTop =
        timelineRowItems.offsetTop;
    } else {
      _element_View_Timeline_Contents_SmallestEventTop = _parameter_Math.min(
        _element_View_Timeline_Contents_SmallestEventTop,
        timelineRowItems.offsetTop
      );
    }
  ...
  function buildTimelineViewTitle() {
    if (
      !fireCustomTrigger(
        _options.events.onTimelineTitleRender,
        _element_View_FullDay_DateSelected
      )
    ) {
      buildDateTimeDisplay(
        _element_View_Timeline_TitleBar,
        _element_View_Timeline_DateSelected,
        false,
        true,
        true
  ...
  function showTimelineView(date, fromOpen) {
    ...
    getFullDayViewOrderedEvents(
      _element_View_Timeline_DateSelected,
      orderedEvents
    );
    updateToolbarButtonVisibleState(
      _element_View_Timeline_TodayButton,
      isCurrentDateVisible
    );
    ...
    if (fromOpen) {
      _element_View_Timeline_Contents.scrollLeft =
        _element_View_Timeline_Contents_SmallestEventLeft;
      _element_View_Timeline_Contents.scrollTop =
        _element_View_Timeline_Contents_SmallestEventTop;
    } else {
    ...
    updateToolbarButtonVisibleState(
      _element_View_Timeline_SearchButton,
      _element_View_Timeline_EventsShown.length > 0
    );
  }
  ...
  function onPreviousAxisTimelineView() {
    var indexOf = _element_View_Timeline_Selected_Axis_Supported.indexOf(
      _element_View_Timeline_Selected_Axis
    );
    indexOf--;
    if (indexOf < 0) {
    ...
    _element_View_Timeline_Selected_Axis =
      _element_View_Timeline_Selected_Axis_Supported[indexOf];
    showTimelineView(_element_View_Timeline_DateSelected);
  }
  ...
  function getTimelineViewAxisSelectedText() {
    ...
    if (_element_View_Timeline_Selected_Axis === "group") {
      ...
      _element_View_Timeline_Selected_Axis === "organizerEmailAddress"
    ) {
  ...
  function buildContextMenuItemWithIcon(
    container,
    iconCSS,
    text,
    onClickEvent,
  ...
  function areContextMenusVisible() {
    return (
      isContextMenuVisible(_element_ContextMenu_Day) ||
      isContextMenuVisible(_element_ContextMenu_Event) ||
      isContextMenuVisible(_element_ContextMenu_FullDay) ||
      isContextMenuVisible(_element_ContextMenu_HeaderDay) ||
      isContextMenuVisible(_element_Dialog_Search_History_DropDown)
    );
  }
  function buildDayContextMenu() {
    ...
    if (_options.manualEditingEnabled) {
      buildContextMenuItemWithIcon(
        _element_ContextMenu_Day,
        "ib-plus-icon",
    ...
    if (_options.manualEditingEnabled) {
      _element_ContextMenu_Day_Paste_Separator = buildContextMenuSeparator(
        _element_ContextMenu_Day
      );
      _element_ContextMenu_Day_Paste = buildContextMenuItemWithIcon(
        _element_ContextMenu_Day,
        "ib-circle-icon",
        _options.pasteText,
        function () {
  ...
  function buildEventContextMenu() {
    ...
    if (_options.manualEditingEnabled) {
      _element_ContextMenu_Event_EditEvent = buildContextMenuItemWithIcon(
        _element_ContextMenu_Event,
        "ib-edit-icon",
        _options.editEventTitle + "...",
        function () {
          showEventEditingDialog(
            _element_ContextMenu_Event_EventDetails,
            null,
            null,
            _element_ContextMenu_Event_DateSelected
          );
        },
        true
      );
      _element_ContextMenu_Event_CutSeparator = buildContextMenuSeparator(
        _element_ContextMenu_Event
      );
      _element_ContextMenu_Event_Cut = buildContextMenuItemWithIcon(
        _element_ContextMenu_Event,
        "ib-pipe-icon",
        _options.cutText,
        function () {
      ...
      _element_ContextMenu_Event_CopySeparator = buildContextMenuSeparator(
        _element_ContextMenu_Event
      );
      _element_ContextMenu_Event_Copy = buildContextMenuItemWithIcon(
        _element_ContextMenu_Event,
      ...
      _element_ContextMenu_Event_DuplicateSeparator = buildContextMenuSeparator(
        _element_ContextMenu_Event
      );
      _element_ContextMenu_Event_Duplicate = buildContextMenuItemWithIcon(
        _element_ContextMenu_Event,
        "ib-equals-icon",
        _options.duplicateText + "...",
        function () {
          showEventEditingDialog(_element_ContextMenu_Event_EventDetails);
          setEventEditingDialogInDuplicateMode();
        }
      );
      _element_ContextMenu_Event_RemoveSeparator = buildContextMenuSeparator(
        _element_ContextMenu_Event
      );
      _element_ContextMenu_Event_Remove = buildContextMenuItemWithIcon(
        _element_ContextMenu_Event,
        "ib-close-icon",
        _options.removeEventText,
        function () {
          addNode(
            _parameter_Document.body,
            _element_Calendar_DisabledBackground
          );
          ...
          var onYesEvent = function () {
            ...
            if (isDefined(_element_ContextMenu_Event_EventDetails.id)) {
              if (
                !_element_Dialog_Message_RemoveAllEvents.checked &&
                _element_ContextMenu_Event_FormattedDateSelected !== null
              ) {
                if (
                  isDefinedArray(
                    _element_ContextMenu_Event_EventDetails.seriesIgnoreDates
                  )
                ) {
                  _element_ContextMenu_Event_EventDetails.seriesIgnoreDates.push(
                    _element_ContextMenu_Event_FormattedDateSelected
                  );
                } else {
                  _element_ContextMenu_Event_EventDetails.seriesIgnoreDates = [
                    _element_ContextMenu_Event_FormattedDateSelected,
                ...
                _that.removeEvent(
                  _element_ContextMenu_Event_EventDetails.id,
                  true
                );
                showNotificationPopUp(
                  _options.eventRemovedText.replace(
                    "{0}",
                    _element_ContextMenu_Event_EventDetails.title
                  )
                );
              }
              refreshOpenedViews();
          ...
          var repeatEvery = getNumber(
              _element_ContextMenu_Event_EventDetails.repeatEvery
            ),
            showCheckBox =
              repeatEvery > _enum_RepeatType.never &&
              _element_ContextMenu_Event_FormattedDateSelected !== null;
          showMessageDialog(
            _options.confirmEventRemoveTitle,
            _options.confirmEventRemoveMessage,
            onYesEvent,
            onNoEvent,
            showCheckBox
          );
      ...
      _element_ContextMenu_Event_OpenUrlSeparator = buildContextMenuSeparator(
        _element_ContextMenu_Event
      );
    }
    _element_ContextMenu_Event_OpenUrl = buildContextMenuItemWithIcon(
      _element_ContextMenu_Event,
      "ib-arrow-top-right-icon",
      _options.openUrlText,
      function () {
        openEventUrl(_element_ContextMenu_Event_EventDetails.url);
      }
    );
    if (_options.exportEventsEnabled) {
      _element_ContextMenu_Event_ExportEventsSeparator =
        buildContextMenuSeparator(_element_ContextMenu_Event);
      _element_ContextMenu_Event_ExportEvents = buildContextMenuItemWithIcon(
        _element_ContextMenu_Event,
    ...
    if (_options.exportEventsEnabled && _parameter_Navigator.share) {
      ...
      buildContextMenuItemWithIcon(
        ...
        _options.shareText + "...",
        function () {
          exportEventsForSharing(
            _events_Selected.length > 0
              ? _events_Selected
              : [_element_ContextMenu_Event_EventDetails]
          );
        }
  ...
  function showEventContextMenu(e, eventDetails, selectedDate) {
    if (_element_ContextMenu_Event !== null) {
      ...
      _element_ContextMenu_Event_FormattedDateSelected = isDefined(selectedDate)
        ? selectedDate
        : null;
      if (_events_Selected.length > 1) {
        ...
        if (_options.exportEventsEnabled) {
          _element_ContextMenu_Event_ExportEventsSeparator.style.display =
            "block";
      ...
      if (
        url !== _string.empty ||
        _element_ContextMenu_Event.childElementCount > 1
      ) {
        hideAllDropDowns();
        cancelBubble(e);
  ...
  function buildFullDayViewContextMenu() {
    ...
    if (_options.manualEditingEnabled) {
      _element_ContextMenu_FullDay = createElement(
        "div",
      ...
      buildContextMenuItemWithIcon(
        _element_ContextMenu_FullDay,
        "ib-plus-icon",
        _options.addEventTitle + "...",
        function () {
          if (_options.useTemplateWhenAddingNewEvent) {
            var newBlankTemplateEvent = buildBlankTemplateEvent(
              _element_ContextMenu_FullDay_DateSelected,
              _element_ContextMenu_FullDay_DateSelected,
              _element_ContextMenu_FullDay_ClickPositionHourMinutes,
              _element_ContextMenu_FullDay_ClickPositionHourMinutes
            );
      ...
      _element_ContextMenu_FullDay_RemoveEvents_Separator =
        buildContextMenuSeparator(_element_ContextMenu_FullDay);
      _element_ContextMenu_FullDay_RemoveEvents = buildContextMenuItemWithIcon(
        _element_ContextMenu_FullDay,
        "ib-close-icon",
        _options.removeEventsTooltipText,
        function () {
      ...
      _element_ContextMenu_FullDay_Paste_Separator = buildContextMenuSeparator(
        _element_ContextMenu_FullDay
      );
      _element_ContextMenu_FullDay_Paste = buildContextMenuItemWithIcon(
        _element_ContextMenu_FullDay,
        "ib-circle-icon",
        _options.pasteText,
        function () {
  ...
  function showFullDayContextMenu(e, dateSelected, eventsShown) {
    if (_element_ContextMenu_FullDay !== null) {
      ...
      if (_element_ContextMenu_FullDay_Paste !== null) {
        ...
        _element_ContextMenu_FullDay_Paste_Separator.style.display =
          pasteDisplay;
        _element_ContextMenu_FullDay_Paste.style.display = pasteDisplay;
      }
      var removeEventsDisplay =
        isDefinedArray(eventsShown) && eventsShown.length > 0
          ? "block"
          : "none";
      _element_ContextMenu_FullDay_RemoveEvents_Separator.style.display =
        removeEventsDisplay;
      _element_ContextMenu_FullDay_RemoveEvents.style.display =
        removeEventsDisplay;
      hideAllDropDowns();
      cancelBubble(e);
  ...
  function buildDayHeaderContextMenu() {
    if (_element_ContextMenu_HeaderDay === null) {
      _element_ContextMenu_HeaderDay = createElement(
        "div",
        "calendar-context-menu"
      );
      ...
      _element_ContextMenu_HeaderDay_HideDay = buildContextMenuItemWithIcon(
        _element_ContextMenu_HeaderDay,
        "ib-close-icon",
        _options.hideDayText,
        function () {
          _options.visibleDays.splice(
            _options.visibleDays.indexOf(
              _element_ContextMenu_HeaderDay_SelectedDay
            ),
            1
          );
          _initialized = false;
      ...
      _element_ContextMenu_HeaderDay_HideDay_Separator =
        buildContextMenuSeparator(_element_ContextMenu_HeaderDay);
      _element_ContextMenu_HeaderDay_ShowOnlyWorkingDays =
        buildContextMenuItemWithIcon(
          _element_ContextMenu_HeaderDay,
          "ib-rhombus-hollow-icon",
          _options.showOnlyWorkingDaysText,
          function () {
      ...
      _element_ContextMenu_HeaderDay_ShowOnlyWorkingDays_Separator =
        buildContextMenuSeparator(_element_ContextMenu_HeaderDay);
      buildContextMenuItemWithIcon(
        _element_ContextMenu_HeaderDay,
        "ib-octagon-hollow-icon",
        _options.visibleDaysText + "...",
        function () {
          showSideMenu(true);
        }
      );
    ...
    if (!_element_Mode_DatePicker_Enabled) {
      ...
      if (_options.sideMenu.showDays) {
        ...
        var hideDayDisplay = _options.visibleDays.length > 1 ? "block" : "none",
          showOnlyWorkingDaysDisplay =
            _options.workingDays.length >= 1 &&
            !areArraysTheSame(_options.workingDays, _options.visibleDays)
              ? "block"
              : "none";
        _element_ContextMenu_HeaderDay_HideDay.style.display = hideDayDisplay;
        _element_ContextMenu_HeaderDay_HideDay_Separator.style.display =
          hideDayDisplay;
        _element_ContextMenu_HeaderDay_ShowOnlyWorkingDays.style.display =
          showOnlyWorkingDaysDisplay;
        _element_ContextMenu_HeaderDay_ShowOnlyWorkingDays_Separator.style.display =
          showOnlyWorkingDaysDisplay;
        cancelBubble(e);
  ...
  function buildDisabledBackground() {
    if (
      ...
      _element_Calendar_DisabledBackground = createElement(
        "div",
        "disabled-background"
      );
    }
  ...
  function isDisabledBackgroundDisplayed() {
    return _parameter_Document.body.contains(
      _element_Calendar_DisabledBackground
    );
  }
  function buildEventEditingDialog() {
    if (!_element_Mode_DatePicker_Enabled) {
      if (_element_Dialog_EventEditor === null) {
        _element_Dialog_EventEditor = createElement(
          "div",
          "calendar-dialog event-editor"
        );
      ...
      _element_Dialog_EventEditor_DisabledArea = createElement(
        "div",
        "disabled-area"
      );
      ...
      buildTab(
        ...
        function (tab) {
          showTabContents(
            tab,
            _element_Dialog_EventEditor_Tab_Event,
            _element_Dialog_EventEditor
          );
        },
        true
      ...
      buildTab(
        tabsContainer,
        _options.typeText.replace(":", _string.empty),
        function (tab) {
          showTabContents(
            tab,
            _element_Dialog_EventEditor_Tab_Type,
            _element_Dialog_EventEditor
          );
        }
      );
      buildTab(
        ...
        function (tab) {
          showTabContents(
            tab,
            _element_Dialog_EventEditor_Tab_Repeats,
            _element_Dialog_EventEditor
          );
        }
      );
      buildTab(tabsContainer, _options.optionalText, function (tab) {
        showTabContents(
          tab,
          _element_Dialog_EventEditor_Tab_Extra,
          _element_Dialog_EventEditor
        );
      });
      _element_Dialog_EventEditor_Tab_Event = buildTabContents(
        contents,
        true,
        false
      );
      _element_Dialog_EventEditor_Tab_Type = buildTabContents(
        contents,
        false,
        false
      );
      _element_Dialog_EventEditor_Tab_Repeats = buildTabContents(
        contents,
        false,
        false
      );
      _element_Dialog_EventEditor_Tab_Extra = buildTabContents(
        contents,
        false,
        false
      );
      ...
      _element_Dialog_EventEditor_RemoveButton = createButtonElement(
        buttonsContainer,
        _options.removeEventText,
        "remove",
  ...
  function buildEventEditorEventTabContent() {
    ...
    if (_options.maximumEventTitleLength > 0) {
      _element_Dialog_EventEditor_Title.maxLength =
    ...
    if (_options.eventColorsEditingEnabled) {
      _element_Dialog_EventEditor_SelectColors = createButtonElement(
        inputTitleContainer,
        "...",
        "select-colors",
    ...
    createTextHeaderElement(
      _element_Dialog_EventEditor_Tab_Event,
      _options.fromText.replace(":", _string.empty) + "/" + _options.toText
    );
    ...
    _element_Dialog_EventEditor_IsAllDay = buildCheckBox(
      _element_Dialog_EventEditor_Tab_Event,
      _options.isAllDayText,
      isAllDayChangedEvent
    )[0];
    _element_Dialog_EventEditor_ShowAlerts = buildCheckBox(
      _element_Dialog_EventEditor_Tab_Event,
      _options.showAlertsText
    )[0];
    _element_Dialog_EventEditor_ShowAsBusy = buildCheckBox(
      _element_Dialog_EventEditor_Tab_Event,
  ...
  function buildEventEditorTypeTabContent() {
    ...
    var radioButtonsTypesContainer = createElement(
      "div",
      "radio-buttons-container"
    );
    _element_Dialog_EventEditor_Tab_Type.appendChild(
      radioButtonsTypesContainer
    );
    for (var eventType in _options_EventType) {
      if (_options_EventType.hasOwnProperty(eventType)) {
        _options_EventType[eventType].eventEditorInput = buildRadioButton(
          radioButtonsTypesContainer,
          _options_EventType[eventType].text,
          "Type"
        );
  ...
  function buildEventEditorRepeatsTabContent() {
    ...
    _element_Dialog_EventEditor_Tab_Repeats.appendChild(
      radioButtonsRepeatsContainer
    );
    _element_Dialog_EventEditor_RepeatEvery_Never = buildRadioButton(
      radioButtonsRepeatsContainer,
      _options.repeatsNever,
      "RepeatType",
      repeatEveryEvent
    );
    _element_Dialog_EventEditor_RepeatEvery_EveryDay = buildRadioButton(
      radioButtonsRepeatsContainer,
      _options.repeatsEveryDayText,
      "RepeatType",
      repeatEveryEvent
    );
    _element_Dialog_EventEditor_RepeatEvery_EveryWeek = buildRadioButton(
      radioButtonsRepeatsContainer,
      _options.repeatsEveryWeekText,
      "RepeatType",
      repeatEveryEvent
    );
    _element_Dialog_EventEditor_RepeatEvery_Every2Weeks = buildRadioButton(
      radioButtonsRepeatsContainer,
      _options.repeatsEvery2WeeksText,
      "RepeatType",
      repeatEveryEvent
    );
    _element_Dialog_EventEditor_RepeatEvery_EveryMonth = buildRadioButton(
      radioButtonsRepeatsContainer,
      _options.repeatsEveryMonthText,
      "RepeatType",
      repeatEveryEvent
    );
    _element_Dialog_EventEditor_RepeatEvery_EveryYear = buildRadioButton(
      radioButtonsRepeatsContainer,
      _options.repeatsEveryYearText,
      "RepeatType",
      repeatEveryEvent
    );
    _element_Dialog_EventEditor_RepeatEvery_Custom = buildRadioButton(
      radioButtonsRepeatsContainer,
      _options.repeatsCustomText,
      "RepeatType",
      repeatEveryEvent
    );
    _element_Dialog_EventEditor_RepeatEvery_RepeatOptionsButton =
      createButtonElement(
        radioButtonsRepeatsContainer,
        "...",
        "repeat-options",
    ...
    _element_Dialog_EventEditor_RepeatEvery_Custom_Value = createElement(
      "input",
      null,
      "number"
    );
    _element_Dialog_EventEditor_RepeatEvery_Custom_Value.setAttribute(
      "min",
      "1"
    );
    toSplitContainer.appendChild(
      _element_Dialog_EventEditor_RepeatEvery_Custom_Value
    );
    var radioButtonsCustomRepeatsContainer = createElement(
      "div",
      "radio-buttons-container split-contents"
    );
    toSplitContainer.appendChild(radioButtonsCustomRepeatsContainer);
    _element_Dialog_EventEditor_RepeatEvery_Custom_Type_Daily =
      buildRadioButton(
        radioButtonsCustomRepeatsContainer,
        _options.dailyText,
        "RepeatCustomType"
      );
    _element_Dialog_EventEditor_RepeatEvery_Custom_Type_Weekly =
      buildRadioButton(
        radioButtonsCustomRepeatsContainer,
        _options.weeklyText,
        "RepeatCustomType"
      );
    _element_Dialog_EventEditor_RepeatEvery_Custom_Type_Monthly =
      buildRadioButton(
        radioButtonsCustomRepeatsContainer,
        _options.monthlyText,
        "RepeatCustomType"
      );
    _element_Dialog_EventEditor_RepeatEvery_Custom_Type_Yearly =
      buildRadioButton(
        radioButtonsCustomRepeatsContainer,
        _options.yearlyText,
        "RepeatCustomType"
  ...
  function buildEventEditorExtraTabContent() {
    ...
    _element_Dialog_EventEditor_AlertOffset = createElement(
      "input",
      null,
      "number"
    );
    ...
    if (_options.maximumEventGroupLength > 0) {
      _element_Dialog_EventEditor_Group.maxLength =
    ...
    _element_Dialog_EventEditor_Description = createElement(
      "textarea",
      "custom-scroll-bars"
    );
    _element_Dialog_EventEditor_Tab_Extra.appendChild(
      _element_Dialog_EventEditor_Description
    );
    if (_options.maximumEventDescriptionLength > 0) {
      _element_Dialog_EventEditor_Description.maxLength =
    ...
    if (_options.maximumEventLocationLength > 0) {
      _element_Dialog_EventEditor_Location.maxLength =
        _options.maximumEventLocationLength;
    }
  }
  function repeatEveryEvent() {
    _element_Dialog_EventEditor_RepeatEvery_RepeatOptionsButton.disabled =
      _element_Dialog_EventEditor_RepeatEvery_Never.checked;
    _element_Dialog_EventEditor_RepeatEvery_Custom_Type_Yearly.disabled =
      !_element_Dialog_EventEditor_RepeatEvery_Custom.checked;
    _element_Dialog_EventEditor_RepeatEvery_Custom_Type_Daily.disabled =
      !_element_Dialog_EventEditor_RepeatEvery_Custom.checked;
    _element_Dialog_EventEditor_RepeatEvery_Custom_Type_Weekly.disabled =
      !_element_Dialog_EventEditor_RepeatEvery_Custom.checked;
    _element_Dialog_EventEditor_RepeatEvery_Custom_Type_Monthly.disabled =
      !_element_Dialog_EventEditor_RepeatEvery_Custom.checked;
    _element_Dialog_EventEditor_RepeatEvery_Custom_Value.disabled =
      !_element_Dialog_EventEditor_RepeatEvery_Custom.checked;
  }
  function isAllDayChanged(eventDetails) {
    eventDetails = isDefined(eventDetails)
      ? eventDetails
      : _element_Dialog_EventEditor_EventDetails;
    var disabled = false,
      locked =
        isDefined(eventDetails) && isDefinedBoolean(eventDetails.locked)
          ? eventDetails.locked
          : false;
    if (locked) {
      disabled = true;
      ...
      if (_element_Dialog_EventEditor_IsAllDay.checked) {
        _element_Dialog_EventEditor_DateTo.value =
          _element_Dialog_EventEditor_DateFrom.value;
    ...
    setMinimumDate(
      _element_Dialog_EventEditor_RepeatOptions_RepeatEnds,
      toDate
    );
    if (fromDate > toDate) {
  ...
  function showEventEditingDialog(
    eventDetails,
    overrideTodayDate,
    overrideTimeValues,
    originDayDate
    ...
    if (isFunction(_options.onBeforeEventAddEdit)) {
      ...
      if (isDefined(eventDetails)) {
        ...
        _element_Dialog_EventEditor_TimeFrom.value = getTimeForDisplay(
          eventDetails.from,
          false
        );
        _element_Dialog_EventEditor_TimeTo.value = getTimeForDisplay(
          eventDetails.to,
          false
        );
        _element_Dialog_EventEditor_IsAllDay.checked = getBoolean(
          eventDetails.isAllDay
        );
        _element_Dialog_EventEditor_ShowAlerts.checked = getBoolean(
          eventDetails.showAlerts,
          true
        );
        _element_Dialog_EventEditor_ShowAsBusy.checked = getBoolean(
          eventDetails.showAsBusy,
          true
        );
        _element_Dialog_EventEditor_Title.value = getString(eventDetails.title);
        _element_Dialog_EventEditor_Description.value = getString(
          eventDetails.description
        );
        _element_Dialog_EventEditor_Location.value = getString(
        ...
        _element_Dialog_EventEditor_Colors_Color.value = getString(
          eventDetails.color,
          _options.defaultEventBackgroundColor
        );
        _element_Dialog_EventEditor_Colors_ColorText.value = getString(
          eventDetails.colorText,
          _options.defaultEventTextColor
        );
        _element_Dialog_EventEditor_Colors_ColorBorder.value = getString(
          eventDetails.colorBorder,
          _options.defaultEventBorderColor
        );
        _element_Dialog_EventEditor_RepeatEvery_Custom_Value.value = getNumber(
          eventDetails.repeatEveryCustomValue,
          1
        );
        _element_Dialog_EventEditor_AlertOffset.value = getNumber(
          eventDetails.alertOffset,
          0
        );
        setSelectedDate(
          eventDetails.from,
          _element_Dialog_EventEditor_DateFrom
        );
        ...
        _element_Dialog_EventEditor_RepeatOptions_Mon.checked =
          excludeDays.indexOf(1) > _value.notFound;
        _element_Dialog_EventEditor_RepeatOptions_Tue.checked =
          excludeDays.indexOf(2) > _value.notFound;
        _element_Dialog_EventEditor_RepeatOptions_Wed.checked =
          excludeDays.indexOf(3) > _value.notFound;
        _element_Dialog_EventEditor_RepeatOptions_Thu.checked =
          excludeDays.indexOf(4) > _value.notFound;
        _element_Dialog_EventEditor_RepeatOptions_Fri.checked =
          excludeDays.indexOf(5) > _value.notFound;
        _element_Dialog_EventEditor_RepeatOptions_Sat.checked =
          excludeDays.indexOf(6) > _value.notFound;
        _element_Dialog_EventEditor_RepeatOptions_Sun.checked =
          excludeDays.indexOf(0) > _value.notFound;
        setSelectedDate(
          eventDetails.repeatEnds,
          _element_Dialog_EventEditor_RepeatOptions_RepeatEnds
        );
        _element_Dialog_EventEditor_ShowEditingRepeatSeriesWarning =
          repeatEvery > _enum_RepeatType.never &&
          isDefined(originDayDate) &&
          !doDatesMatch(eventDetails.from, originDayDate);
        ...
        _element_Dialog_EventEditor_Colors_Color.value =
          _options.defaultEventBackgroundColor;
        _element_Dialog_EventEditor_Colors_ColorText.value =
          _options.defaultEventTextColor;
        _element_Dialog_EventEditor_Colors_ColorBorder.value =
          _options.defaultEventBorderColor;
        _element_Dialog_EventEditor_RepeatEvery_Never.checked = true;
  ...
  function setEventEditingDialogInDuplicateMode() {
    ...
    _element_Dialog_EventEditor_AddUpdateButton.value = _options.addText;
    _element_Dialog_EventEditor_RemoveButton.style.display = "none";
    _element_Dialog_EventEditor_EventDetails = cloneEventDetails(
      _element_Dialog_EventEditor_EventDetails
    );
    buildToolbarButton(
      _element_Dialog_EventEditor_TitleBar,
    ...
    if (fromTime.length < 2) {
      ...
      var fromDate = getSelectedDate(_element_Dialog_EventEditor_DateFrom),
        ...
        repeatEveryCustomValue = parseInt(
          _element_Dialog_EventEditor_RepeatEvery_Custom_Value.value
        ),
      ...
      if (toDate < fromDate) {
        ...
        var normalSave = function (
          ...
          setTimeOnDate(
            newFromDate,
            _element_Dialog_EventEditor_TimeFrom.value
          );
          ...
          if (!ignoreFields) {
            var isExistingEvent = isDefined(newId),
              newEvent = {
                ...
                colorBorder:
                  _element_Dialog_EventEditor_EventDetails.colorBorder,
                repeatEveryExcludeDays:
                  _element_Dialog_EventEditor_EventDetails.repeatEveryExcludeDays,
            ...
            if (_element_Dialog_EventEditor_RepeatEvery_Never.checked) {
              ...
              _element_Dialog_EventEditor_RepeatEvery_EveryDay.checked
            ) {
              newEvent.repeatEvery = _enum_RepeatType.everyDay;
            } else if (
              _element_Dialog_EventEditor_RepeatEvery_EveryWeek.checked
            ) {
              newEvent.repeatEvery = _enum_RepeatType.everyWeek;
            } else if (
              _element_Dialog_EventEditor_RepeatEvery_Every2Weeks.checked
            ) {
              newEvent.repeatEvery = _enum_RepeatType.every2Weeks;
            } else if (
              _element_Dialog_EventEditor_RepeatEvery_EveryMonth.checked
            ) {
              newEvent.repeatEvery = _enum_RepeatType.everyMonth;
            } else if (
              _element_Dialog_EventEditor_RepeatEvery_EveryYear.checked
            ) {
            ...
            if (
              _element_Dialog_EventEditor_RepeatEvery_Custom_Type_Daily.checked
            ) {
              newEvent.repeatEveryCustomType = _enum_RepeatCustomType.daily;
            } else if (
              _element_Dialog_EventEditor_RepeatEvery_Custom_Type_Weekly.checked
            ) {
              newEvent.repeatEveryCustomType = _enum_RepeatCustomType.weekly;
            } else if (
              _element_Dialog_EventEditor_RepeatEvery_Custom_Type_Monthly.checked
            ) {
              newEvent.repeatEveryCustomType = _enum_RepeatCustomType.monthly;
            } else if (
              _element_Dialog_EventEditor_RepeatEvery_Custom_Type_Yearly.checked
            ) {
            ...
            if (isExistingEvent) {
              ...
              showNotificationPopUp(
                _options.eventUpdatedText.replace(
                  "{0}",
                  _element_Dialog_EventEditor_EventDetails.title
                )
              );
            } else {
              _that.addEvent(newEvent, false);
              showNotificationPopUp(
                _options.eventAddedText.replace("{0}", newEvent.title)
              );
            }
            if (_options.isWidget) {
              build(_calendar_CurrentDate);
            ...
            _element_Dialog_EventEditor_EventDetails.repeatEnds =
              newRepeatEndsDate;
          }
        };
        if (_element_Dialog_EventEditor_ShowEditingRepeatSeriesWarning) {
          ...
          var onNoEvent = function () {
            ...
            normalSave(
              _element_Dialog_EventEditor_EventDetails.id,
              fromDate,
              toDate,
              repeatEnds
            );
          ...
          showMessageDialog(
            _options.confirmEventUpdateTitle,
            _options.confirmEventUpdateMessage,
            onYesEvent,
            onNoEvent,
            false,
            true,
            _options.forwardText,
            _options.seriesText
          );
        } else {
          normalSave(
  ...
  function isEventLocked(eventDetails) {
    return isDefined(eventDetails) && isDefinedBoolean(eventDetails.locked)
      ? eventDetails.locked
      : false;
  }
  ...
  function buildEventEditingColorDialog() {
    if (
      ...
      _element_Dialog_EventEditor_Colors = createElement(
        "div",
        "calendar-dialog event-editor-colors"
      );
  ...
  function eventColorsDialogEvent_OK() {
    eventColorsDialogEvent_Cancel();
    _element_Dialog_EventEditor_EventDetails.color =
      _element_Dialog_EventEditor_Colors_Color.value;
    _element_Dialog_EventEditor_EventDetails.colorText =
      _element_Dialog_EventEditor_Colors_ColorText.value;
    _element_Dialog_EventEditor_EventDetails.colorBorder =
      _element_Dialog_EventEditor_Colors_ColorBorder.value;
  }
  ...
  function buildEventEditingRepeatOptionsDialog() {
    if (
      ...
      _element_Dialog_EventEditor_RepeatOptions = createElement(
        "div",
        "calendar-dialog event-editor-repeat-options"
      );
      _elements_InDocumentBody.push(_element_Dialog_EventEditor_RepeatOptions);
      _parameter_Document.body.appendChild(
        _element_Dialog_EventEditor_RepeatOptions
      );
      var titleBar = createElement("div", "title-bar");
      ...
      createTextHeaderElement(
        section1,
        _options.daysToExcludeText,
        "text-header"
      );
      _element_Dialog_EventEditor_RepeatOptions_Mon = buildCheckBox(
        section1,
        _options.dayNames[0]
      )[0];
      _element_Dialog_EventEditor_RepeatOptions_Tue = buildCheckBox(
        section1,
        _options.dayNames[1]
      )[0];
      _element_Dialog_EventEditor_RepeatOptions_Wed = buildCheckBox(
        section1,
        _options.dayNames[2]
      )[0];
      _element_Dialog_EventEditor_RepeatOptions_Thu = buildCheckBox(
        section1,
        _options.dayNames[3]
      )[0];
      _element_Dialog_EventEditor_RepeatOptions_Fri = buildCheckBox(
        section1,
        _options.dayNames[4]
      )[0];
      _element_Dialog_EventEditor_RepeatOptions_Sat = buildCheckBox(
        section1,
        _options.dayNames[5]
      )[0];
      _element_Dialog_EventEditor_RepeatOptions_Sun = buildCheckBox(
        section1,
        _options.dayNames[6]
      )[0];
      var section2 = createElement("div", "section");
      ...
      createButtonElement(
        ...
        _options.updateText,
        "update",
        eventRepeatOptionsDialogEvent_OK
      );
      createButtonElement(
        buttonsContainer,
        _options.cancelText,
  ...
  function showMessageDialog(
    ...
    onYesEvent,
    onNoEvent,
    showRemoveAllEventsCheckBox,
    showNoButton,
    yesText,
    noText
  ) {
    showRemoveAllEventsCheckBox = isDefined(showRemoveAllEventsCheckBox)
      ? showRemoveAllEventsCheckBox
      : false;
  ...
  function buildExportEventsDialog() {
    if (
      ...
      _element_Dialog_ExportEvents = createElement(
        "div",
        "calendar-dialog export-events"
      );
      ...
      _element_Dialog_ExportEvents_Filename = createElement(
        "input",
        null,
        "text"
      );
      _element_Dialog_ExportEvents_Filename.placeholder =
        _options.exportFilenamePlaceholderText;
      ...
      _element_Dialog_ExportEvents_Option_ExportEventsToClipboard =
        buildCheckBox(
          contents,
          _options.copyToClipboardOnlyText,
          showExportEventsDialogOptions
        )[0];
      _element_Dialog_ExportEvents_Options = createElement(
        "div",
        "split options"
      );
      contents.appendChild(_element_Dialog_ExportEvents_Options);
      var radioButtonsContainer1 = createElement(
        "div",
        "radio-buttons-container split-contents"
      );
      _element_Dialog_ExportEvents_Options.appendChild(radioButtonsContainer1);
      var radioButtonsContainer2 = createElement(
        "div",
        "radio-buttons-container split-contents"
      );
      _element_Dialog_ExportEvents_Options.appendChild(radioButtonsContainer2);
      _element_Dialog_ExportEvents_Option_CSV = buildRadioButton(
        radioButtonsContainer1,
        "CSV",
        "ExportType"
      );
      _element_Dialog_ExportEvents_Option_XML = buildRadioButton(
        radioButtonsContainer1,
        "XML",
        "ExportType"
      );
      _element_Dialog_ExportEvents_Option_JSON = buildRadioButton(
        radioButtonsContainer1,
        "JSON",
        "ExportType"
      );
      _element_Dialog_ExportEvents_Option_TEXT = buildRadioButton(
        radioButtonsContainer1,
        "TXT",
        "ExportType"
      );
      _element_Dialog_ExportEvents_Option_iCAL = buildRadioButton(
        radioButtonsContainer2,
        "iCAL",
        "ExportType"
      );
      _element_Dialog_ExportEvents_Option_MD = buildRadioButton(
        radioButtonsContainer2,
        "MD",
        "ExportType"
      );
      _element_Dialog_ExportEvents_Option_HTML = buildRadioButton(
        radioButtonsContainer2,
        "HTML",
        "ExportType"
      );
      _element_Dialog_ExportEvents_Option_TSV = buildRadioButton(
        radioButtonsContainer2,
        "TSV",
        "ExportType"
      );
  ...
  function showExportEventsDialogOptions() {
    _element_Dialog_ExportEvents_Filename.disabled =
      _element_Dialog_ExportEvents_Option_ExportEventsToClipboard.checked;
  }
  ...
  function exportEventsFromOptionSelected() {
    ...
    if (_element_Dialog_ExportEvents_Option_CSV.checked) {
      exportEvents(
        _element_Dialog_ExportEvents_ExportEvents,
        "csv",
        _element_Dialog_ExportEvents_Filename.value,
        _element_Dialog_ExportEvents_Option_ExportEventsToClipboard.checked
      );
  ...
  function buildSearchDialog() {
    if (!_element_Mode_DatePicker_Enabled && _element_Dialog_Search === null) {
      ...
      var closeButton = buildToolbarButton(
        titleBar,
        "ib-close",
        _options.closeTooltipText,
        hideSearchDialog
      ...
      _element_Dialog_Search_MinimizedRestoreButton = buildToolbarButton(
        titleBar,
        "ib-minus",
        _options.minimizedTooltipText,
      ...
      _element_Dialog_Search_For.placeholder =
        _options.searchTextBoxPlaceholder;
      ...
      _element_Dialog_Search_History_DropDown_Button = createElement(
        "div",
        "ib-arrow-down-full"
      );
      _element_Dialog_Search_History_DropDown_Button.style.display = "none";
      _element_Dialog_Search_History_DropDown_Button.onclick =
        showFullSearchHistory;
      historyContainer.appendChild(
        _element_Dialog_Search_History_DropDown_Button
      );
      _element_Dialog_Search_History_DropDown = createElement(
        "div",
        "history-dropdown custom-scroll-bars"
      );
      ...
      _element_Dialog_Search_Not = buildCheckBox(
        checkboxOptionsContainer,
        _options.notSearchText,
        searchOptionsChanged
      )[0];
      _element_Dialog_Search_MatchCase = buildCheckBox(
        checkboxOptionsContainer,
        _options.matchCaseText,
        searchOptionsChanged
      )[0];
      _element_Dialog_Search_Advanced = buildCheckBox(
        checkboxOptionsContainer,
        _options.advancedText + ":",
        searchAdvancedChecked
      )[0];
      _element_Dialog_Search_Advanced.checked = true;
      _element_Dialog_Search_Advanced_Container = createElement(
        "div",
        "advanced"
      );
      _element_Dialog_Search_Contents.appendChild(
        _element_Dialog_Search_Advanced_Container
      );
      var optionsSplitContainer = createElement("div", "split");
      _element_Dialog_Search_Advanced_Container.appendChild(
      ...
      _element_Dialog_Search_Include_Title = buildCheckBox(
        checkboxContainer,
        _options.titleText.replace(":", _string.empty),
        searchOptionsChanged
      )[0];
      _element_Dialog_Search_Include_Location = buildCheckBox(
        checkboxContainer,
        _options.locationText.replace(":", _string.empty),
        searchOptionsChanged
      )[0];
      _element_Dialog_Search_Include_Description = buildCheckBox(
        checkboxContainer,
        _options.descriptionText.replace(":", _string.empty),
        searchOptionsChanged
      )[0];
      _element_Dialog_Search_Include_Group = buildCheckBox(
        checkboxContainer,
        _options.groupText.replace(":", _string.empty),
        searchOptionsChanged
      )[0];
      _element_Dialog_Search_Include_Url = buildCheckBox(
        checkboxContainer,
        _options.urlText.replace(":", _string.empty),
        searchOptionsChanged
      )[0];
      _element_Dialog_Search_Include_Title.checked = true;
      createTextHeaderElement(
        splitContents2,
        _options.optionsText,
        "text-header"
      );
      var radioButtonsContainer = createElement(
        "div",
        "radio-buttons-container"
      );
      splitContents2.appendChild(radioButtonsContainer);
      _element_Dialog_Search_Option_StartsWith = buildRadioButton(
        radioButtonsContainer,
        _options.startsWithText,
        "SearchOptionType",
        searchOptionsChanged
      );
      _element_Dialog_Search_Option_EndsWith = buildRadioButton(
        radioButtonsContainer,
        _options.endsWithText,
        "SearchOptionType",
        searchOptionsChanged
      );
      _element_Dialog_Search_Option_Contains = buildRadioButton(
        radioButtonsContainer,
        _options.containsText,
        "SearchOptionType",
        searchOptionsChanged
      );
      ...
      _element_Dialog_Search_Previous = createButtonElement(
        buttonsContainer,
        _options.previousText,
        "previous",
        searchOnPrevious
      );
      _element_Dialog_Search_Next = createButtonElement(
        buttonsContainer,
        _options.nextText,
        "next",
        searchOnNext
  ...
  function searchForTextChanged(showHistoryDropDown) {
    showHistoryDropDown = isDefined(showHistoryDropDown)
      ? showHistoryDropDown
      : true;
    ...
    _element_Dialog_Search_Next.disabled =
      _element_Dialog_Search_For.value === _string.empty;
    _element_Dialog_Search_SearchResults = [];
    _element_Dialog_Search_SearchIndex = 0;
    _element_Dialog_Search_FocusedEventID = null;
  ...
  function centerSearchDialog() {
    if (
      _element_Dialog_Search !== null &&
      !_element_Dialog_Search_Moved &&
      !_element_Mode_DatePicker_Enabled
    ) {
      if (isDefinedNumber(_options_Search.left)) {
        _element_Dialog_Search.style.left = _options_Search.left + "px";
      } else {
        _element_Dialog_Search.style.left =
          _parameter_Window.innerWidth / 2 -
          _element_Dialog_Search.offsetWidth / 2 +
          "px";
      }
      if (isDefinedNumber(_options_Search.top)) {
        _element_Dialog_Search.style.top = _options_Search.top + "px";
      } else {
        _element_Dialog_Search.style.top =
          _parameter_Window.innerHeight / 2 -
          _element_Dialog_Search.offsetHeight / 2 +
          "px";
      }
    }
  }
  ...
  function minimizeRestoreDialog() {
    if (isSearchDialogContentVisible()) {
      ...
      _element_Dialog_Search_MinimizedRestoreButton.className =
        "ib-square-hollow";
      addToolTip(
        _element_Dialog_Search_MinimizedRestoreButton,
        _options.restoreTooltipText
      );
    } else {
  ...
  function searchOnEnter(e) {
    if (
      e.keyCode === _enum_KeyCodes.enter &&
      isControlKey(e) &&
      !_element_Dialog_Search_Previous.disabled
    ) {
      searchOnPrevious();
    } else if (
      e.keyCode === _enum_KeyCodes.enter &&
  ...
  function searchOnNext() {
    if (_element_Dialog_Search_SearchResults.length === 0) {
      var startingID = _element_ID_Event_Day,
        ...
        search = !matchCase
          ? _element_Dialog_Search_For.value.toLowerCase()
          : _element_Dialog_Search_For.value,
        monthYearsFound = {},
      ...
      for (
        ...
        if (isEventVisible(eventDetails)) {
          ...
          if (
            _element_Dialog_Search_Include_Title.checked &&
            isSearchTextAvailable(title, search)
          ) {
            found = true;
          } else if (
            _element_Dialog_Search_Include_Location.checked &&
            isSearchTextAvailable(location, search)
          ) {
            found = true;
          } else if (
            _element_Dialog_Search_Include_Description.checked &&
            isSearchTextAvailable(description, search)
          ) {
            found = true;
          } else if (
            _element_Dialog_Search_Include_Group.checked &&
            isSearchTextAvailable(group, search)
          ) {
            found = true;
          } else if (
            _element_Dialog_Search_Include_Url.checked &&
            isSearchTextAvailable(url, search)
          ) {
            found = true;
          }
          if (not) {
          ...
          if (found) {
            ...
            if (
              eventElement !== null ||
              (!isFullDayViewVisible &&
                !isAllEventsViewVisible &&
                !isFullWeekViewVisible &&
                !isTimelineViewVisible)
            ) {
              if (
                isFullDayViewVisible ||
                isAllEventsViewVisible ||
                ...
                _element_Dialog_Search_SearchResults.push(
                  cloneEventDetails(eventDetails, false)
                );
              } else {
                var monthYear =
                  eventDetails.from.getMonth() +
                  "-" +
                  eventDetails.from.getFullYear();
  ...
  function updateSearchButtons() {
    ...
    _element_Dialog_Search_Next.disabled =
      _element_Dialog_Search_SearchIndex ===
        _element_Dialog_Search_SearchResults.length - 1 ||
      _element_Dialog_Search_SearchResults.length === 0;
  }
  ...
  function storeSearchOptions(storeSearchHistory) {
    storeSearchHistory = isDefined(storeSearchHistory)
      ? storeSearchHistory
      : false;
    ...
    startTimer(
      _timer_Name.searchOptionsChanged,
      function () {
        ...
        if (!storeSearchHistory || searchForTextAddedToHistory) {
          ...
          _options_Search.showAdvanced =
            _element_Dialog_Search_Advanced.checked;
          _options_Search.searchTitle =
            _element_Dialog_Search_Include_Title.checked;
          _options_Search.searchLocation =
            _element_Dialog_Search_Include_Location.checked;
          _options_Search.searchDescription =
            _element_Dialog_Search_Include_Description.checked;
          _options_Search.searchGroup =
            _element_Dialog_Search_Include_Group.checked;
          _options_Search.searchUrl =
            _element_Dialog_Search_Include_Url.checked;
          _options_Search.startsWith =
            _element_Dialog_Search_Option_StartsWith.checked;
          _options_Search.endsWith =
            _element_Dialog_Search_Option_EndsWith.checked;
          _options_Search.contains =
            _element_Dialog_Search_Option_Contains.checked;
          ...
          fireCustomTrigger(
            _options.events.onSearchOptionsUpdated,
            _options_Search
          );
        }
      },
  ...
  function showSearchHistoryDropDownForSearch() {
    ...
    if (historyLength > 0) {
      ...
      startTimer(
        _timer_Name.searchEventsHistoryDropDown,
        function () {
          ...
          if (trimString(lookupText) !== _string.empty) {
            ...
            for (
              ...
              var historyText = _options_Search.history[historyIndex];
              if (
                startsWithAnyCase(historyText, lookupText) &&
                historyText.toLowerCase() !== lookupText.toLowerCase()
              ) {
                addSearchHistoryDropDownItem(
                  _options_Search.history[historyIndex],
                  lookupText.length
                );
  ...
  function addSearchHistoryDropDownItem(historyText, startBoldLength) {
    ...
    historyDropDownItem.onclick = function (e) {
      ...
      _element_Dialog_Search_For.selectionStart =
        _element_Dialog_Search_For.selectionEnd =
          _element_Dialog_Search_For.value.length;
      _element_Dialog_Search_For.focus();
      searchForTextChanged(false);
    };
  }
  function hideSearchHistoryDropDownMenu() {
    ...
    if (
      _element_Dialog_Search_History_DropDown !== null &&
      _element_Dialog_Search_History_DropDown_Button.className ===
        "ib-arrow-up-full"
    ) {
  ...
  function buildConfigurationDialog() {
    if (
      ...
      _element_Dialog_Configuration = createElement(
        "div",
        "calendar-dialog configuration"
      );
      ...
      buildToolbarButton(
        ...
        "ib-close",
        _options.closeTooltipText,
        configurationDialogEvent_Cancel,
        true
      );
      var contents = createElement("div", "contents");
      ...
      buildTab(
        tabsContainer,
        _options.displayTabText,
        function (tab) {
          showTabContents(
            tab,
            _element_Dialog_Configuration_Display,
            _element_Dialog_Configuration
          );
        },
        true
      );
      buildTab(tabsContainer, _options.organizerTabText, function (tab) {
        showTabContents(
          tab,
          _element_Dialog_Configuration_Organizer,
          _element_Dialog_Configuration
        );
      });
      _element_Dialog_Configuration_Display = buildTabContents(
        contents,
        true,
        false
      );
      _element_Dialog_Configuration_Organizer = buildTabContents(
        contents,
        false,
        false
      );
      _element_Dialog_Configuration_Display_EnableAutoRefresh = buildCheckBox(
        _element_Dialog_Configuration_Display,
        _options.enableAutoRefreshForEventsText
      )[0];
      _element_Dialog_Configuration_Display_EnableBrowserNotifications =
        buildCheckBox(
          _element_Dialog_Configuration_Display,
          _options.enableBrowserNotificationsText,
          null,
          null,
          null,
          "checkbox-tabbed-in"
        )[0];
      _element_Dialog_Configuration_Display_EnableTooltips = buildCheckBox(
        _element_Dialog_Configuration_Display,
        _options.enableTooltipsText,
        null,
        null,
        null,
        "checkbox-tabbed-down"
      )[0];
      _element_Dialog_Configuration_Display_EnableDragAndDropForEvents =
        buildCheckBox(
          _element_Dialog_Configuration_Display,
          _options.enableDragAndDropForEventText
        )[0];
      _element_Dialog_Configuration_Display_ShowHolidaysInTheDisplays =
        buildCheckBox(
          _element_Dialog_Configuration_Display,
          _options.showHolidaysInTheDisplaysText
        )[0];
      createTextHeaderElement(
        _element_Dialog_Configuration_Organizer,
        _options.organizerNameText
      );
      _element_Dialog_Configuration_Organizer_Name = createElement(
        "input",
        null,
        "text"
      );
      _element_Dialog_Configuration_Organizer.appendChild(
        _element_Dialog_Configuration_Organizer_Name
      );
      createTextHeaderElement(
        _element_Dialog_Configuration_Organizer,
        _options.organizerEmailAddressText
      );
      _element_Dialog_Configuration_Organizer_Email = createElement(
        "input",
        null,
        "email"
      );
      _element_Dialog_Configuration_Organizer.appendChild(
        _element_Dialog_Configuration_Organizer_Email
      );
      var buttonsContainer = createElement("div", "buttons-container");
      contents.appendChild(buttonsContainer);
      createButtonElement(
  ...
  function configurationDialogEvent_OK() {
    ...
    _options.eventNotificationsEnabled =
      _element_Dialog_Configuration_Display_EnableBrowserNotifications.checked;
    _options.tooltipsEnabled =
      _element_Dialog_Configuration_Display_EnableTooltips.checked;
    _options.dragAndDropForEventsEnabled =
      _element_Dialog_Configuration_Display_EnableDragAndDropForEvents.checked;
    _options.showHolidays =
      _element_Dialog_Configuration_Display_ShowHolidaysInTheDisplays.checked;
    _options.organizerName = _element_Dialog_Configuration_Organizer_Name.value;
    _options.organizerEmailAddress =
      _element_Dialog_Configuration_Organizer_Email.value;
    _initialized = false;
    fireCustomTrigger(_options.events.onOptionsUpdated, _options);
  ...
  function showConfigurationDialog() {
    addNode(_parameter_Document.body, _element_Calendar_DisabledBackground);
    _element_Dialog_Configuration_Display_EnableAutoRefresh.checked =
      _timer_AutoRefresh_Enabled;
    _element_Dialog_Configuration_Display_EnableBrowserNotifications.checked =
      _options.eventNotificationsEnabled;
    _element_Dialog_Configuration_Display_EnableTooltips.checked =
      _options.tooltipsEnabled;
  ...
  function isEventVisible(eventDetails) {
    ...
    if (group !== _string.empty) {
      if (isDefined(_options_Configuration.visibleGroups)) {
        visible =
          _options_Configuration.visibleGroups.indexOf(configGroup) >
          _value.notFound;
      }
    } else {
    ...
    if (
      visible &&
      isDefined(_options_Configuration.visibleEventTypes) &&
      _options_EventType.hasOwnProperty(type)
    ) {
      visible =
        _options_Configuration.visibleEventTypes.indexOf(type) >
        _value.notFound;
    }
    return visible;
  }
  function buildJumpTpDateDialog() {
    if (
      !_element_Mode_DatePicker_Enabled &&
      _element_Dialog_JumpToDate === null &&
      _options.jumpToDateEnabled
    ) {
      _element_Dialog_JumpToDate = createElement(
        "div",
        "calendar-dialog jump-to-date"
      );
      ...
      var buttonsContainer = createElement("div", "buttons-container");
      contents.appendChild(buttonsContainer);
      var goButton = createButtonElement(
        buttonsContainer,
        _options.goText,
        "go",
        jumpToSelectedDate
  ...
  function setDefaultJumpToDate() {
    ...
    if (viewOpen === null) {
      fireCustomTrigger(
        _options.events.onSetDate,
        _element_Dialog_JumpToDate_Date
      );
      ...
      if (viewOpen === _element_View_FullDay) {
        ...
        setSelectedDate(
          _element_View_FullWeek_DateSelected,
          _element_Dialog_JumpToDate_Date
        );
      } else if (viewOpen === _element_View_FullYear) {
        setSelectedDate(
  ...
  function jumpToSelectedDate() {
    ...
    if (
      selectedDate.getFullYear() >= _options.minimumYear &&
      selectedDate.getFullYear() <= _options.maximumYear
    ) {
      if (viewOpen === null) {
        build(selectedDate);
        buildFullMonthViewPinUpImage();
  ...
  function buildTooltip() {
    if (_element_Tooltip === null) {
      ...
      _element_Tooltip_TitleButtons_CloseButton = createElement(
        "div",
        "ib-close"
      );
      _element_Tooltip_TitleButtons_EditButton = createElement(
        "div",
        "ib-edit"
      );
      _element_Tooltip_TitleButtons = createElement("div", "title-buttons");
      _element_Tooltip_TitleButtons.appendChild(
        _element_Tooltip_TitleButtons_CloseButton
      );
      _element_Tooltip_TitleButtons.appendChild(
        _element_Tooltip_TitleButtons_EditButton
      );
  ...
  function showTooltip(e, eventDetails, text, overrideShow) {
    ...
    if (
      ...
      var toolTipDelay = isDefinedString(text)
        ? _options.tooltipDelay
        : _options.eventTooltipDelay;
      startTimer(
        _timer_Name.showToolTip,
        function () {
          if (
            overrideShow ||
            (!isDisabledBackgroundDisplayed() &&
              !isFullMonthViewYearDropDownVisible() &&
              !areContextMenusVisible() &&
              _events_Dragged === null)
          ) {
            text = isDefined(text) ? text : _string.empty;
            _element_Tooltip.className =
              text === _string.empty
            ...
            if (isDefinedString(text)) {
              ...
              if (
                !fireCustomTrigger(
                  _options.events.onToolTipEventRender,
                  _element_Tooltip,
                  eventDetails
                )
              ) {
                ...
                if (isDefinedStringAndSet(eventDetails.url)) {
                  var urlText = createSpanElement(
                    _element_Tooltip_Url,
                    getShortUrlString(eventDetails.url),
                    "url-text"
                  );
                ...
                _element_Tooltip_Title.innerHTML += stripHTMLTagsFromText(
                  eventDetails.title
                );
                if (
                  isDefinedNumber(eventDetails.repeatEvery) &&
                ...
                if (eventDetails.from.getDate() === eventDetails.to.getDate()) {
                  if (eventDetails.isAllDay) {
                    ...
                    setNodeText(
                      _element_Tooltip_Date,
                      getTimeToTimeDisplay(eventDetails.from, eventDetails.to)
                    );
                    setNodeText(
                      _element_Tooltip_TotalTime,
                      getFriendlyTimeBetweenTwoDate(
                        eventDetails.from,
                        eventDetails.to
                      )
  ...
  function isTooltipVisible() {
    return (
      doesTimerExist(_timer_Name.showToolTip) ||
      (_element_Tooltip !== null && _element_Tooltip.style.display === "block")
    );
  }
  ...
  function buildViewDayNamesHeader(container, reverseHeaders) {
    if (container !== null) {
      ...
      if (
        ...
        buildViewDayNamesHeaderSection(
          container,
          _options.startOfWeekDay,
          headerNamesLength
        );
  ...
  function toggleSingleDayView(headerNameIndex) {
    if (!_element_Mode_DatePicker_Enabled) {
      ...
      if (
        _element_Calendar_PreviousDaysVisibleBeforeSingleDayView.length === 0
      ) {
        var visibleDaysLength = _options.visibleDays.length;
        if (visibleDaysLength > 1) {
          for (
            var visibleDayIndex = 0;
            visibleDayIndex < visibleDaysLength;
            visibleDayIndex++
          ) {
            _element_Calendar_PreviousDaysVisibleBeforeSingleDayView.push(
              _options.visibleDays[visibleDayIndex]
            );
          }
        ...
        _options.visibleDays = [];
        var originalVisibleDaysLength =
          _element_Calendar_PreviousDaysVisibleBeforeSingleDayView.length;
        for (
          var previousVisibleDayIndex = 0;
          previousVisibleDayIndex < originalVisibleDaysLength;
          previousVisibleDayIndex++
        ) {
          _options.visibleDays.push(
            _element_Calendar_PreviousDaysVisibleBeforeSingleDayView[
              previousVisibleDayIndex
            ]
          );
  ...
  function removeViewOpened(element, triggerCustomTrigger) {
    triggerCustomTrigger = isDefined(triggerCustomTrigger)
    ...
    for (
      ...
      viewOpenedIndex < viewsOpenedLength;
      viewOpenedIndex++
    ) {
  ...
  function isViewVisible(element) {
    return (
      isDefined(element) &&
      element.className &&
      element.className.indexOf("view-shown") > _value.notFound
    );
  }
  ...
  function getActiveView() {
    return _element_View_Opened.length > 0
      ? _element_View_Opened[_element_View_Opened.length - 1]
      : null;
  }
  ...
  function getHourMinutesFromMousePositionClick(e, container) {
    var contentHoursOffset = getOffset(container),
      ...
      minutesFromTop = _parameter_Math.floor(
        (e.pageY - contentHoursOffset.top) / pixelsPerMinute
      ),
  ...
  function setEventPositionAndGetScrollTop(
    contents,
    container,
    displayDate,
    event,
    ...
    if (!eventDetails.isAllDay) {
      var repeatEvery = getNumber(eventDetails.repeatEvery);
      if (
        doDatesMatch(eventDetails.from, displayDate) ||
        repeatEvery > _enum_RepeatType.never
      ) {
        minutesTop = pixelsPerMinute * getMinutesIntoDay(eventDetails.from);
      }
      if (
        doDatesMatch(eventDetails.to, displayDate) ||
        repeatEvery > _enum_RepeatType.never
      ) {
        minutesHeight =
          pixelsPerMinute * getMinutesIntoDay(eventDetails.to) - minutesTop;
      } else {
    ...
    if (
      event.offsetTop + event.offsetHeight >
      contentHeight - _options.spacing
    ) {
      var newHeight =
        contentHeight - event.offsetTop - _options.spacing * 3 + "px";
      event.style.height = newHeight;
      event.style.maxHeight = newHeight;
    } else {
      event.style.maxHeight =
        contentHeight -
        (event.offsetTop + _options.spacing * 2) -
        pixelsPerMinute +
        "px";
    }
  ...
  function setEventPositionAndGetScrollLeft(
    contents,
    displayDate,
    event,
    eventDetails
    ...
    if (
      ...
      minutesWidth =
        pixelsPerMinute * getMinutesIntoDay(eventDetails.to) - minutesLeft;
    } else {
    ...
    if (
      event.offsetLeft + event.offsetWidth >
      contentWidth - _options.spacing
    ) {
      var newWidth =
        contentWidth - event.offsetLeft - _options.spacing * 4 + "px";
      event.style.width = newWidth;
      event.style.maxWidth = newWidth;
    } else {
      event.style.maxWidth =
        contentWidth -
        (event.offsetLeft + _options.spacing * 2) -
        pixelsPerMinute +
        "px";
    }
    return minutesLeft - _options.spacing;
  }
  function updateViewTimeArrowPosition(
    date,
    elementView,
    elementTimeArrow,
    container,
    ...
    if (elementTimeArrow !== null) {
      if (isTimeArrowVisible(date, elementView, setting)) {
        ...
        elementTimeArrow.style.top =
          top - elementTimeArrow.offsetHeight / 2 + "px";
        topPosition = top;
      } else {
      ...
      for (var eventIndex1 = 0; eventIndex1 < eventsLength; eventIndex1++) {
        ...
        for (var eventIndex2 = 0; eventIndex2 < eventsLength; eventIndex2++) {
          if (eventIndex2 !== eventIndex1) {
            ...
            if (overlaps) {
              var event1Position = getString(
                  event1.getAttribute("event-position")
                ),
                event2Position = getString(
                  event2.getAttribute("event-position")
                );
              if (
                event1Position === _string.empty &&
                event2Position === _string.empty
              ) {
  ...
  function setOverlappingEventWidth(event) {
    event.style.width =
      event.offsetWidth / 2 -
      (_options.spacing * 3 + _options.spacing / 4) +
      "px";
  }
  function setOverlappingEventLeft(event1, event2) {
    event1.style.left =
      event2.offsetLeft + event2.offsetWidth + _options.spacing + "px";
  }
  ...
  function doEventElementsOverlap(element1, element2) {
    ...
    if (
      topPlusHeight1 < offsetTop2 ||
      offsetTop1 > topPlusHeight2 ||
      leftPlusWidth1 < offsetLeft2 ||
      offsetLeft1 > leftPlusWidth2
    ) {
      result = false;
    }
    return result;
  ...
  function onViewEventDropped(e, dateDropped, container) {
    ...
    if (_element_View_Event_Dragged === null) {
      if (e.dataTransfer.files.length === 0) {
        dropEventsFromOtherCalendar(
          e,
          dateDropped.getFullYear(),
          dateDropped.getMonth(),
          dateDropped.getDate()
        );
      } else {
        dropFileOnDisplay(e);
      ...
      var pixelsPerMinute = getPixelsPerMinuteForHeight(container),
        ...
        top =
          _parameter_Math.abs(e.pageY) -
          offset.top +
          _element_View_Event_Dragged_ClickOffset,
        difference = top - _element_View_Event_Dragged_OffsetTop,
        differenceMinutes = difference / pixelsPerMinute;
      _element_View_Event_Dragged_EventDetails.from = addMinutesToDate(
        _element_View_Event_Dragged_EventDetails.from,
        differenceMinutes
      );
      _element_View_Event_Dragged_EventDetails.to = addMinutesToDate(
        _element_View_Event_Dragged_EventDetails.to,
        differenceMinutes
      );
      storeEventsInLocalStorage();
      fireCustomTrigger(
        _options.events.onEventUpdated,
        _element_View_Event_Dragged_EventDetails
      );
      showNotificationPopUp(
        _options.eventUpdatedText.replace(
          "{0}",
          _element_View_Event_Dragged_EventDetails.title
        )
      );
      refreshOpenedViews();
      _element_View_Event_Dragged = null;
    ...
    if (_options.manualEditingEnabled) {
      ...
      if (container !== null && eventsLength > 0) {
        ...
        for (var eventIndex = 0; eventIndex < eventsLength; eventIndex++) {
          ...
          if (
            eventSizeDetails.height !==
            eventSizeDetails.eventElement.offsetHeight
          ) {
            var difference =
                eventSizeDetails.eventElement.offsetHeight -
                eventSizeDetails.height,
            ...
            eventSizeDetails.height =
              eventSizeDetails.eventElement.offsetHeight;
            eventSizeDetails.eventDetails.to = addMinutesToDate(
              eventSizeDetails.eventDetails.to,
              differenceMinutes
            );
            eventsResized = true;
            fireCustomTrigger(
              _options.events.onEventUpdated,
              eventSizeDetails.eventDetails
            );
            showNotificationPopUp(
              _options.eventUpdatedText.replace(
                "{0}",
                eventSizeDetails.eventDetails.title
              )
            );
          }
        }
  ...
  function turnOffFullScreenMode() {
    if (_element_Calendar_FullScreenModeOn && _options.fullScreenModeEnabled) {
      _element_Calendar_FullScreenModeOn = false;
      _element_Calendar.className = _element_Calendar.className.replace(
        " full-screen-view",
        _string.empty
      );
      _element_Calendar.style.cssText =
        _element_Calendar_FullScreenModeCachedStyled;
      updateFullScreenModeExpandButtons(
        "ib-arrow-expand-left-right",
        _options.enableFullScreenTooltipText
      );
      refreshOpenedViews();
  ...
  function forceTurnOnFullScreenMode() {
    ...
    updateFullScreenModeExpandButtons(
      ...
      _options.disableFullScreenTooltipText
    );
    refreshOpenedViews();
  }
  function updateFullScreenModeExpandButtons(className, tooltipText) {
    setElementClassName(
      _element_View_FullMonth_TitleBar_FullScreenButton,
      className
    );
  ...
  function getHolidaysText(date) {
    ...
    if (_options.showHolidays) {
      ...
      for (
        var holidayIndex = 0;
        holidayIndex < holidaysLength;
        holidayIndex++
      ) {
        var holiday = _options.holidays[holidayIndex],
          holidayText = getString(holiday.title, _string.empty);
        if (
          isHolidayDateValidForDate(holiday, date) &&
          holidayText !== _string.empty &&
          holidayTextItemsAnyCase.indexOf(holidayText.toLowerCase())
        ) {
  ...
  function addHolidays(date, dayMutedClass, dayElement) {
    if (_options.showHolidays) {
      ...
      for (
        ...
        var holiday = _options.holidays[holidayIndex],
          holidayText = getString(holiday.title, _string.empty),
          holidayBackgroundColor = getString(
  ...
  function addHolidayText(holiday, dayElement, holidayText, dayMutedClass) {
    var className =
        isDefinedFunction(holiday.onClick) ||
        isDefinedString(holiday.onClickUrl)
          ? "holiday-link"
          : "holiday",
      ...
      if (_events.hasOwnProperty(storageDate)) {
        for (var storageGuid in _events[storageDate]) {
          if (_events[storageDate].hasOwnProperty(storageGuid)) {
            var event = getAdjustedAllDayEvent(
                _events[storageDate][storageGuid]
              ),
  ...
  function removeNonRepeatingEventsOnSpecificDate(date, compareFunc) {
    ...
    var onYesEvent = function () {
      ...
      getAllEventsFunc(function (eventDetails) {
        ...
        if (
          repeatEvery === _enum_RepeatType.never &&
          compareFunc(eventDetails.from, date)
        ) {
      ...
      storeEventsInLocalStorage();
      showNotificationPopUp(
        _options.eventsRemovedText.replace("{0}", eventsRemoved)
      );
      refreshViews();
    };
    showMessageDialog(
      _options.confirmEventsRemoveTitle,
      _options.confirmEventsRemoveMessage,
      onYesEvent,
      onNoEvent
    );
  }
  function getAdjustedAllDayEvent(eventDetails) {
    ...
    if (adjustedEvent.isAllDay) {
      adjustedEvent.from = new Date(
        adjustedEvent.from.getFullYear(),
        adjustedEvent.from.getMonth(),
        adjustedEvent.from.getDate(),
        0,
        0
      );
      adjustedEvent.to = new Date(
        adjustedEvent.from.getFullYear(),
        adjustedEvent.from.getMonth(),
        adjustedEvent.from.getDate(),
        23,
  ...
  function setEventTypeInputCheckedStates(selectedEventType) {
    selectedEventType =
      isDefined(selectedEventType) &&
      _options_EventType.hasOwnProperty(selectedEventType)
        ? selectedEventType
        : 0;
    for (var eventType in _options_EventType) {
      if (
        _options_EventType.hasOwnProperty(eventType) &&
        isDefined(_options_EventType[eventType].eventEditorInput)
      ) {
  ...
  function getEventTypeInputChecked() {
    ...
    for (var eventType in _options_EventType) {
      if (
        _options_EventType.hasOwnProperty(eventType) &&
        isDefined(_options_EventType[eventType].eventEditorInput) &&
        _options_EventType[eventType].eventEditorInput.checked
      ) {
  ...
  function getTotalDaysBetweenDates(from, to) {
    var fromDate = new Date(
      ...
      differenceDays = _parameter_Math.ceil(
  ...
  function getWeekNumber(date) {
    var firstDay = new Date(date.getFullYear(), 0, 1),
      weekNumber = _parameter_Math.ceil(
  ...
  function getFriendlyTimeBetweenTwoDate(date1, date2) {
    ...
    if (days > 0) {
      text.push(
        days.toString() +
          _string.space +
          (days === 1 ? _options.dayText : _options.daysText)
      );
    }
    ...
    if (hours > 0) {
      text.push(
        hours.toString() +
          _string.space +
          (hours === 1 ? _options.hourText : _options.hoursText)
      );
    }
    ...
    if (minutes > 0) {
      text.push(
        minutes.toString() +
          _string.space +
          (minutes === 1 ? _options.minuteText : _options.minutesText)
      );
    }
    return text.join(", ");
  }
  ...
  function getSelectedDate(input, defaultValue) {
    ...
    if (isDefined(result)) {
      result = new Date(
        result.getTime() +
          _parameter_Math.abs(result.getTimezoneOffset() * 60000)
      );
    }
    return result;
  }
  ...
  function getCustomFormattedDateText(dateFormat, date) {
    ...
    result = result.replace(
      "{ddd}",
      _options.dayNamesAbbreviated[weekDayNumber]
    );
  ...
  function getTimeToTimeDisplay(fromDate, toDate) {
    return (
      getTimeForDisplay(fromDate) +
      _string.space +
      _options.toTimeText +
      _string.space +
      getTimeForDisplay(toDate)
    );
  }
  ...
  function buildDateTimeToDateTimeDisplay(container, fromDate, toDate) {
    ...
    createSpanElement(
      container,
      _string.space + _options.toTimeText + _string.space
    );
  ...
  function buildDateTimeDisplay(container, date, addTime, addYear, addDayName) {
    ...
    if (addDayName) {
      createSpanElement(
        container,
        _options.dayNames[getWeekdayNumber(date)] + ", "
      );
    }
    buildDayDisplay(container, date);
    createSpanElement(
      container,
      _string.space + _options.monthNames[date.getMonth()]
    );
    if (addYear) {
  ...
  function makeEventDraggable(event, eventDetails, dragFromDate, container) {
    if (
      ...
      var draggedFromDate = new Date(dragFromDate),
        ...
        dragDisabledClass = !isDateWeekendDay
          ? " drag-not-allowed"
          : " drag-not-allowed-weekend-day";
      event.setAttribute("draggable", true);
      event.ondragstart = function (e) {
        fireCustomTrigger(_options.events.onEventDragStart, eventDetails);
        e.dataTransfer.setData(
          "event_details",
          _parameter_Json.stringify(eventDetails)
        );
      ...
      event.ondragend = function () {
        ...
        if (isDefined(container)) {
          ...
          makeAreaDroppable(
            container,
            draggedFromDate.getFullYear(),
            draggedFromDate.getMonth(),
            draggedFromDate.getDate()
          );
        }
        updateContainerClassChildren(
          "cell",
  ...
  function showDraggingEffect(e, dayElement, areaDate) {
    ...
    if (
      _events_Dragged !== null &&
      dayElement.className.indexOf(" drag-over") === _value.notFound &&
      !doDatesMatch(_events_Dragged_DateFrom, areaDate)
    ) {
  ...
  function hideDraggingEffect(e, dayElement, areaDate) {
    ...
    if (
      ...
      dayElement.className = dayElement.className.replace(
        " drag-over",
        _string.empty
      );
    }
  ...
  function dropEventOnDay(e, year, month, day) {
    ...
    if (
      _events_Dragged !== null &&
      !doDatesMatch(_events_Dragged_DateFrom, dropDate)
    ) {
      fireCustomTrigger(
        _options.events.onEventDragDrop,
        _events_Dragged,
        dropDate
      );
      if (!isDefined(day)) {
      ...
      var daysBetweenDraggedFromAndFrom = getTotalDaysBetweenDates(
          _events_Dragged.from,
          _events_Dragged_DateFrom
        ),
        daysBetweenFromAndTo = getTotalDaysBetweenDates(
          _events_Dragged.from,
          _events_Dragged.to
        ),
        fromDate = new Date(
          ...
          month,
          day,
          _events_Dragged.from.getHours(),
          _events_Dragged.from.getMinutes()
        ),
        toDate = new Date(
          year,
          month,
          day,
          _events_Dragged.to.getHours(),
          _events_Dragged.to.getMinutes()
        ),
      ...
      if (isDefined(repeatEndsDate)) {
        var newFromDaysDifference = getTotalDaysBetweenDates(
          fromDate,
          _events_Dragged.from
        );
        if (fromDate > _events_Dragged.from) {
          repeatEndsDate.setDate(
            repeatEndsDate.getDate() + newFromDaysDifference
          );
        } else {
          repeatEndsDate.setDate(
            repeatEndsDate.getDate() - newFromDaysDifference
      ...
      showNotificationPopUp(
        _options.eventUpdatedText.replace("{0}", _events_Dragged.title)
      );
      refreshViews();
    } else {
  ...
  function dropEventsFromOtherCalendar(e, year, month, day) {
    var eventDetails = getObjectFromString(
      e.dataTransfer.getData("event_details")
    );
    if (isDefined(eventDetails) && _events_Dragged === null) {
      ...
      eventDetails.from = new Date(
        year,
        month,
        day,
        sourceFromDate.getHours(),
        sourceFromDate.getMinutes(),
        0,
        0
      );
      eventDetails.to = new Date(
        year,
        month,
        day,
        sourceToDate.getHours(),
        sourceToDate.getMinutes(),
        0,
        0
      );
      _that.addEvent(eventDetails);
      showNotificationPopUp(
        _options.eventAddedText.replace("{0}", eventDetails.title)
      );
    }
  }
  function dropFileOnDisplay(e) {
    if (
      isDefined(_parameter_Window.FileReader) &&
      _options.importEventsEnabled
    ) {
  ...
  function buildTab(container, text, onClickEvent, selected) {
    ...
    var className =
        "tab tab-control" + (selected ? "-selected" : _string.empty),
  ...
  function buildTabContents(container, selected, canScroll) {
    ...
    var tabContainer = createElement(
      "div",
      "checkbox-container tab-content custom-scroll-bars"
    );
  ...
  function showTabContents(tab, contents, container) {
    ...
    for (
      ...
      allContentsIndex < allContentsLength;
      allContentsIndex++
    ) {
  ...
  function setEventClassesAndColors(
    event,
    eventDetails,
    toDate,
    setNotInMonthCss
    ...
    if (
      setNotInMonthCss &&
      isDefined(toDate) &&
      toDate < _calendar_CurrentDate &&
      (toDate.getFullYear() !== _calendar_CurrentDate.getFullYear() ||
        toDate.getMonth() !== _calendar_CurrentDate.getMonth())
    ) {
  ...
  function checkEventForBrowserNotifications(date, eventDetails) {
    if (isDateToday(date) && !_element_Mode_DatePicker_Enabled) {
      var newFrom = new Date(),
        ...
        repeatEvery = getNumber(eventDetails.repeatEvery);
      newFrom.setHours(
        eventDetails.from.getHours(),
        eventDetails.from.getMinutes(),
        0,
        0
      );
      newTo.setHours(
        eventDetails.to.getHours(),
        eventDetails.to.getMinutes(),
        0,
        0
      );
      if (
        repeatEvery === _enum_RepeatType.never &&
        !isDateToday(eventDetails.from)
      ) {
        newFrom.setHours(0, 0, 0, 0);
      }
      if (
        repeatEvery === _enum_RepeatType.never &&
        !isDateToday(eventDetails.to)
      ) {
        newTo.setHours(23, 59, 59, 99);
      }
      if (
        isDefinedNumber(eventDetails.alertOffset) &&
        eventDetails.alertOffset > 0
      ) {
        newFrom = addMinutesToDate(newFrom, -eventDetails.alertOffset);
      }
      if (today >= newFrom && today <= newTo) {
        if (
          !isDefinedBoolean(eventDetails.showAsBusy) ||
          eventDetails.showAsBusy
        ) {
          _calendar_IsBusy = true;
        }
        if (
          (!_events_NotificationsTriggered.hasOwnProperty(eventDetails.id) &&
            !isDefinedBoolean(eventDetails.showAlerts)) ||
          eventDetails.showAlerts
        ) {
          runBrowserNotificationAction(
            function () {
  ...
  function setCopiedEventsClasses(clear) {
    ...
    for (
      ...
      copiedEventDetailsIndex < copiedEventDetailsLength;
      copiedEventDetailsIndex++
    ) {
  ...
  function setCopiedEvents(eventDetails) {
    ...
    if (selectedEventsLength > 0) {
      for (
        ...
        selectedEventIndex < selectedEventsLength;
        selectedEventIndex++
      ) {
  ...
  function isEventIdSelected(id) {
    ...
    for (
      ...
      eventsSelectedIndex < eventsSelectedLength;
      eventsSelectedIndex++
    ) {
  ...
  function pasteEventsToDate(date, cut) {
    ...
    for (
      ...
      var eventDetails = _events_Copied[copiedEventDetailsIndex],
        totalDays = getTotalDaysBetweenDates(
          eventDetails.from,
          eventDetails.to
        );
    ...
    showNotificationPopUp(
      _options.eventsPastedText.replace("{0}", copiedEventDetailsLength)
    );
    if (cut) {
      clearSelectedEvents();
      _events_Copied = [];
  ...
  function clearSelectedEvents() {
    ...
    if (eventsSelectedLength > 0) {
      for (
        ...
        updateEventClasses(
          _events_Selected[eventsSelectedIndex].id,
          "selected-event",
          true
        );
      }
  ...
  function getGroups() {
    ...
    getAllEventsFunc(function (eventDetails) {
      ...
      if (
        group !== _string.empty &&
        groupsAnyCase.indexOf(group.toLowerCase()) === _value.notFound
      ) {
        groups.push(group);
  ...
  function createElement(type, className, inputType, inputId) {
    ...
    if (!_elements_Type.hasOwnProperty(nodeType)) {
      _elements_Type[nodeType] = isText
        ? _parameter_Document.createTextNode(_string.empty)
        : _parameter_Document.createElement(nodeType);
    }
  ...
  function getScrollPosition() {
    var doc = _parameter_Document.documentElement,
      left =
        (_parameter_Window.pageXOffset || doc.scrollLeft) -
        (doc.clientLeft || 0),
      top =
        (_parameter_Window.pageYOffset || doc.scrollTop) - (doc.clientTop || 0);
    return { left: left, top: top };
  }
  function removeLastCloseWindowEvent(popCloseWindowEvent) {
    popCloseWindowEvent = isDefined(popCloseWindowEvent)
      ? popCloseWindowEvent
      : true;
  ...
  function getStyleValueByName(element, stylePropertyName, toFloat) {
    ...
    if (_parameter_Window.getComputedStyle) {
      value = _parameter_Document.defaultView
  ...
  function updateContainerClassChildren(containerClass, func, ignoreElement) {
    ...
    for (var elementIndex = 0; elementIndex < elementsLength; elementIndex++) {
      ...
      for (
        ...
        elementChildrenIndex < elementChildrenLength;
  ...
  function removeTrackedElementsFromDocument() {
    ...
    for (
      ...
      _parameter_Document.body.removeChild(
        _elements_InDocumentBody[elementsIndex]
      );
    }
    _elements_InDocumentBody = [];
  }
  ...
  function removeElementsClassName(container, className) {
    var elements = container.getElementsByClassName(className);
    while (elements[0]) {
      elements[0].className = elements[0].className.replace(
        className,
        _string.empty
      );
    }
  ...
  function buildCheckBox(
    ...
    extraClassName = isDefined(extraClassName)
      ? _string.space + extraClassName
      : _string.empty;
  ...
  function newGuid() {
    ...
    for (var charIndex = 0; charIndex < 32; charIndex++) {
      ...
      var character = _parameter_Math
        .floor(_parameter_Math.random() * 16)
        .toString(16);
  ...
  function startsWithAnyCase(data, start) {
    return (
      data.substring(0, start.length).toLowerCase() === start.toLowerCase()
    );
  }
  ...
  function getShortUrlString(url, maxLength) {
    ...
    if (url.length > maxLength) {
      ...
      result =
        url.substring(0, sideLength) +
        "..." +
        url.substring(url.length - sideLength);
    }
    return result;
  }
  ...
  function areArraysTheSame(array1, array2) {
    ...
    if (result) {
      ...
      array2.sort();
      result =
        _parameter_Json.stringify(array1) === _parameter_Json.stringify(array2);
    }
    return result;
  }
  ...
  function storeEventsInLocalStorage() {
    if (_options.useLocalStorageForEvents && _parameter_Window.localStorage) {
      ...
      for (
        ...
        _parameter_Window.localStorage.setItem(
          "CJS_" + orderedEventIndex.toString(),
          orderedEventsJson
        );
      }
    }
  ...
  function loadEventsFromLocalStorage() {
    if (_options.useLocalStorageForEvents && _parameter_Window.localStorage) {
      ...
      for (var keyIndex = 0; keyIndex < keysLength; keyIndex++) {
        var eventJson = _parameter_Window.localStorage.getItem(
            _parameter_Window.localStorage.key(keyIndex)
          ),
  ...
  function clearLocalStorageObjects() {
    if (_options.useLocalStorageForEvents && _parameter_Window.localStorage) {
      ...
      for (
        ...
        keyToRemoveIndex < keysToRemoveLength;
        keyToRemoveIndex++
      ) {
        _parameter_Window.localStorage.removeItem(
          keysToRemove[keyToRemoveIndex]
        );
      }
    }
  }
  ...
  function importEventsFromJson(file, onLoadEnd) {
    ...
    reader.onload = function (event) {
      ...
      if (
        isDefinedObject(readingEvents) &&
        readingEvents.hasOwnProperty("events")
      ) {
      ...
      for (
        ...
        readingEventsIndex < readingEventsLength;
        readingEventsIndex++
      ) {
  ...
  function importEventsFromICal(file, onLoadEnd) {
    ...
    reader.onload = function (event) {
      ...
      if (
        contentLines[0].indexOf("BEGIN:VCALENDAR") > _value.notFound &&
        contentLines[contentLinesLength - 1].indexOf("END:VCALENDAR") >
          _value.notFound
      ) {
        ...
        for (
          ...
          contentLineIndex < contentLinesLength;
          contentLineIndex++
        ) {
          ...
          if (contentLine.indexOf("BEGIN:VEVENT") > _value.notFound) {
            ...
            var eventDetails = _parameter_Json.parse(
              _parameter_Json.stringify(readingEventDetails)
            );
            readingEvent = false;
            readingEventDetails = {};
            _that.removeEvent(eventDetails.id, false, false);
          ...
          if (readingEvent) {
            if (startsWith(contentLine, "UID:")) {
              ...
              startsWith(contentLine, "DTSTART:") ||
              startsWith(contentLine, "DTSTART;")
            ) {
              readingEventDetails.from = importICalDateTime(
                contentLine.split(":").pop()
              );
              readingEventDetails.isAllDay =
                contentLine.split(":").pop().length === 8;
            } else if (
              startsWith(contentLine, "DTEND:") ||
              startsWith(contentLine, "DTEND;")
            ) {
              readingEventDetails.to = importICalDateTime(
                contentLine.split(":").pop(),
                true
              );
            } else if (startsWith(contentLine, "CREATED:")) {
              readingEventDetails.created = importICalDateTime(
                contentLine.split(":").pop()
              );
              ...
              readingEventDetails.showAsBusy =
                contentLine.split(":").pop() === "OPAQUE";
  ...
  function importICalOrganizer(readingEventDetails, contentLine) {
    ...
    readingEventDetails.organizerName = organizerDetailsParts[0].replace(
      "CN=",
      _string.empty
    );
  ...
  function importICalRRule(readingEventDetails, contentLine) {
    ...
    for (
      ...
      rRuleDetailsPartsIndex < rRuleDetailsPartsLength;
      rRuleDetailsPartsIndex++
    ) {
    ...
    if (isDefined(freq)) {
      ...
      if (isDefined(readingEventDetails.repeatEveryCustomValue)) {
        if (freq === "DAILY") {
          readingEventDetails.repeatEveryCustomType =
            _enum_RepeatCustomType.daily;
        } else if (freq === "WEEKLY") {
          readingEventDetails.repeatEveryCustomType =
            _enum_RepeatCustomType.weekly;
        } else if (freq === "MONTHLY") {
          readingEventDetails.repeatEveryCustomType =
            _enum_RepeatCustomType.monthly;
        } else if (freq === "YEARLY") {
          readingEventDetails.repeatEveryCustomType =
            _enum_RepeatCustomType.yearly;
        }
      } else {
        if (freq === "DAILY") {
  ...
  function importFromFilesCompleted(eventsAddedOrUpdated) {
    if (eventsAddedOrUpdated.length > 0) {
      ...
      refreshOpenedViews();
      showNotificationPopUp(
        _options.eventsImportedText.replace("{0}", eventsAddedOrUpdated.length)
      );
  ...
  function exportEvents(events, type, filename, copyToClipboard) {
    ...
    if (contents !== _string.empty) {
      if (copyToClipboard) {
        ...
        filename = isDefined(filename)
          ? filename
          : getExportDownloadFilename(extension);
        ...
        showNotificationPopUp(
          _options.eventsExportedToText.replace("{0}", filename)
        );
      }
  ...
  function getExportDownloadFilename(extension, filenameStart) {
    filenameStart = isDefined(filenameStart)
      ? filenameStart
      : _options.exportStartFilename;
    var date = new Date(),
      datePart =
        padNumber(date.getDate()) +
        "-" +
  ...
  function getStringFromDateTime(eventDate) {
    ...
    if (isDefined(eventDate)) {
      var date =
          padNumber(eventDate.getDate()) +
          "/" +
          padNumber(eventDate.getMonth() + 1) +
          "/" +
          eventDate.getFullYear(),
        time =
          padNumber(eventDate.getHours()) +
          ":" +
          padNumber(eventDate.getMinutes());
  ...
  function getArrayText(value, includeSpeechMarks) {
    includeSpeechMarks = isDefined(includeSpeechMarks)
      ? includeSpeechMarks
      : false;
  ...
  function getExportRow(eventDetails) {
    ...
    eventContents.push(
      getYesNoFromBoolean(
        !isDefinedBoolean(eventDetails.showAlerts) || eventDetails.showAlerts
      )
    );
    eventContents.push(
      getYesNoFromBoolean(
        !isDefinedBoolean(eventDetails.showAsBusy) || eventDetails.showAsBusy
      )
    );
  ...
  function getExportDateTime() {
    var dateExported = new Date(),
      dateExportedMeta = getCustomFormattedDateText(
        "{ddd}, {dd} {mmm} {yyyy}",
        dateExported
      );
    dateExportedMeta +=
      " " +
      padNumber(dateExported.getHours()) +
      ":" +
      padNumber(dateExported.getMinutes()) +
      ":" +
      padNumber(dateExported.getSeconds());
    return dateExportedMeta;
  }
  function exportEventsForSharing(events) {
    var fileContents = getJsonContents(events),
      fileMimeType = { type: "text/plain" },
      fileBlob = new Blob([fileContents], fileMimeType),
      file = new File(
        [fileBlob],
        getExportDownloadFilename("txt", _options.shareStartFilename),
        fileMimeType
      ),
      fileShareData = { files: [file] };
  ...
  function getCsvContents(orderedEvents) {
    ...
    for (
      var orderedEventIndex = 0;
      orderedEventIndex < orderedEventLength;
      orderedEventIndex++
    ) {
  ...
  function storeCsvData(csvContents, eventDetails) {
    ...
    for (
      ...
      eventContentsIndex < eventContentsLength;
      eventContentsIndex++
    ) {
      eventContents[eventContentsIndex] = getCsvValue(
        eventContents[eventContentsIndex]
      );
    }
  ...
  function getXmlContents(orderedEvents, filename) {
    ...
    for (
      ...
      for (
        ...
        propertyNameIndex < propertyNamesLength;
        propertyNameIndex++
      ) {
        var propertyName = propertyNames[propertyNameIndex];
        if (propertyName !== "customTags") {
          if (
            orderedEvent.hasOwnProperty(propertyName) &&
            orderedEvent[propertyName] !== null
          ) {
            ...
            contents.push(
              "<" +
                newPropertyName +
                ">" +
                getPropertyValue(propertyName, orderedEvent[propertyName]) +
                "</" +
                newPropertyName +
                ">"
            );
  ...
  function getJsonContents(orderedEvents) {
    ...
    for (
      ...
      contents[contents.length - 1] = lastJsonEntry.substring(
        0,
        lastJsonEntry.length - 1
      );
      contents.push("},");
    }
  ...
  function getICalContents(orderedEvents) {
    ...
    contents.push(
      "PRODID:-//Bunoon//Calendar.js v" + _that.getVersion() + "//EN"
    );
    ...
    for (
      ...
      if (
        !isDefinedBoolean(orderedEvent.showAsBusy) ||
        orderedEvent.showAsBusy
      ) {
      ...
      if (isDefinedDate(orderedEvent.lastUpdated)) {
        contents.push(
          "LAST-MODIFIED:" + getICalDateTimeString(orderedEvent.lastUpdated)
        );
      }
      if (
        isDefinedString(organizerName) &&
        isDefinedString(organizerEmailAddress)
      ) {
        contents.push(
      ...
      if (repeatEvery !== _enum_RepeatType.never) {
        contents.push(
          "RRULE:" + getICalRRuleForEvent(orderedEvent, repeatEvery)
        );
      }
      ...
      if (isDefinedString(orderedEvent.description)) {
        contents.push(
          "DESCRIPTION:" + getICalSingleLine(orderedEvent.description)
        );
      }
      ...
      if (
        !isDefinedBoolean(orderedEvent.showAlerts) ||
        orderedEvent.showAlerts
      ) {
        contents.push("BEGIN:VALARM");
        contents.push(
          "TRIGGER;VALUE=DATE-TIME:" + getICalDateTimeString(orderedEvent.from)
        );
  ...
  function getICalRRuleForEvent(orderedEvent, repeatEvery) {
    ...
    if (repeatEvery === _enum_RepeatType.custom) {
      ...
      if (repeatEvery === _enum_RepeatType.everyDay) {
        ...
        repeatEvery === _enum_RepeatType.everyWeek ||
        repeatEvery === _enum_RepeatType.every2Weeks
      ) {
        contents.push("FREQ=WEEKLY");
      } else if (repeatEvery === _enum_RepeatType.everyMonth) {
        contents.push("FREQ=MONTHLY");
  ...
  function storeTsvData(csvContents, eventDetails) {
    ...
    for (
      ...
      eventContents[eventContentsIndex] = getTsvValue(
        eventContents[eventContentsIndex]
      );
    }
  ...
  _that.setCurrentDisplayDate = function (date) {
    if (
      isDefinedDate(date) &&
      (!_element_Mode_DatePicker_Enabled || _element_Mode_DatePicker_Visible)
    ) {
      var newDate = new Date(date);
      if (!doDatesMatch(_calendar_CurrentDate, newDate)) {
        if (
          newDate.getFullYear() >= _options.minimumYear &&
          newDate.getFullYear() <= _options.maximumYear
        ) {
          build(newDate);
  ...
  _that.getSelectedDatePickerDate = function () {
    return _element_Mode_DatePicker_Enabled
      ? new Date(_calendar_CurrentDate_ForDatePicker)
      : null;
  };
  _that.setSelectedDatePickerDate = function (date) {
    if (isDefinedDate(date) && _element_Mode_DatePicker_Enabled) {
      ...
      if (
        newDateAllowed &&
        !doDatesMatch(newDate, _calendar_CurrentDate_ForDatePicker)
      ) {
        if (
          newDate.getFullYear() >= _options.minimumYear &&
          newDate.getFullYear() <= _options.maximumYear
  ...
  _that.setEventsFromJson = function (json, updateEvents, triggerEvent) {
    if (!_element_Mode_DatePicker_Enabled) {
      ...
      if (isDefinedArray(dataObject)) {
        ...
        isDefinedObject(dataObject) &&
        dataObject.hasOwnProperty("events")
      ) {
    ...
    if (!_element_Mode_DatePicker_Enabled) {
      ...
      if (event.from <= event.to) {
        ...
        if (!_events[storageDate].hasOwnProperty(storageGuid)) {
          ...
          if (
            isDefined(_options_Configuration.visibleGroups) &&
            !isDefinedFunction(_options.events.onEventsFetch)
          ) {
            var groupName = getGroupName(group);
            if (
              _options_Configuration.visibleGroups.indexOf(groupName) ===
              _value.notFound
            ) {
          ...
          if (
            ...
            title !== _string.empty &&
            title.length > _options.maximumEventTitleLength
          ) {
            event.title = event.title.substring(
              0,
              _options.maximumEventTitleLength
            );
          }
          if (
            ...
            description !== _string.empty &&
            description.length > _options.maximumEventDescriptionLength
          ) {
          ...
          if (
            ...
            location !== _string.empty &&
            location.length > _options.maximumEventLocationLength
          ) {
            event.location = event.location.substring(
              0,
              _options.maximumEventLocationLength
            );
          }
          if (
            ...
            group !== _string.empty &&
            group.length > _options.maximumEventGroupLength
          ) {
            event.group = event.group.substring(
              0,
              _options.maximumEventGroupLength
            );
          }
  ...
  _that.updateEventDateTimes = function (
    id,
    from,
    to,
    repeatEnds,
  ...
  _that.setVisibleEventTypes = function (ids, triggerEvent) {
    if (isDefinedArray(ids) && !_element_Mode_DatePicker_Enabled) {
      ...
      for (var idIndex = 0; idIndex < idsLength; idIndex++) {
        if (
          _options_Configuration.visibleEventTypes.indexOf(ids[idIndex]) ===
          _value.notFound
        ) {
  ...
  _that.removeGroup = function (groupName, updateEvents, triggerEvent) {
    if (isDefinedString(groupName) && !_element_Mode_DatePicker_Enabled) {
      ...
      getAllEventsFunc(function (eventDetails) {
        if (
          eventDetails.group !== null &&
          eventDetails.group.toLowerCase() === checkGroupName
        ) {
          eventDetails.group = null;
        }
      });
  ...
  _that.setVisibleGroups = function (groupNames, triggerEvent) {
    if (isDefinedArray(groupNames) && !_element_Mode_DatePicker_Enabled) {
      ...
      for (
        ...
        groupNameIndex < groupNamesLength;
        groupNameIndex++
      ) {
  ...
  _that.setOptions = function (newOptions, triggerEvent) {
    ...
    if (_initialized) {
      ...
      if (
        !_element_Mode_DatePicker_Enabled ||
        _element_Mode_DatePicker_Visible
      ) {
        build(_calendar_CurrentDate, true, true);
      }
    }
  ...
  function buildDefaultOptions(newOptions) {
    ...
    _options.showDayNumberOrdinals = getDefaultBoolean(
      _options.showDayNumberOrdinals,
      true
    );
    _options.dragAndDropForEventsEnabled = getDefaultBoolean(
      _options.dragAndDropForEventsEnabled,
      true
    );
    _options.exportEventsEnabled = getDefaultBoolean(
      _options.exportEventsEnabled,
      true
    );
    _options.manualEditingEnabled = getDefaultBoolean(
      _options.manualEditingEnabled,
      true
    );
    _options.autoRefreshTimerDelay = getDefaultNumber(
      _options.autoRefreshTimerDelay,
      30000
    );
    _options.fullScreenModeEnabled = getDefaultBoolean(
      _options.fullScreenModeEnabled,
      true
    );
    _options.tooltipDelay = getDefaultNumber(_options.tooltipDelay, 1000);
    _options.holidays = getDefaultArray(
      _options.holidays,
      getStandardHolidays()
    );
    _options.organizerName = getDefaultString(
      _options.organizerName,
      _string.empty
    );
    _options.organizerEmailAddress = getDefaultString(
      _options.organizerEmailAddress,
      _string.empty
    );
    _options.spacing = getDefaultNumber(_options.spacing, 10);
    _options.maximumEventTitleLength = getDefaultNumber(
      _options.maximumEventTitleLength,
      0
    );
    _options.maximumEventDescriptionLength = getDefaultNumber(
      _options.maximumEventDescriptionLength,
      0
    );
    _options.maximumEventLocationLength = getDefaultNumber(
      _options.maximumEventLocationLength,
      0
    );
    _options.maximumEventGroupLength = getDefaultNumber(
      _options.maximumEventGroupLength,
      0
    );
    _options.eventNotificationsEnabled = getDefaultBoolean(
      _options.eventNotificationsEnabled,
      false
    );
    _options.tooltipsEnabled = getDefaultBoolean(
      _options.tooltipsEnabled,
      true
    );
    _options.urlWindowTarget = getDefaultString(
      _options.urlWindowTarget,
      "_blank"
    );
    _options.defaultEventBackgroundColor = getDefaultString(
      _options.defaultEventBackgroundColor,
      "#484848"
    );
    _options.defaultEventTextColor = getDefaultString(
      _options.defaultEventTextColor,
      "#F5F5F5"
    );
    _options.defaultEventBorderColor = getDefaultString(
      _options.defaultEventBorderColor,
      "#282828"
    );
    _options.hideEventsWithoutGroupAssigned = getDefaultBoolean(
      _options.hideEventsWithoutGroupAssigned,
      false
    );
    _options.showHolidays = getDefaultBoolean(_options.showHolidays, true);
    _options.useTemplateWhenAddingNewEvent = getDefaultBoolean(
      _options.useTemplateWhenAddingNewEvent,
      true
    );
    _options.useEscapeKeyToExitFullScreenMode = getDefaultBoolean(
      _options.useEscapeKeyToExitFullScreenMode,
      true
    );
    _options.allowHtmlInDisplay = getDefaultBoolean(
      _options.allowHtmlInDisplay,
      false
    );
    ...
    _options.configurationDialogEnabled = getDefaultBoolean(
      _options.configurationDialogEnabled,
      true
    );
    _options.popUpNotificationsEnabled = getDefaultBoolean(
      _options.popUpNotificationsEnabled,
      true
    );
    _options.startOfWeekDay = getDefaultNumber(
      _options.startOfWeekDay,
      _enum_Day.monday
    );
    _options.useLocalStorageForEvents = getDefaultBoolean(
      _options.useLocalStorageForEvents,
      false
    );
    _options.shortcutKeysEnabled = getDefaultBoolean(
      _options.shortcutKeysEnabled,
      true
    );
    _options.reverseOrderDaysOfWeek = getDefaultBoolean(
      _options.reverseOrderDaysOfWeek,
      false
    );
    _options.importEventsEnabled = getDefaultBoolean(
      _options.importEventsEnabled,
      true
    );
    _options.useAmPmForTimeDisplays = getDefaultBoolean(
      _options.useAmPmForTimeDisplays,
      false
    );
    _options.isWidget = getDefaultBoolean(_options.isWidget, false);
    _options.viewToOpenOnFirstLoad = getDefaultString(
      _options.viewToOpenOnFirstLoad,
      null
    );
    _options.eventColorsEditingEnabled = getDefaultBoolean(
      _options.eventColorsEditingEnabled,
      true
    );
    _options.eventTooltipDelay = getDefaultNumber(
      _options.eventTooltipDelay,
      1000
    );
    _options.jumpToDateEnabled = getDefaultBoolean(
      _options.jumpToDateEnabled,
      true
    );
  ...
  function buildDefaultSideMenuOptions() {
    ...
    _options.sideMenu.showDays = getDefaultBoolean(
      _options.sideMenu.showDays,
      true
    );
    _options.sideMenu.showGroups = getDefaultBoolean(
      _options.sideMenu.showGroups,
      true
    );
    _options.sideMenu.showEventTypes = getDefaultBoolean(
      _options.sideMenu.showEventTypes,
      true
    );
    _options.sideMenu.showWorkingDays = getDefaultBoolean(
      _options.sideMenu.showWorkingDays,
      true
    );
    _options.sideMenu.showWeekendDays = getDefaultBoolean(
      _options.sideMenu.showWeekendDays,
      true
    );
  }
  function buildDefaultViewOptionsForFullDay() {
    ...
    _options.views.fullDay.enabled = getDefaultBoolean(
      _options.views.fullDay.enabled,
      true
    );
    _options.views.fullDay.showAllDayEventDetails = getDefaultBoolean(
      _options.views.fullDay.showAllDayEventDetails,
      false
    );
    _options.views.fullDay.minutesBetweenSections = getDefaultNumber(
      _options.views.fullDay.minutesBetweenSections,
      30
    );
    _options.views.fullDay.showTimelineArrow = getDefaultBoolean(
      _options.views.fullDay.showTimelineArrow,
      true
    );
    _options.views.fullDay.showExtraTitleBarButtons = getDefaultBoolean(
      _options.views.fullDay.showExtraTitleBarButtons,
      true
    );
  }
  function buildDefaultViewOptionsForFullWeek() {
    ...
    _options.views.fullWeek.enabled = getDefaultBoolean(
      _options.views.fullWeek.enabled,
      true
    );
    _options.views.fullWeek.showAllDayEventDetails = getDefaultBoolean(
      _options.views.fullWeek.showAllDayEventDetails,
      false
    );
    _options.views.fullWeek.showDayNamesHeaders = getDefaultBoolean(
      _options.views.fullWeek.showDayNamesHeaders,
      true
    );
    _options.views.fullWeek.minutesBetweenSections = getDefaultNumber(
      _options.views.fullWeek.minutesBetweenSections,
      30
    );
    _options.views.fullWeek.showTimelineArrow = getDefaultBoolean(
      _options.views.fullWeek.showTimelineArrow,
      true
    );
    _options.views.fullWeek.showWeekNumbersInTitles = getDefaultBoolean(
      _options.views.fullWeek.showWeekNumbersInTitles,
      false
    );
    _options.views.fullWeek.showExtraTitleBarButtons = getDefaultBoolean(
      _options.views.fullWeek.showExtraTitleBarButtons,
      true
    );
    _options.views.fullWeek.showDatesInDayHeaders = getDefaultBoolean(
      _options.views.fullWeek.showDatesInDayHeaders,
      true
    );
  }
  function buildDefaultViewOptionsForFullMonth() {
    ...
    _options.views.fullMonth.maximumEventsPerDayDisplay = getDefaultNumber(
      _options.views.fullMonth.maximumEventsPerDayDisplay,
      3
    );
    _options.views.fullMonth.showTimesInEvents = getDefaultBoolean(
      _options.views.fullMonth.showTimesInEvents,
      false
    );
    _options.views.fullMonth.minimumDayHeight = getDefaultNumber(
      _options.views.fullMonth.minimumDayHeight,
      0
    );
    _options.views.fullMonth.showPreviousNextMonthNames = getDefaultBoolean(
      _options.views.fullMonth.showPreviousNextMonthNames,
      true
    );
    _options.views.fullMonth.useOnlyDotEvents = getDefaultBoolean(
      _options.views.fullMonth.useOnlyDotEvents,
      false
    );
    _options.views.fullMonth.applyCssToEventsNotInCurrentMonth =
    ...
    _options.views.fullMonth.addYearButtons = getDefaultBoolean(
      _options.views.fullMonth.addYearButtons,
      false
    );
    _options.views.fullMonth.titleBarDateFormat = getDefaultString(
      _options.views.fullMonth.titleBarDateFormat,
      "{mmmm} {yyyy}"
    );
    _options.views.fullMonth.showDayNamesHeaders = getDefaultBoolean(
      _options.views.fullMonth.showDayNamesHeaders,
      true
    );
    _options.views.fullMonth.isPinUpViewEnabled = getDefaultBoolean(
      _options.views.fullMonth.isPinUpViewEnabled,
      false
    );
    _options.views.fullMonth.pinUpViewImageUrls = getDefaultArray(
      _options.views.fullMonth.pinUpViewImageUrls,
      []
    );
    _options.views.fullMonth.showMonthButtonsInYearDropDownMenu =
    ...
    _options.views.fullMonth.showExtraTitleBarButtons = getDefaultBoolean(
      _options.views.fullMonth.showExtraTitleBarButtons,
      true
    );
    _options.views.fullMonth.pinUpViewImageCssClasses = getDefaultArray(
      _options.views.fullMonth.pinUpViewImageCssClasses,
      []
    );
  ...
  function buildDefaultViewOptionsForFullYear() {
    ...
    _options.views.fullYear.enabled = getDefaultBoolean(
      _options.views.fullYear.enabled,
      true
    );
    _options.views.fullYear.showExtraTitleBarButtons = getDefaultBoolean(
      _options.views.fullYear.showExtraTitleBarButtons,
      true
    );
  }
  function buildDefaultViewOptionsForTimeline() {
    ...
    _options.views.timeline.enabled = getDefaultBoolean(
      _options.views.timeline.enabled,
      true
    );
    _options.views.timeline.defaultAxis = getDefaultString(
      _options.views.timeline.defaultAxis,
      "group"
    );
    _options.views.timeline.minutesBetweenSections = getDefaultNumber(
      _options.views.timeline.minutesBetweenSections,
      30
    );
    _options.views.timeline.showExtraTitleBarButtons = getDefaultBoolean(
      _options.views.timeline.showExtraTitleBarButtons,
      true
    );
  }
  function buildDefaultViewOptionsForAllEvents() {
    ...
    _options.views.allEvents.enabled = getDefaultBoolean(
      _options.views.allEvents.enabled,
      true
    );
    _options.views.allEvents.showExtraTitleBarButtons = getDefaultBoolean(
      _options.views.allEvents.showExtraTitleBarButtons,
      true
    );
  }
  function buildDefaultViewOptionsForDatePicker() {
    _options.views.datePicker = getOptions(_options.views.datePicker);
    _options.views.datePicker.selectedDateFormat = getDefaultString(
      _options.views.datePicker.selectedDateFormat,
      "{d}{o} {mmmm} {yyyy}"
    );
    _options.views.datePicker.minimumDate = getDefaultDate(
      _options.views.datePicker.minimumDate,
      null
    );
    _options.views.datePicker.maximumDate = getDefaultDate(
      _options.views.datePicker.maximumDate,
      null
    );
  }
  function buildDefaultEventOptions() {
    ...
    _options.events.onPreviousMonth = getDefaultFunction(
      _options.events.onPreviousMonth,
      null
    );
    _options.events.onNextMonth = getDefaultFunction(
      _options.events.onNextMonth,
      null
    );
    _options.events.onPreviousYear = getDefaultFunction(
      _options.events.onPreviousYear,
      null
    );
    _options.events.onNextYear = getDefaultFunction(
      _options.events.onNextYear,
      null
    );
    _options.events.onToday = getDefaultFunction(_options.events.onToday, null);
    _options.events.onEventAdded = getDefaultFunction(
      _options.events.onEventAdded,
      null
    );
    _options.events.onEventUpdated = getDefaultFunction(
      _options.events.onEventUpdated,
      null
    );
    _options.events.onEventRemoved = getDefaultFunction(
      _options.events.onEventRemoved,
      null
    );
    _options.events.onEventsAdded = getDefaultFunction(
      _options.events.onEventsAdded,
      null
    );
    _options.events.onEventsCleared = getDefaultFunction(
      _options.events.onEventsCleared,
      null
    );
    _options.events.onEventsExported = getDefaultFunction(
      _options.events.onEventsExported,
      null
    );
    _options.events.onSetDate = getDefaultFunction(
      _options.events.onSetDate,
      null
    );
    _options.events.onEventsSet = getDefaultFunction(
      _options.events.onEventsSet,
      null
    );
    _options.events.onGroupsCleared = getDefaultFunction(
      _options.events.onGroupsCleared,
      null
    );
    _options.events.onEventsUpdated = getDefaultFunction(
      _options.events.onEventsUpdated,
      null
    );
    _options.events.onOptionsUpdated = getDefaultFunction(
      _options.events.onOptionsUpdated,
      null
    );
    _options.events.onNotificationClicked = getDefaultFunction(
      _options.events.onNotificationClicked,
      null
    );
    _options.events.onSearchOptionsUpdated = getDefaultFunction(
      _options.events.onSearchOptionsUpdated,
      null
    );
    _options.events.onFullScreenModeChanged = getDefaultFunction(
      _options.events.onFullScreenModeChanged,
      null
    );
    _options.events.onEventsSetFromJSON = getDefaultFunction(
      _options.events.onEventsSetFromJSON,
      null
    );
    _options.events.onEventsAddedFromJSON = getDefaultFunction(
      _options.events.onEventsAddedFromJSON,
      null
    );
    _options.events.onDatePickerDateChanged = getDefaultFunction(
      _options.events.onDatePickerDateChanged,
      null
    );
    _options.events.onGroupRemoved = getDefaultFunction(
      _options.events.onGroupRemoved,
      null
    );
    _options.events.onEventUrlClicked = getDefaultFunction(
      _options.events.onEventUrlClicked,
      null
    );
    _options.events.onDestroy = getDefaultFunction(
      _options.events.onDestroy,
      null
    );
    _options.events.onRefresh = getDefaultFunction(
      _options.events.onRefresh,
      null
    );
    _options.events.onDatePickerOpened = getDefaultFunction(
      _options.events.onDatePickerOpened,
      null
    );
    _options.events.onDatePickerClosed = getDefaultFunction(
      _options.events.onDatePickerClosed,
      null
    );
    _options.events.onRender = getDefaultFunction(
      _options.events.onRender,
      null
    );
    _options.events.onEventDragStart = getDefaultFunction(
      _options.events.onEventDragStart,
      null
    );
    _options.events.onEventDragStop = getDefaultFunction(
      _options.events.onEventDragStop,
      null
    );
    _options.events.onEventDragDrop = getDefaultFunction(
      _options.events.onEventDragDrop,
      null
    );
    _options.events.onEventClick = getDefaultFunction(
      _options.events.onEventClick,
      null
    );
    _options.events.onEventDoubleClick = getDefaultFunction(
      _options.events.onEventDoubleClick,
      null
    );
    _options.events.onVisibleGroupsChanged = getDefaultFunction(
      _options.events.onVisibleGroupsChanged,
      null
    );
    _options.events.onVisibleEventTypesChanged = getDefaultFunction(
      _options.events.onVisibleEventTypesChanged,
      null
    );
    _options.events.onNotification = getDefaultFunction(
      _options.events.onNotification,
      null
    );
    _options.events.onBeforeEventAddEdit = getDefaultFunction(
      _options.events.onBeforeEventAddEdit,
      null
    );
    _options.events.onBusyStateChange = getDefaultFunction(
      _options.events.onBusyStateChange,
      null
    );
    _options.events.onEventsFetch = getDefaultFunction(
      _options.events.onEventsFetch,
      null
    );
    _options.events.onEventsImported = getDefaultFunction(
      _options.events.onEventsImported,
      null
    );
    _options.events.onFullDayEventRender = getDefaultFunction(
      _options.events.onFullDayEventRender,
      null
    );
    _options.events.onFullWeekEventRender = getDefaultFunction(
      _options.events.onFullWeekEventRender,
      null
    );
    _options.events.onFullMonthEventRender = getDefaultFunction(
      _options.events.onFullMonthEventRender,
      null
    );
    _options.events.onAllEventsEventRender = getDefaultFunction(
      _options.events.onAllEventsEventRender,
      null
    );
    _options.events.onTimelineEventRender = getDefaultFunction(
      _options.events.onTimelineEventRender,
      null
    );
    _options.events.onWidgetEventRender = getDefaultFunction(
      _options.events.onWidgetEventRender,
      null
    );
    _options.events.onToolTipEventRender = getDefaultFunction(
      _options.events.onToolTipEventRender,
      null
    );
    _options.events.onFullDayTitleRender = getDefaultFunction(
      _options.events.onFullDayTitleRender,
      null
    );
    _options.events.onFullWeekTitleRender = getDefaultFunction(
      _options.events.onFullWeekTitleRender,
      null
    );
    _options.events.onTimelineTitleRender = getDefaultFunction(
      _options.events.onTimelineTitleRender,
      null
    );
    _options.events.onFullMonthPinUpRender = getDefaultFunction(
      _options.events.onFullMonthPinUpRender,
      null
    );
    _options.events.onViewChange = getDefaultFunction(
      _options.events.onViewChange,
      null
    );
  }
  function buildDefaultSearchOptions(newSearchOptions) {
    ...
    _options_Search.lastSearchText = getDefaultString(
      _options_Search.lastSearchText,
      _string.empty
    );
    _options_Search.not = getDefaultBoolean(_options_Search.not, false);
    _options_Search.matchCase = getDefaultBoolean(
      _options_Search.matchCase,
      false
    );
    _options_Search.showAdvanced = getDefaultBoolean(
      _options_Search.showAdvanced,
      false
    );
    _options_Search.searchTitle = getDefaultBoolean(
      _options_Search.searchTitle,
      true
    );
    _options_Search.searchLocation = getDefaultBoolean(
      _options_Search.searchLocation,
      false
    );
    _options_Search.searchDescription = getDefaultBoolean(
      _options_Search.searchDescription,
      false
    );
    _options_Search.searchGroup = getDefaultBoolean(
      _options_Search.searchGroup,
      false
    );
    _options_Search.searchUrl = getDefaultBoolean(
      _options_Search.searchUrl,
      false
    );
    _options_Search.startsWith = getDefaultBoolean(
      _options_Search.startsWith,
      false
    );
    _options_Search.endsWith = getDefaultBoolean(
      _options_Search.endsWith,
      false
    );
    _options_Search.contains = getDefaultBoolean(
      _options_Search.contains,
      true
    );
  ...
  function buildDefaulStringOptions() {
    ...
    _options.previousMonthTooltipText = getDefaultAnyString(
      _options.previousMonthTooltipText,
      "Previous Month"
    );
    _options.nextMonthTooltipText = getDefaultAnyString(
      _options.nextMonthTooltipText,
      "Next Month"
    );
    _options.previousDayTooltipText = getDefaultAnyString(
      _options.previousDayTooltipText,
      "Previous Day"
    );
    _options.nextDayTooltipText = getDefaultAnyString(
      _options.nextDayTooltipText,
      "Next Day"
    );
    _options.previousWeekTooltipText = getDefaultAnyString(
      _options.previousWeekTooltipText,
      "Previous Week"
    );
    _options.nextWeekTooltipText = getDefaultAnyString(
      _options.nextWeekTooltipText,
      "Next Week"
    );
    _options.addEventTooltipText = getDefaultAnyString(
      _options.addEventTooltipText,
      "Add Event"
    );
    _options.closeTooltipText = getDefaultAnyString(
      _options.closeTooltipText,
      "Close"
    );
    _options.exportEventsTooltipText = getDefaultAnyString(
      _options.exportEventsTooltipText,
      "Export Events"
    );
    _options.todayTooltipText = getDefaultAnyString(
      _options.todayTooltipText,
      "Today"
    );
    _options.refreshTooltipText = getDefaultAnyString(
      _options.refreshTooltipText,
      "Refresh"
    );
    _options.searchTooltipText = getDefaultAnyString(
      _options.searchTooltipText,
      "Search"
    );
    _options.expandDayTooltipText = getDefaultAnyString(
      _options.expandDayTooltipText,
      "Expand Day"
    );
    _options.viewAllEventsTooltipText = getDefaultAnyString(
      _options.viewAllEventsTooltipText,
      "View All Events"
    );
    _options.viewFullWeekTooltipText = getDefaultAnyString(
      _options.viewFullWeekTooltipText,
      "View Full Week"
    );
    _options.fromText = getDefaultAnyString(_options.fromText, "From:");
    _options.toText = getDefaultAnyString(_options.toText, "To:");
    _options.isAllDayText = getDefaultAnyString(
      _options.isAllDayText,
      "Is All-Day"
    );
    _options.titleText = getDefaultAnyString(_options.titleText, "Title:");
    _options.descriptionText = getDefaultAnyString(
      _options.descriptionText,
      "Description:"
    );
    _options.locationText = getDefaultAnyString(
      _options.locationText,
      "Location:"
    );
    ...
    _options.removeEventText = getDefaultAnyString(
      _options.removeEventText,
      "Remove"
    );
    _options.addEventTitle = getDefaultAnyString(
      _options.addEventTitle,
      "Add Event"
    );
    _options.editEventTitle = getDefaultAnyString(
      _options.editEventTitle,
      "Edit Event"
    );
    _options.exportStartFilename = getDefaultAnyString(
      _options.exportStartFilename,
      "exported_events_"
    );
    _options.fromTimeErrorMessage = getDefaultAnyString(
      _options.fromTimeErrorMessage,
      "Please select a valid 'From' time."
    );
    _options.toTimeErrorMessage = getDefaultAnyString(
      _options.toTimeErrorMessage,
      "Please select a valid 'To' time."
    );
    _options.toSmallerThanFromErrorMessage = getDefaultAnyString(
      _options.toSmallerThanFromErrorMessage,
      "Please select a 'To' date that is larger than the 'From' date."
    );
    _options.titleErrorMessage = getDefaultAnyString(
    ...
    _options.allEventsText = getDefaultAnyString(
      _options.allEventsText,
      "All Events"
    );
    _options.toTimeText = getDefaultAnyString(_options.toTimeText, "to");
    _options.confirmEventRemoveTitle = getDefaultAnyString(
      _options.confirmEventRemoveTitle,
      "Confirm Event Removal"
    );
    _options.confirmEventRemoveMessage = getDefaultAnyString(
      _options.confirmEventRemoveMessage,
      "Removing this event cannot be undone.  Do you want to continue?"
    );
    _options.okText = getDefaultAnyString(_options.okText, "OK");
    _options.exportEventsTitle = getDefaultAnyString(
      _options.exportEventsTitle,
      "Export Events"
    );
    _options.selectColorsText = getDefaultAnyString(
      _options.selectColorsText,
      "Select Colors"
    );
    _options.backgroundColorText = getDefaultAnyString(
      _options.backgroundColorText,
      "Background Color:"
    );
    _options.textColorText = getDefaultAnyString(
      _options.textColorText,
      "Text Color:"
    );
    _options.borderColorText = getDefaultAnyString(
      _options.borderColorText,
      "Border Color:"
    );
    _options.searchEventsTitle = getDefaultAnyString(
      _options.searchEventsTitle,
      "Search Events"
    );
    _options.previousText = getDefaultAnyString(
      _options.previousText,
      "Previous"
    );
    _options.nextText = getDefaultAnyString(_options.nextText, "Next");
    _options.matchCaseText = getDefaultAnyString(
      _options.matchCaseText,
      "Match Case"
    );
    _options.repeatsText = getDefaultAnyString(
      _options.repeatsText,
      "Repeats:"
    );
    _options.repeatDaysToExcludeText = getDefaultAnyString(
      _options.repeatDaysToExcludeText,
      "Repeat Days To Exclude:"
    );
    _options.daysToExcludeText = getDefaultAnyString(
      _options.daysToExcludeText,
      "Days To Exclude:"
    );
    _options.seriesIgnoreDatesText = getDefaultAnyString(
      _options.seriesIgnoreDatesText,
      "Series Ignore Dates:"
    );
    _options.repeatsNever = getDefaultAnyString(_options.repeatsNever, "Never");
    _options.repeatsEveryDayText = getDefaultAnyString(
      _options.repeatsEveryDayText,
      "Every Day"
    );
    _options.repeatsEveryWeekText = getDefaultAnyString(
      _options.repeatsEveryWeekText,
      "Every Week"
    );
    _options.repeatsEvery2WeeksText = getDefaultAnyString(
      _options.repeatsEvery2WeeksText,
      "Every 2 Weeks"
    );
    _options.repeatsEveryMonthText = getDefaultAnyString(
      _options.repeatsEveryMonthText,
      "Every Month"
    );
    _options.repeatsEveryYearText = getDefaultAnyString(
      _options.repeatsEveryYearText,
      "Every Year"
    );
    _options.repeatsCustomText = getDefaultAnyString(
      _options.repeatsCustomText,
      "Custom:"
    );
    _options.repeatOptionsTitle = getDefaultAnyString(
      _options.repeatOptionsTitle,
      "Repeat Options"
    );
    _options.moreText = getDefaultAnyString(_options.moreText, "More");
    _options.includeText = getDefaultAnyString(
      _options.includeText,
      "Include:"
    );
    _options.minimizedTooltipText = getDefaultAnyString(
      _options.minimizedTooltipText,
      "Minimize"
    );
    _options.restoreTooltipText = getDefaultAnyString(
      _options.restoreTooltipText,
      "Restore"
    );
    _options.removeAllEventsInSeriesText = getDefaultAnyString(
      _options.removeAllEventsInSeriesText,
      "Remove All Events In Series"
    );
    _options.createdText = getDefaultAnyString(
      _options.createdText,
      "Created:"
    );
    _options.organizerNameText = getDefaultAnyString(
      _options.organizerNameText,
      "Organizer:"
    );
    _options.organizerEmailAddressText = getDefaultAnyString(
      _options.organizerEmailAddressText,
      "Organizer Email:"
    );
    _options.enableFullScreenTooltipText = getDefaultAnyString(
      _options.enableFullScreenTooltipText,
      "Turn On Full-Screen Mode"
    );
    _options.disableFullScreenTooltipText = getDefaultAnyString(
      _options.disableFullScreenTooltipText,
      "Turn Off Full-Screen Mode"
    );
    _options.idText = getDefaultAnyString(_options.idText, "ID:");
    _options.expandMonthTooltipText = getDefaultAnyString(
      _options.expandMonthTooltipText,
      "Expand Month"
    );
    _options.repeatEndsText = getDefaultAnyString(
      _options.repeatEndsText,
      "Repeat Ends:"
    );
    _options.noEventsAvailableText = getDefaultAnyString(
      _options.noEventsAvailableText,
      "No events available."
    );
    _options.viewFullWeekText = getDefaultAnyString(
      _options.viewFullWeekText,
      "View Full Week"
    );
    _options.noEventsAvailableFullText = getDefaultAnyString(
    ...
    _options.toAddANewEventText = getDefaultAnyString(
    ...
    _options.configurationTooltipText = getDefaultAnyString(
      _options.configurationTooltipText,
      "Configuration"
    );
    _options.configurationTitleText = getDefaultAnyString(
      _options.configurationTitleText,
      "Configuration"
    );
    _options.groupsText = getDefaultAnyString(_options.groupsText, "Groups");
    _options.eventNotificationTitle = getDefaultAnyString(
      _options.eventNotificationTitle,
      "Calendar.js"
    );
    _options.eventNotificationBody = getDefaultAnyString(
      _options.eventNotificationBody,
      "The event '{0}' has started."
    );
    _options.optionsText = getDefaultAnyString(
      _options.optionsText,
      "Options:"
    );
    _options.startsWithText = getDefaultAnyString(
      _options.startsWithText,
      "Starts With"
    );
    _options.endsWithText = getDefaultAnyString(
      _options.endsWithText,
      "Ends With"
    );
    _options.containsText = getDefaultAnyString(
      _options.containsText,
      "Contains"
    );
    _options.displayTabText = getDefaultAnyString(
      _options.displayTabText,
      "Display"
    );
    _options.enableAutoRefreshForEventsText = getDefaultAnyString(
      _options.enableAutoRefreshForEventsText,
      "Enable auto-refresh for events"
    );
    _options.enableBrowserNotificationsText = getDefaultAnyString(
      _options.enableBrowserNotificationsText,
      "Enable browser notifications"
    );
    _options.enableTooltipsText = getDefaultAnyString(
      _options.enableTooltipsText,
      "Enable tooltips"
    );
    ...
    _options.enableDragAndDropForEventText = getDefaultAnyString(
      _options.enableDragAndDropForEventText,
      "Enable drag & drop for events"
    );
    _options.organizerTabText = getDefaultAnyString(
      _options.organizerTabText,
      "Organizer"
    );
    _options.removeEventsTooltipText = getDefaultAnyString(
      _options.removeEventsTooltipText,
      "Remove Events"
    );
    _options.confirmEventsRemoveTitle = getDefaultAnyString(
      _options.confirmEventsRemoveTitle,
      "Confirm Events Removal"
    );
    _options.confirmEventsRemoveMessage = getDefaultAnyString(
      _options.confirmEventsRemoveMessage,
    ...
    _options.optionalText = getDefaultAnyString(
      _options.optionalText,
      "Optional"
    );
    _options.urlText = getDefaultAnyString(_options.urlText, "Url:");
    _options.openUrlText = getDefaultAnyString(
      _options.openUrlText,
      "Open Url"
    );
    _options.thisWeekTooltipText = getDefaultAnyString(
      _options.thisWeekTooltipText,
      "This Week"
    );
    ...
    _options.repeatsByCustomSettingsText = getDefaultAnyString(
      _options.repeatsByCustomSettingsText,
      "By Custom Settings"
    );
    _options.lastUpdatedText = getDefaultAnyString(
      _options.lastUpdatedText,
      "Last Updated:"
    );
    _options.advancedText = getDefaultAnyString(
      _options.advancedText,
      "Advanced"
    );
    ...
    _options.duplicateText = getDefaultAnyString(
      _options.duplicateText,
      "Duplicate"
    );
    _options.showAlertsText = getDefaultAnyString(
      _options.showAlertsText,
      "Show Alerts"
    );
    _options.selectDatePlaceholderText = getDefaultAnyString(
      _options.selectDatePlaceholderText,
      "Select date..."
    );
    _options.hideDayText = getDefaultAnyString(
      _options.hideDayText,
      "Hide Day"
    );
    _options.notSearchText = getDefaultAnyString(
      _options.notSearchText,
      "Not (opposite)"
    );
    _options.showHolidaysInTheDisplaysText = getDefaultAnyString(
      _options.showHolidaysInTheDisplaysText,
      "Show holidays in the main display and title bars"
    );
    _options.newEventDefaultTitle = getDefaultAnyString(
      _options.newEventDefaultTitle,
      "* New Event"
    );
    _options.urlErrorMessage = getDefaultAnyString(
      _options.urlErrorMessage,
      "Please enter a valid Url in the 'Url' field (or leave blank)."
    );
    _options.searchTextBoxPlaceholder = getDefaultAnyString(
      _options.searchTextBoxPlaceholder,
      "Search title, description, etc..."
    );
    _options.currentMonthTooltipText = getDefaultAnyString(
      _options.currentMonthTooltipText,
      "Current Month"
    );
    _options.cutText = getDefaultAnyString(_options.cutText, "Cut");
    _options.showMenuTooltipText = getDefaultAnyString(
      _options.showMenuTooltipText,
      "Show Menu"
    );
    _options.eventTypesText = getDefaultAnyString(
      _options.eventTypesText,
      "Event Types"
    );
    ...
    _options.sideMenuHeaderText = getDefaultAnyString(
      _options.sideMenuHeaderText,
      "Calendar.js"
    );
    _options.sideMenuDaysText = getDefaultAnyString(
      _options.sideMenuDaysText,
      "Days"
    );
    _options.visibleDaysText = getDefaultAnyString(
      _options.visibleDaysText,
      "Visible Days"
    );
    _options.previousYearTooltipText = getDefaultAnyString(
      _options.previousYearTooltipText,
      "Previous Year"
    );
    _options.nextYearTooltipText = getDefaultAnyString(
      _options.nextYearTooltipText,
      "Next Year"
    );
    _options.showOnlyWorkingDaysText = getDefaultAnyString(
      _options.showOnlyWorkingDaysText,
      "Show Only Working Days"
    );
    _options.exportFilenamePlaceholderText = getDefaultAnyString(
      _options.exportFilenamePlaceholderText,
      "Name (optional)"
    );
    ...
    _options.configurationUpdatedText = getDefaultAnyString(
      _options.configurationUpdatedText,
      "Configuration updated."
    );
    _options.eventAddedText = getDefaultAnyString(
      _options.eventAddedText,
      "{0} event added."
    );
    _options.eventUpdatedText = getDefaultAnyString(
      _options.eventUpdatedText,
      "{0} event updated."
    );
    _options.eventRemovedText = getDefaultAnyString(
      _options.eventRemovedText,
      "{0} event removed."
    );
    _options.eventsRemovedText = getDefaultAnyString(
      _options.eventsRemovedText,
      "{0} events removed."
    );
    _options.eventsExportedToText = getDefaultAnyString(
      _options.eventsExportedToText,
      "Events exported to {0}."
    );
    _options.eventsPastedText = getDefaultAnyString(
      _options.eventsPastedText,
      "{0} events pasted."
    );
    _options.eventsExportedText = getDefaultAnyString(
      _options.eventsExportedText,
      "Events exported."
    );
    _options.copyToClipboardOnlyText = getDefaultAnyString(
      _options.copyToClipboardOnlyText,
      "Copy to clipboard only"
    );
    _options.workingDaysText = getDefaultAnyString(
      _options.workingDaysText,
      "Working Days"
    );
    _options.weekendDaysText = getDefaultAnyString(
      _options.weekendDaysText,
      "Weekend Days"
    );
    _options.showAsBusyText = getDefaultAnyString(
      _options.showAsBusyText,
      "Show As Busy"
    );
    _options.selectAllText = getDefaultAnyString(
      _options.selectAllText,
      "Select All"
    );
    _options.selectNoneText = getDefaultAnyString(
      _options.selectNoneText,
      "Select None"
    );
    _options.importEventsTooltipText = getDefaultAnyString(
      _options.importEventsTooltipText,
      "Import Events"
    );
    _options.eventsImportedText = getDefaultAnyString(
      _options.eventsImportedText,
      "{0} events imported."
    );
    _options.viewFullYearTooltipText = getDefaultAnyString(
      _options.viewFullYearTooltipText,
      "View Full Year"
    );
    _options.currentYearTooltipText = getDefaultAnyString(
      _options.currentYearTooltipText,
      "Current Year"
    );
    _options.alertOffsetText = getDefaultAnyString(
      _options.alertOffsetText,
      "Alert Offset (minutes):"
    );
    _options.viewFullDayTooltipText = getDefaultAnyString(
      _options.viewFullDayTooltipText,
      "View Full Day"
    );
    _options.confirmEventUpdateTitle = getDefaultAnyString(
      _options.confirmEventUpdateTitle,
      "Confirm Event Update"
    );
    _options.confirmEventUpdateMessage = getDefaultAnyString(
      _options.confirmEventUpdateMessage,
    ...
    _options.viewTimelineTooltipText = getDefaultAnyString(
      _options.viewTimelineTooltipText,
      "View Timeline"
    );
    _options.nextPropertyTooltipText = getDefaultAnyString(
      _options.nextPropertyTooltipText,
      "Next Property"
    );
    ...
    _options.shareStartFilename = getDefaultAnyString(
      _options.shareStartFilename,
      "shared_events_"
    );
    _options.previousPropertyTooltipText = getDefaultAnyString(
      _options.previousPropertyTooltipText,
      "Previous Property"
    );
    _options.jumpToDateTitle = getDefaultAnyString(
      _options.jumpToDateTitle,
      "Jump To Date"
    );
  ...
  (function (
    ...
    if (
      isDefinedString(_parameter_ElementID) ||
      isDefinedDOMElement(_parameter_ElementID)
    ) {
      ...
      if (
        _element_Calendar !== null &&
        isDefinedBoolean(_options.openInFullScreenMode) &&
        _options.openInFullScreenMode &&
        !_element_Mode_DatePicker_Enabled
      ) {
        forceTurnOnFullScreenMode();
      }
