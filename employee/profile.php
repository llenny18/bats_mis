...
if (isset($_POST['changeicon']) && isset($_FILES['upload'])) {
  $employee_id = $_SESSION['employee_id'];
  $file = $_FILES['upload'];

  // Validate file
  $allowedTypes = ['image/jpeg', 'image/png', 'image/gif'];
  if (in_array($file['type'], $allowedTypes) && $file['size'] <= 800000) {
    // Read the file content
    $fileContent = file_get_contents($file['tmp_name']);

    // Update query
    $stmt = $conn->prepare("UPDATE employeedetails SET user_icon = ? WHERE employee_id = ?");
    $stmt->bind_param("si", $fileContent, $employee_id);

    if ($stmt->execute()) {
      echo "<script>alert('User icon updated successfully'); window.location.href = 'profile.php';</script>";
    } else {
      echo "<script>alert('Error updating user icon: " . $stmt->error . "');</script>";
    }
  } else {
    echo "<script>alert('Invalid file type or size. " . $file['size'] . "Allowed types: JPG, GIF, PNG. Max size: 800K.');</script>";
  }
}
    ...
    <div class="layout-container">
      ...
      <div class="content-wrapper">
        ...
        <div class="container-xxl flex-grow-1 container-p-y">
          <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Account Settings /</span> Profile <a href="view-efiles.php?user_id=<?= $_SESSION['employee_id'] ?>" class="btn btn-primary"> View my Files </a></h4>
                ...
                <form id="formAccountSettings" method="POST" enctype="multipart/form-data">
                  <div class="card-body">
                    <div class="d-flex align-items-start align-items-sm-center gap-4">
                      <img src="data:image/jpeg;base64, <?php echo base64_encode($userData['user_icon']) ?>" alt="user-avatar" class="d-block rounded" height="100" width="100" id="uploadedAvatar" />
                      <div class="button-wrapper">
                        <label for="upload" class="btn btn-primary me-2 mb-4" tabindex="0">
                          <span class="d-none d-sm-block">Upload new photo</span>
                          <i class="bx bx-upload d-block d-sm-none"></i>
                          <input type="file" id="upload" name="upload" hidden accept="image/png, image/jpeg" />
                        </label>
                        <button type="submit" class="btn btn-outline-secondary account-image-reset mb-4" name="changeicon">
                          <i class="bx bx-check-circle d-block d-sm-none"></i>
                          <span class="d-none d-sm-block">Change Icon</span>
                        </button>
                  ...
                  <form id="formAccountSettings" method="POST">
                    <div class="row">
                      ...
                      <div class="mb-3 col-md-6">
                        ...
                        <input class="form-control" type="text" id="email" name="email" value="<?= $userData['email']  ?>" placeholder="john.doe@example.com" />
                      </div>
                      <div class="mb-3 col-md-6">
                        <label for="organization" class="form-label">Employment Type</label>
                        <input type="text" class="form-control" id="organization" name="employment_type" value="<?= $userData['employment_type']  ?>" />
                      </div>
                      <div class="mb-3 col-md-6">
                        <label class="form-label" for="phoneNumber">Phone Number</label>
                        <div class="input-group input-group-merge">
                          <span class="input-group-text">PH (+639)</span>
                          <input type="text" id="phoneNumber" name="phone_number" class="form-control" placeholder="0934234422" value="<?= $userData['phone_number']  ?>" />
                        </div>
                      </div>
                      <div class="mb-3 col-md-6">
                      ...
                      <div class="mb-3 col-md-6">
                        ...
                        <input type="text" class="form-control" id="zipCode" name="password" placeholder="231465" value="<?= $userData['user_password']  ?>" />
                      </div>
